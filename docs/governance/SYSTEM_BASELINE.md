...
**名称**: AgomSAAF (Agom Strategic Asset Allocation Framework)
...
| 指标 | 数值 | 来源 |
|------|------|------|
| **系统版本** | V3.5 | `core/settings/base.py` |
| **发布状态** | 生产就绪 | `docs/development/system-review-report.md` |
| **最后更新** | 2026-03-18 | Git log |
| **业务模块** | 32个 | `apps/` 目录扫描 |
| **代码行数** | 50,000+ | 代码统计 |
| **API 端点** | 100+ | API 扫描 |
...
**基础设施层 (4个)**: `regime`, `ai_provider`, `events`, `macro`

**核心业务层 (13个)**: `signal`, `policy`, `sentiment`, `filter`, `alpha_trigger`, `beta_gate`, `alpha`, `factor`, `hedge`, `rotation`, `sector`, `agent_runtime`, `task_monitor`

**资产分析层 (3个)**: `asset_analysis`, `equity`, `fund`

**应用集成层 (10个)**: `backtest`, `audit`, `dashboard`, `prompt`, `realtime`, `market_data`, `terminal`, `strategy`, `decision_rhythm`, `share`

**顶层聚合层 (2个)**: `simulated_trading`, `account`

### 3.2 完整清单（按字母序）

```
account, ai_provider, agent_runtime, alpha, alpha_trigger, 
asset_analysis, audit, backtest, beta_gate, dashboard, 
decision_rhythm, equity, events, factor, filter, 
fund, hedge, macro, market_data, policy, 
prompt, realtime, regime, rotation, sector, 
sentiment, share, signal, simulated_trading, strategy, 
...
```
宏观数据采集 → Regime 判定 → Policy 档位 → Alpha 选股 → 信号生成 → 模拟盘交易 → 事后审计
    ↓              ↓             ↓            ↓            ↓            ↓            ↓
  macro         regime        policy       alpha       signal   simulated_trading  audit
```
...
外部数据源 → Market Data 统一接口 → 各模块消费 → 数据库存储 → 审计日志
     ↓               ↓                  ↓             ↓            ↓
 Tushare/AKShare  market_data      regime/policy   SQLite      audit
 东方财富/Redis
```
