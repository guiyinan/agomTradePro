...
> 2026-08-11 完整路线图复审与软件续批：三组 Luna Max 再次按 R1—R8 目标、最低验收和全局研究纪律逐项核对，未发现 P0。Readiness 静态机制证据改为显式 allowlist，真实 outcome、Portfolio snapshot 和 execution feedback 不能再由 manifest 冒充 verified。R1 baseline trial 评估改用 trusted clock、单一 shared UoW/atomic 和完整 owner graph 双读，caller `as_of` 仅作 PIT cutoff；入口对 command 现场 exact-type/live 重验。R4 新增服务端三候选构造，从同一 sealed exposure/factor-covariance/asset-covariance/constraint/cost 图计算等权、资产风险平价与宏观因子风险平价，不接受 caller 预填权重。三项均保持 research-only/blocked，不替代真实 Publication、PIT、Promotion 或 consumer 验收。
> 2026-08-11 R1 actual owner 续批：Data Center `0068` 新增版本化 actual source definition 与 materialized manifest receipt 两本 append-only、schema-only、zero-seed ledger。Definition 显式封存 dataset/subject/industry、calendar 三元组、period×metric member/vintage 规则和 coverage policy；materialization 只能在 shared UoW 中双读 canonical fact/member provider 后内部构造完整 snapshot。Production registration/materialization 保持无状态 unavailable，exact PIT provider 空库返回 `None`，不使用现有信息不足的 PIT 表伪造 actual。真实 member/vintage source 和 Research trial authorization provider 仍缺，R1 保持 `blocked`。
> 2026-08-11 R7 Forecast Ledger owner 续批：Signal `0011` 新增 realization manifest header 与 assessment-scoped member receipt 两本 append-only、schema-only、zero-seed ledger；写入时锁定并 exact 重读既有 immutable ForecastOutcome/ForecastLedgerEntry，不复制或接受 caller 自报 outcome。Research 窄适配器只把严格 PIT receipt 投影为 R7 monitoring realization；缺 receipt/source metadata 时返回 `None`。Public append 是无状态 unavailable façade，真实 writer 仅存在于非导出 test factory，不发布 probability/current/decision/execution；真实 outcome 历史、source metadata、Risk Center policy 与 authorization 仍缺，R7 保持 `blocked`。
> 2026-08-11 R4 monitoring canonical owner 续批：Research `0019` 新增 policy/calendar 两本 append-only owner registry，Portfolio `0012` 新增 raw-fact receipt ledger，均为 schema-only、zero-seed。三条 ID-only registration 在同一 shared UoW/atomic 内使用 trusted clock、完整 owner graph 双读和动态 participant 复核；现有 R4 active decision、Portfolio result 与 R3 attestation 通过 exact PIT adapter 组装 monitoring graph。Public registration 和 canonical monitoring mutation 均为无状态 unavailable façade，真实 writer/token 只存在于非导出 test runtime；空库或真实 owner 缺失时零写且继续 `blocked`，不接 current/decision/execution/consumer。
> 2026-08-11 R1 Research trial prereg 续批：Research `0020` 新增单本 derived prereg evidence ledger，精确依赖 `0019`、schema-only、zero-seed。ID-only registration 在 shared UoW/atomic 内由 trusted clock 对 canonical trial definition、approved baseline spec 与 artifact 完整图双读后内部构造 evidence；strict codec、exact PIT Equity provider、winner/fork/rollback 与 append-only guards 均 fail closed。Public registration inert，空库返回 `None`；真实 definition/spec/artifact owner composition 未接入，故不把通用 ExperimentTrial 或事后 Promotion receipt 冒充 trial authorization，R1 继续 `blocked`。
> 2026-08-11 R7 realization source-definition 续批：Signal `0012` 新增 source definition header/member 两本 append-only、schema-only、zero-seed ledger；既有 `0011` writer 不再接受 caller source provider，只能 exact 读取 `0012` registry。Registration 为 ID-only、shared-UoW 双读和 trusted clock，exact query 绑定完整 content hash/PIT；public mutation inert，空 registry 零写。现有 Forecast Ledger 仍不能提供独立 calendar hash、完整 PIT manifest/member metadata 等真源字段，因此 canonical production provider保持 unavailable，R7 继续 `blocked`。
> 2026-08-11 R2 trial-policy owner 续批：Research `0021` 新增 selection 前的单本 append-only、schema-only、zero-seed policy registry，严格复用 Phase A policy factory/seal。ID-only registration 在 shared UoW/atomic 内以 trusted clock 对 owner definition 双读，exact PIT provider可直接满足既有 R2 evaluator；public mutation保持 inert，空库返回 `None`。真实 Publication、两个完整市场周期、Audit outcome与owner authorization仍缺，R2继续 `blocked`。
> 2026-08-11 R5 research-control preflight 续批：新增只读 preflight，将 exact active Promotion/lifecycle、fixed-income owner seal 与最新完整 monitoring assessment 在 shared UoW 内双读后合并；调用方不能选择 assessment 或 health。只有健康证据可进入 `ELIGIBLE_FOR_MANUAL_CONSUMER_REVIEW`，其余一律 `BLOCKED`，并固定禁止 current/decision/execution。Portfolio canonical raw-source owner仍缺，production composition保持零写，R5继续 `blocked`。
> 2026-08-11 R8 monitoring calendar owner 续批：Portfolio `0013` 新增单本 append-only、schema-only、zero-seed canonical monitoring calendar registry，并补 active result 与 canonical input receipt 的窄只读 adapter。Registration 为 ID-only、shared-UoW 双读和 trusted clock，exact PIT 查询绑定完整 calendar/source seal；public mutation inert。Research policy/Promotion与Portfolio/Broker feedback/raw facts仍不可用，monitoring保持 `BLOCKED`，R8总门禁不变。
> 2026-08-11 R1 evaluation preflight 续批：Equity 新增纯只读 production preflight，在单一 shared UoW/atomic 内以 trusted clock 双读本地 spec/artifact、Data Center `0068` actual 与 Research `0020` trial prereg。完整图只发布 prospective candidate hash 与 `READY/BLOCKED`，不持久化 trial、不生成 Promotion，也不暴露 current/decision/execution；空库稳定列出四类 canonical blocker，R1继续 `blocked`。
> 2026-08-11 R5 canonical monitoring owner 续批：Research `0022` 新增 policy/calendar 两本 registry，Portfolio `0014` 新增完整 sealed raw-fact receipt，均为append-only、schema-only、zero-seed。三条 ID-only registration 使用 shared-UoW、trusted clock、definition/source双读、exact PIT、winner/fork/rollback与ORM guards；private composition已串接Phase A、Phase B和research-control preflight，public mutation保持using-only inert。真实owner source、Publication/PIT/OOS与consumer验收仍缺，R5继续 `blocked`。
> 2026-08-11 R8 Broker reconciliation owner 续批：Broker Execution `0007` 新增单本 append-only、schema-only、zero-seed period receipt，严格封存三项反馈的raw numerator/denominator、result/input-receipt/calendar/period/plan/reconciliation identity与source/trusted clocks。既有Fill/Run/Difference缺canonical成本分子、Portfolio plan reference和comparison denominator，禁止反推；public registration inert，缺独立receipt时adapter返回`None`并令monitoring保持`BLOCKED`，R8总门禁不变。
> 2026-08-11 第四批 R2 research-control preflight：新增纯只读、无迁移的 production preflight，以 policy identity/hash/as-of 为唯一选择器，在单一 shared UoW/atomic 内双读 Research `0021` policy 与 Research `0016` 服务端选出的 latest-complete trial/monitoring pair；调用方不能指定 assessment，same-rank fork、证据替换、future cutoff 与 UoW 漂移均 fail closed。Data Center 完整 Publication/calendar/cycle projection 与 Audit outcome/fact owner 仍缺，production 稳定 `BLOCKED` 且零写，R2 总门禁不变。
> 2026-08-11 第四批 R6 manual-activation preflight：新增纯只读 production preflight，将 Research `0008` active qualification、`0011` latest-complete healthy monitoring 与 `0012` exact activation state 在 shared UoW/trusted clock 下双读，只允许进入 `ELIGIBLE_FOR_MANUAL_ACTIVATION_REVIEW`，不直接 ACTIVATE、RETIRE、发布 current 或执行。由于 canonical scope owner 与 scope→qualification 映射尚不存在，公开 using-only runtime 固定 fail closed，R6 继续 `blocked`。
> 2026-08-11 第四批 R8 monitoring owner 续批：Research `0023` 新增独立 policy registry，Portfolio `0015` 新增七类 source member/八项 raw-derived feedback receipt；与 Broker `0007` 三项 reconciliation feedback 合计覆盖 Phase A 的 11 项 metric 输入。所有数值保存 raw numerator/denominator、member identity、source/available/trusted clocks 与 strict seals，禁止从 assessment 快照反充 owner。Public registration using-only inert，private graph 在 R3/R4/R5 Promotion owner 缺失时保持 `BLOCKED`，不开放 Phase B writer、consumer、current、decision 或 execution。
> 2026-08-12 第五批 R3 PIT source owner 续批：Data Center `0069` 新增 schema-only、zero-seed source/calendar/period-member registry，完整封存 target/proxy value decoding、unit、coverage 与 inference period。ID-only registration 在 shared UoW/trusted clock 下对 owner definition 全过程动态重读；Macro Factor 窄适配器只能从 exact PIT manifest/fact rows 重建，缺字段、clock、coverage或同member多revision一律返回`None`，不猜latest。Public mutation inert，不接runner/current/decision/execution，R3继续`blocked`。
> 2026-08-12 第五批 R4 research-control preflight：新增无迁移纯只读 production preflight，以 `scope_id/as_of` 服务端选择 active Promotion 与 latest-complete monitoring唯一winner，在 shared UoW/trusted clock 下重放完整六owner graph。Same-rank fork、tamper、owner drift、非健康或缺owner均`BLOCKED`；只允许进入人工consumer review，不开放consumer/current/decision/execution，R4总门禁不变。
> 2026-08-12 第五批 R7 calibration sample owner：Signal `0013` 新增definition/expected-membership/receipt/member四本append-only、schema-only、zero-seed ledger，完整分母逐member显式记录`resolved/unresolved/censored/invalidated`，caller不能提交raw outcome或probability。Owner graph在shared UoW内三次exact重读并统一UTC canonical clock；public mutation inert，Research仅窄只读adapter。真实Forecast history/source metadata、Risk Center approved policy及analogy/path owner仍缺，R7继续`blocked`。
> 2026-08-12 第六批 R3 Regime assignment owner：Regime `0010` 新增definition/assignment receipt两本append-only、schema-only、zero-seed ledger，封存policy、source-contract、完整OOS row×三fact规则与PIT/source clocks；assignment只能由artifact+canonical facts四次exact重读现场派生。Public runtime只暴露窄exact-read repository，writer/token仅private test composition；Macro Factor adapter按artifact/hash/as-of投影治理报告。真实Regime owner definition、宏观事实历史、trial/Promotion仍缺，R3继续`blocked`。
> 2026-08-12 第六批 R5 production read composition：public research-control preflight不再顶层硬编码active unavailable，而是按scope/as-of重放Research `0006` lifecycle，并exact复核policy、trial、authorization与Fixed Income owner。Portfolio upstream canonical outcome source仍缺，阻断被下沉到该owner叶并返回`BLOCKED`；public graph只读且零写，不开放consumer/current/decision/execution，R5总门禁不变。
> 2026-08-12 第七批 R6/R7 owner 软件边界：Research `0024` 登记scope→qualification canonical binding并接入manual-activation preflight；Research `0025`以六张schema-only、zero-seed表分别封存analogy与path definition/raw receipt/member图。R7 similarity与conditional/transition probability均由完整PIT raw成员现场派生，caller不能提交score/probability成品；public registration继续inert、exact read为空或fork时返回`None`并使下游`BLOCKED`。真实scope definition、analogy/path历史源、Promotion与consumer仍未形成。
> 2026-08-12 第八批软件收口：R7 result private canonical composition已直接接入Signal calibration与Research `0025` analogy/path exact providers，缺任一owner时事务内零result写；同时关闭R2 preflight participant双真源、R3 Regime adapter持writer、R6 qualification public builder暴露caller authority三项审计债务。所有public mutation继续inert/read-only，真实Publication/PIT/OOS/Promotion/consumer阻断不变。
> 2026-08-12 第八批终核补充：R6 active qualification lifecycle查询已把`recorded_at <= as_of`下推至ORM，再对可见prefix做restore/duplicate/fork校验；未来损坏事件不再污染历史PIT，到达对应cutoff后仍稳定corruption。终核未发现剩余无真实数据可闭合的软件P0/P1。
...
> 2026-08-10 R5 monitoring Phase B：Research `0014` 新增 assessment-scoped observation、assessment 与 immutable audit snapshot 三张 schema-only append-only ledger，zero seed。ID/as-of-only writer 在 shared UoW 内重读 active lifecycle、fixed-income result、policy、calendar 与 Portfolio raw facts并现场复算；existing winner 在读取新 server clock 前完整重放，支持时钟前进、回退或异常后的幂等恢复。Strict codec、owner/server 双时钟、exact PIT、签名审计分页、race/fork/rollback、ORM/Collector guard与正反迁移均已验证。Production composition 不接假 owner provider，不自动 RETIRE、不发布 current/decision/execution，R5 保持 `blocked`。
> 2026-08-10 R7 production composition 能力面收口：sample policy、research result 与 lifecycle 的公开 builder 仅接受数据库别名，mutation façade 为无状态 ID-only unavailable surface，公开对象图不再保留 provider、clock、writer、store 或 token；可注入成功路径仅保留为非导出的 component-test factory。Exact read 与 audit query 仍可用，真实 Risk Center/Research owner source 未接入时所有写入保持 zero-write blocked。该收口不补 monitoring 或 family rollback，R7 总门禁不变。
> 2026-08-10 R3 authoritative runner-spec persistence：Research `0015` 新增完整 `MacroFactorRunnerSpec` append-only ledger，strict codec 无损重建 target、candidate、temporal-CV、benchmark、cost、freshness、validity 与 reproducibility 全语义。ID-only writer 在 shared UoW 内执行 owner 首读、trusted server clock、owner 二读、动态 UoW 复核和 exact winner replay；ledger time 必须早于首次 selection。Production 缺真实 definition provider 时注册 façade保持无状态 unavailable，exact read/provider只返回严格恢复的完整 spec；不接 manifest、dataset、run、current、Promotion 或执行，R3 保持 `blocked`。
> 2026-08-10 R7 post-promotion monitoring Phase A：新增纯 Domain 合同，active result 只能从带 exact head/count attestation 的完整 lifecycle stream 重放，截断 RETIRE tail 或直接自造 projection 均拒绝。预测严格来自 active result 的 pre-period subjective/model projection；兑现事实来自独立 Forecast Ledger owner record，逐 observation 封存 identity/version/hash、horizon、available/recorded/PIT clocks。完整连续 calendar 与 exact member、两类 coverage/Brier、PIT cutoff和研究安全标志均现场重验；缺 calibration/analogy/path 证据时稳定 `BLOCKED`，不以伪聚合补齐。该批无 Application/ORM/persistence、不会发布概率/current/decision/execution，真实 Forecast Ledger、获批 policy 和 lifecycle authorization 仍缺，R7 保持 `blocked`。
> 2026-08-10 R2 trial/monitoring Phase B：Research `0016` 新增 explanatory trial、assessment-scoped monitoring observation、monitoring assessment 与 immutable audit snapshot 四张 schema-only append-only ledger，zero seed。Phase A 六类 owner/clock 全部纳入 shared UoW，事务内双读；persistence 在 append 紧前再次完整取证、比较全 evidence并复核 UoW，owner 漂移保持 zero-write。Strict codec、server-ledger clock、exact PIT、first-winner/fork、签名审计快照、ORM/Collector guard及 0015↔0016 正反迁移均已验证。Production mutation/audit façade保持无状态 unavailable，不复用或升级旧 `0009` v1 Promotion；真实 Publication、两个市场周期 outcome、Audit owner与授权仍缺，R2 保持 `blocked`。
> 2026-08-10 R7 post-promotion monitoring Phase B 与 family rollback：Research `0017` 新增 assessment、assessment-scoped observation 与 immutable audit snapshot 三张 schema-only ledger；完整 result/lifecycle head attestation、calendar、Forecast Ledger realization 与 assessment graph 在 shared UoW 中重复重放，exact PIT、first-winner/fork、签名 audit 和 public inert façade均 fail closed。独立 family v2 lifecycle 只允许 exact `stack[-2]` rollback，禁止 A→B→A 重复入栈，并在 append 前最终重读 authorization 与完整 owner graph。两轮 Luna Max 终核均为 P0/P1=0；真实 Forecast Ledger、policy、authorization 与 outcome 历史仍缺，R7 保持 `blocked`。
> 2026-08-10 R3 governed-read 与 R7 family persistence 收口：R3 governed read 现以 trusted clock、单一 shared UoW 和完整 owner graph 双读封闭 Regime/trial/Promotion/monitoring 的读取时竞态，provider、clock、ledger异常统一返回稳定 blocker。Research `0018` 为 R7 family v2 lifecycle 新增 authorization receipt、event、stream commit 与 immutable audit snapshot 四张 schema-only ledger；exact winner 使用首次封存 ledger cutoff，不依赖重试时钟，family restore 同时重验 canonical result 与本地 authorization↔event owner graph。Production apply/audit 保持无状态 unavailable；真实 family owner、authorization、Forecast outcome 与 consumer 仍缺，R3/R7 继续 `blocked`。
> 2026-08-10 R3/R6 production authority 与 R3 ledger 完整性收口：R3、R6 public production builder 仅保留数据库别名与无状态 unavailable mutation façade，可注入 owner/clock/writer 仅存在于非导出 test factory。R3 三类 exact read 统一恢复完整 source/artifact/output/lifecycle graph；`macro_factor.0003` 以 cumulative stream commit 加独立 per-artifact latest head 封存 lifecycle count、event/commit hash 与 stream hash，因此 output、artifact、event、commit 或配对 tail 截断均 fail closed。0003 正反迁移为 zero-seed，R3/R6 仍不接真实 owner、current、decision 或 execution。
> 2026-08-10 R6 monitoring capability 二次收口：公开 exact-read repository 仅保存数据库别名，不再持有 write token、可注入 clock或激活写 UoW；production audit 改为无状态 unavailable façade，live-ledger audit仅留在非导出的测试 store。公开 runtime 对象图无法递归取得 store/token/clock，合法或畸形 audit 请求均在写入前阻断。真实 canonical owner、monitoring facts、Promotion/authorization 与 consumer 仍缺，R6 保持 `blocked`。
