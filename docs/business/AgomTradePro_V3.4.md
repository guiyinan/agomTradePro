# 📑 AgomTradePro 项目需求与系统架构全手册 (V3.4 实施版)
...
>   - 🗺️ 路线图标注各阶段开发/部署模式
> - V3.4.1（2026-02-28）：导航与路由口径收敛
>   - 🧭 宏观环境入口统一为“政策/情绪/热点工作台”（`/policy/workbench/`）
>   - 💼 投资管理账户入口统一为“我的投资账户”
>   - 📚 API 文档入口仅保留“系统”菜单中的 `/api/docs/`
>   - 🔗 页面导航统一使用 Django `{% url %}`，禁止硬编码业务路径与直连业务 API

---

## 1. 项目概览 (Project Overview)

* **项目代号**：AgomTradePro (Agom Strategic Asset Allocation Framework)
...
### 1.2 导航与入口治理（2026-02-28）

为保证 UI 一页通与路由稳定性，导航层执行以下统一规则：

1. 宏观环境中的政策相关入口统一为：`政策/情绪/热点工作台`，页面路由：`/policy/workbench/`。
2. 投资管理中的账户入口统一文案为：`我的投资账户`。
3. API 文档入口只保留在“系统”菜单：`/api/docs/`；其他位置不重复展示。
4. 页面模板中的业务页面导航链接必须使用 Django `{% url %}` 反解，不允许硬编码业务路径。
...
# DATABASE_URL=postgres://user:password@localhost:5432/agomtradepro  # 生产环境
...
services:
  ...
  web:
    ...
    ports:
      - "8000:8000"
    environment:
      - DATABASE_URL=postgres://agomtradepro:${POSTGRES_PASSWORD}@db:5432/agomtradepro
      - REDIS_URL=redis://redis:6379/0
      - DEBUG=False
    depends_on:
      - db
...
说明：`sdk/agomtradepro` 与 `sdk/agomtradepro_mcp` 为系统对外接入层，独立于 `apps/` 业务模块统计口径。
...
> **本章新增于 V3.5**：详细描述AgomTradePro的多层级风控机制，包括14项已实现功能和待增强的风控能力。

### 12.1 风控框架总览

AgomTradePro 的风险管理采用**多层级、全流程**的防御体系：
...
```
你是一个 AgomTradePro 项目的开发者。请严格遵守以下约束：
