...
> 最后更新：2026-06-28
> 状态：V1 已落地，已接入账户止盈止损、模拟盘自动交易买入前检查、策略执行编排买入前检查、投后风控巡检、风控/持仓日报、Web 控制台、TUI、SDK/MCP。
...
- 日报归档：风控日报和持仓日报按 `account_id + report_date` 留存，支持单日和区间查询。

V1 的目标不是替换所有交易模块，而是提供统一配置、统一解析、统一前置风控门、投后巡检入口和日报输出。后续新增交易入口必须优先调用 `risk_center.application.trade_guard.EvaluatePreTradeRiskUseCase` 或对应 SDK/MCP 能力；账户健康页、自动投顾和定时监控应复用 `EvaluatePostInvestmentRiskUseCase`；日报类输出应复用 `GenerateRiskCenterDailyReportUseCase`。
...
| `POST` | `/api/risk-center/daily-report/` | 生成风控日报和持仓日报，并按账户和日期归档 |
| `GET` | `/api/risk-center/daily-report/?account_id=...&report_date=YYYY-MM-DD` | 查询某账户某一天的日报 |
| `GET` | `/api/risk-center/daily-report/?account_id=...&start_date=YYYY-MM-DD&end_date=YYYY-MM-DD` | 查询某账户日报历史；管理员可不传账户查询全部，普通用户只返回有权限账户 |
...
- 基于同一份日终快照生成风控日报和持仓日报。
- 按账户、单日或日期区间查询已归档日报历史。
- 创建管理员例外。
...
- `generate_daily_report(payload)`
- `get_daily_report(account_id, report_date)`
- `list_daily_reports(account_id=None, report_date=None, start_date=None, end_date=None, limit=None)`
...
| `generate_risk_center_daily_report` | 生成风控日报和持仓日报 |
| `get_risk_center_daily_report` | 按账户和日期读取已归档日报 |
| `list_risk_center_daily_reports` | 查询已归档日报历史，支持账户、单日和日期区间过滤 |
...
- `risk-center.daily-report`
- `risk-center.daily-report-history`
...
7. 日终任务需要输出日报时，优先调用 `/api/risk-center/daily-report/` 或 `GenerateRiskCenterDailyReportUseCase`，日报必须同时包含 `risk_daily_report` 和 `position_daily_report`，并按 `account_id + report_date` 归档，方便后续复盘查询。
...
- Application/API 日报：风控日报、持仓日报、底层投后巡检结果、单日读取和区间历史查询。
- 自动交易：风控中心拒绝时不调用买入用例。
