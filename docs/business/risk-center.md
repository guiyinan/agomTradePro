...
| `POST` | `/api/risk-center/daily-report/` | 生成风控日报和持仓日报 |
...
7. 日终任务需要输出日报时，优先调用 `/api/risk-center/daily-report/` 或 `GenerateRiskCenterDailyReportUseCase`，日报必须同时包含 `risk_daily_report` 和 `position_daily_report`。
...
- Application/API 日报：风控日报、持仓日报和底层投后巡检结果。
- 自动交易：风控中心拒绝时不调用买入用例。
