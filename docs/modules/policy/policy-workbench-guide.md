...
**关键约束**：
- 热点情绪事件 **不直接修改** P0-P3 政策档位
- 政策当前档位 **只计算已生效**（`gate_effective=True`）的政策事件
...
test_hotspot_events_do_not_affect_policy() # 热点事件不影响政策档位
...
A: 设计上，政策档位（P0-P3）仅反映宏观政策风险，热点情绪闸门（L0-L3）反映市场情绪风险。两者独立运行，互不影响。
