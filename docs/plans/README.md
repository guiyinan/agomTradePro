...
| 2026-08-13 | 第一期 P0 | Evidence M0 Transition Plan internal writer freeze | 冻结 5 个默认启用的 legacy build/save/upsert/status/approval-request writer 与 4 个 canonical build/validate/save/approve writer；固定 replacement 与关键 AST calls，`6 passed` | 冻结不等于批准 legacy upsert；后续将旧写链迁到 Portfolio facade 并关闭默认 legacy 路径 |
