...
- 随后的真实时钟复核发现生产 `latest_completed_cn_market_session()` 返回空：Tushare 自定义 relay 的 `trade_cal` 响应为空表并触发 `MODEL_MARKET_CALENDAR_SCHEMA_INVALID`，而 AKShare model source 只有 `trade_days`，未实现带来源、覆盖范围和观测时间的 `TradingCalendarEvidencePort`，所以日历路由没有可用 failover。最终候选从 AKShare `tool_trade_date_hist_sina` 的完整开放日序列构造严格证据：源序列必须跨越请求窗口两端，任一非法日期、重复会话或截断都用稳定业务码阻断；不得用工作日近似或请求日期制造覆盖。2026-09-26 的真实读取证明请求窗口 `2026-08-12..2026-09-26` 有 32 个开放日，最近一日为 `2026-09-24`，与中秋休市窗口一致。组合测试覆盖 Tushare schema 失败后正常切至 AKShare。
- `513767b240` 的唯一运行 S6 使用隔离 PostgreSQL、目标交易日 `2026-09-24` 和同 SHA 的主分支 PostgreSQL run，候选镜像 `sha256:5e78a534…b665` 构建及身份校验通过，随后在真实 `provider_probe` 阻断。估值通过统一单端点 POST 完成 50/50，行情仍向另一路径 POST 并返回 HTTP 404，结果为 0/50、`REHEARSAL_RETAINED_RESPONSE_MISSING`。根因不是 egress client：`TushareGateway.get_quote_snapshots` 在创建已绑定 provider_id 的 client 前无条件优先调用旧 compatibility adapter，导致此前新增的 provider 身份参数根本没有进入真实行情路径。修复把 compatibility adapter 限定为无 provider 身份的遗留入口；正式 provider 行情必须使用配置行的 token、URL、request mode、region 与 dataset key，并增加“即使旧 adapter 可构造也不得被调用”的契约测试。该改动产生新 SHA 后须重新执行完整 CI 和全新唯一 S6。
- `19ab4be434` 的独立 S6 再次在 provider probe 阻断。候选容器内只读诊断确认 provider id=2、`sdk_path`、自定义单端点 URL、`_RoutedSdkClient` 均正确且 compatibility adapter 为空；估值仍为 50/50，行情仍向 `/internal/tushare-gateway/daily` POST 并 404。最终分叉位于 `_RoutedSdkClient` 的无匹配 egress 分支：仅 `daily_basic` 走内部单端点 POST，其余 API 交给第三方 SDK，真实 SDK 改写了自定义 endpoint。整改把所有显式自定义 endpoint 的 configured provider 统一留在单端点 JSON-body 合同；只有没有自定义 endpoint 的官方 SDK 路径可继续交给 SDK。新增 negative contract 证明无 egress rule 时 `daily` 仍只 POST 精确 base URL，且 SDK `query` 不得被调用。
