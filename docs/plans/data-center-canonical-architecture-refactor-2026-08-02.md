...
## DATA-14 境内出口实施（2026-09-09）

用户授权的区域出口开发归入 DATA-14。配置中心拥有出口与加密凭据，数据中台拥有
路由、请求预算、故障状态和审计；Alpha/Qlib 消费者继续使用中台端口。
已补股票 raw/hfq 与 Tushare/RDS 请求适配、现有 TUI 管理入口及可选 frpc 部署产物。
具体边界、测试证据、启停步骤与待验收项统一见
[境内出口实施记录](data-center-regional-egress-plan-2026-09-09.md#2026-09-09-仓库实施记录)。
DATA-02 的生产数据、真实出口和价格一致性验收独立保留，本主计划不因本地实现而关闭。
