...
- `scripts/deploy_vps_verify.py` 新增 canonical schema gate，要求 `data_center_canonical_publication/publication_member/sync_run/sync_batch/sync_checkpoint/raw_payload/schema_fingerprint/reconciliation_evidence/publication_rollback` 全部存在；本地 verifier 21 tests passed，VPS 只读探针以 exit=1 列出全部 9 个缺失表。提交前不允许把旧镜像判为可切换。
