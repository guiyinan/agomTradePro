...
| 组件文件 | 用途 | HTMX 数据源 |
|---------|------|-------------|
| `core/templates/components/regime_status_bar.html` | 顶部 Regime 状态条 | `GET /api/regime/navigator/` |
| `core/templates/components/pulse_card.html` | Pulse 四维仪表盘 | `GET /api/pulse/current/` |
| `core/templates/components/attention_items.html` | 今日关注卡片 | `GET /api/dashboard/attention-items/` |
| `core/templates/components/action_recommendation.html` | 行动建议卡片 | `GET /api/regime/action/` |
