...
| 下一动作 | priority_next | 先补 TAR-02 的真实 PostgreSQL 双连接 first-winner/rollback 证据，再实现 admission/queue policy、`on_commit` dispatch、专用 Worker 与事件恢复；在 TAR-01/TAR-05 生产门和真实容量证据通过前，继续保持 inline 单槽、queued intake/worker 关闭 |
