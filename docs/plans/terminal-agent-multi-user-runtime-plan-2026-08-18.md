...
| Local CLI | active (foundation delivered) | 已补 `agomtradepro-agent` 本地入口、环境/OS keyring 凭据读取、redacted `doctor`、远端 streamable MCP Token 组合与 provider-key 不出远端的契约测试；完整 capability discovery/schema/confirmation resume、断线重连/Token 轮换和 Windows/WSL/Linux 打包仍在 TAR-04 |
