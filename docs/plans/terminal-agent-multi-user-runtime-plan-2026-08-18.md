...
| TAR-01 hard-SLO 与 deterministic matrix gate | completed (fail-closed pure Application contract) | 新增 `terminal_runtime_slo.py` 与 `terminal_runtime_test_matrix.py`：按第 11.1 节把 19 项阈值从观测值计算，不接受 caller 自报 `passed`；缺项、unavailable 或越限均阻断；11 层场景与 10 类 threat ID 生成 canonical SHA-256，collector 要求 candidate `test_matrix_digest` exact-match 后才采集；baseline/collector/SLO/matrix/manifest focused `39 passed`，增量 mypy `0 regressions`，active registry `0 violations` | 修复“任意完整数值可误判 capacity-ready”的验收门漏洞，但仍只有纯合同，没有 concrete observer、真实 HTTP/进程/Redis/DB/MCP/model 数据、1/5/10/20 负载、chaos 或生产容量证据；inline 单槽及 queued intake/worker 关闭保持不变，TAR-01 仍 active |
...
| 下一动作 | priority_next | `TAR-01` 的 ADR、machine contract、Application boundary、reserved route guard、feature flag、硬 SLO 和 deterministic matrix 已冻结；下一 repository slice 是实现严格只读 concrete observer/候选绑定证据序列化，再在获准环境取得受控 1/5/10/20 用户基线、容量与回滚指标。全部 TAR-01 exit gate 通过前保持 inline 单槽、queued intake/worker 关闭，之后才进入 `TAR-02` durable admission |
