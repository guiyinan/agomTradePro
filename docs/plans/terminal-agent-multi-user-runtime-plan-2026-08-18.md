...
| 下一动作 | priority_next | `TAR-01` 的 ADR、machine contract、failing-first API boundary 与 feature flag 已冻结；剩余只是在同一不可变候选上取得受控 1/5/10/20 用户基线、容量与回滚指标。全部 TAR-01 exit gate 通过前保持 inline 单槽、queued intake/worker 关闭，之后才进入 `TAR-02` durable admission |
