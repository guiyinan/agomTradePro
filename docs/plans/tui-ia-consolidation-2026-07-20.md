...
- dev 分支首次远端验证暴露的四类门禁问题已完成代码收口：TUI 增量 mypy 类型回归已消除；Gitleaks 仅对生成图中两个明确的相关性窗口字段键放行；AI Provider 三个 governed MCP read 已补 catalog replacement 契约证据；Alpha API 单测已隔离外部股票名称查询，避免 Nightly 因公网调用超时。
