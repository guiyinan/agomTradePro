...
- release `20260829220843` 的首轮 candidate-bound production-safe recorder 严格得到 `9/10`：唯一失败是 operator queue grid 在 Playwright 默认 `5s` 可见性断言刚过期后完成并渲染 `7` 行；失败截图同时显示命名 grid、真实数据与“读取完成”，因此不是角色权限或业务响应缺失。canonical UAT 未写入。
- `TUX-05` 再次作为唯一 repository focus，仅把该只读角色断言接入现有 `60s` production read settlement 上限；仍要求 operator grid/完成态、regular 不可用提示与 grid=0，且不改写后 `60s` SLO、不加重试或并发、不降低 `108` 路由、三角色、回执与 cleanup 门禁。
- Sol/Luna 复核后将两个串行等待收紧为单一 `60s` deadline：先等待“读取完成”，再同步断言命名 grid；regular 亦先等待不可用提示，再同步断言 grid=0。最终隔离 Chromium production-safe fixed suite 为 `10/10`、`0 skipped`、`138.83s`；strategy/provider 两条同 run receipt 均 readback 成功且 cleanup residual=`0`。规范化证据见 [`tux05-production-read-settlement-corrective-evidence-2026-08-29.json`](../testing/tux05-production-read-settlement-corrective-evidence-2026-08-29.json)。下一门仍是 CI/Sol review/merge、新 main clean-worktree 部署与完整生产 recorder；外部 AI、queued runtime、authority/approval、load/fault、maintenance/live rollback 继续禁止。
