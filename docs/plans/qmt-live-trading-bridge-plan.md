# QMT 整体桥开发计划（交易执行与行情采集）

> 2026-09-09 范围扩展：在现有 `qmt_agent/` 上整合行情采集，形成统一 QMT 桥；开发范围、分阶段验收与回滚见第 25 节，首期仓库实现与边界见第 26 节。原交易桥实现记录不能作为真实行情接通的证据。
...
> 状态：原交易桥与整体桥行情首期仓库实现已交付；真实券商行情 QB0/QB5 与交易 WP0/WP7 仍分别验收
> 创建日期：2026-07-21
> 最后更新：2026-09-09
> 目标版本：待确定
> 主线类型：QMT 整体桥；行情与交易按独立工作包实施
> 服务端 owner：交易执行归 `apps/broker_execution`；行情采集归 `apps/data_center`
...
## 25. QMT 整体桥行情扩展（2026-09-09）

### 25.1 目标与整合决策

沿用现有 `qmt_agent/`，交付一个安装包、一套本地运行入口和统一运维视图，同时承载行情采集与交易执行。复用配置加载、客户端连接管理、HTTPS 传输、机器认证机制、心跳、日志、升级和恢复基础设施；行情和交易使用独立能力授权、任务队列、状态存储命名空间、健康指标及启停开关。复用连接管理不代表强制共享 SDK 会话或线程，具体会话隔离以目标券商兼容性验证为准。

```text
Windows：统一 QMT Agent + QMT/xtquant
  ├─ 行情工作单元：订阅快照、日线补采 → 主动 HTTPS 上传 → VPS data_center
  └─ 交易工作单元：主动 HTTPS 拉取指令、回传券商事实 ↔ VPS broker_execution
VPS：data_center 校验、存储、发布 → 策略与风控 → broker_execution 编排订单
```

本地仍只负责 QMT 接入、数据传输和执行，不计算策略或发布决策。VPS 不安装 `xtquant`，Windows 不引入 Django/Redis/Celery。服务端行情归 `data_center`，订单生命周期归 `broker_execution`，不得把交易方法放入行情 `QMTGateway`，也不得让数据中台依赖交易模块 Infrastructure。共用机制通过 Protocol 和 composition root 装配；业务授权、规则和默认配置不得搬入 `shared/`。

现有 `QMTGateway` 和统一 Provider 适配器是本地行情读取基础，不是 Windows 到 VPS 行情上传闭环的完成证据。行情 DTO/字段映射按轻量 Agent 依赖边界复用或提取，不能直接把服务端依赖链搬入安装包。

### 25.2 首期范围与独立控制

- 首期支持已授权 A 股/ETF 标的的行情快照及日线历史补采；分钟线、逐笔持久化、Level-2、财务和其他市场另行验收后扩展，不宣称 QMT 可替代全部数据源。
- 提供行情单独运行模式，不要求开通下单权限或创建交易账户绑定；需要券商提供可用的行情权限。现有配置对交易账户的必填约束须按运行能力拆分，旧交易配置保持兼容。
- 沿用机器认证机制，新增独立行情 scope、允许的数据源/标的范围和上传额度；只读行情凭证不得拉单、报单、撤单或访问账户事实，旧交易凭证不自动获得行情写入权限。
- 明确“暂停行情”“紧急停止交易”“停止整个 Agent”三种语义。交易紧急停止继续遵守现有停止新单、保留回报/撤单/对账的规则；行情采集可继续运行。行情不可用时由服务端数据与风控门禁判断是否阻断新单，不阻塞已有订单回报和对账。
- 行情与交易工作单元采用独立有界队列、超时和资源预算；历史补采限速、可暂停，避免挤占下单、撤单与回报。验证 SDK 阻塞时的影响，必要时使用同一安装包下的独立工作进程。

### 25.3 行情契约与可靠性要求

- Agent 主动出站上传，不开放本地入站端口。定义版本化批次契约，包含 Agent/provider 身份、批次 ID、标的、周期、源观测时间、采集时间、服务端接收时间、交易日、复权口径和源单位；不得用上传/重试时间刷新源观测时间。
- VPS 在 Infrastructure 边界验证并标准化代码、数值、单位和时区后交给 Application；业务配置进入数据库并提供初始化/管理入口，不新增硬编码市场、单位映射或策略阈值。
- 本地缓冲支持容量上限、断线重连和确认后清理；VPS 持久化成功后才确认批次。批次与数据记录均需幂等，覆盖重复、乱序、部分失败及进程重启；旧批次不得覆盖较新快照。积压快照仅作历史事实或按显式策略丢弃并记录，不能伪装成当前行情；日线支持按缺口续传和更正审计。
- 发布 freshness、reliability、`must_not_use_for_decision` 和稳定阻断原因；检查交易日/交易时段、未来时间、缺失时间与时钟偏差。心跳正常不能替代行情新鲜度判断。
- 接入现有 Provider 注册、优先级、熔断与 failover。先旁路比对，再按能力提升 QMT 优先级；切换使用同标的、同观测时点/允许窗口、同复权及单位口径进行一致性检查，默认容差 1%，超限告警；stale 结果必须继续尝试后续源，全部不合格则阻断决策。
- 批次发布规范化 `outcome` 和 `requested/succeeded/failed/stored`，部分成功、零写入均显式说明。全市场 current Publication 仍按冻结 universe 原子发布，单标的与中间批次仅写 fact，不能缩小 current 集合或绕过现有发布门禁。

### 25.4 工作包与实施顺序

以下为完整目标工作包，首期仓库交付范围见第 26 节；行情门禁与原交易 WP0/WP7 分开记录，行情验收不解除任何实盘门禁。

| 工作包 | 交付物 | 完成门禁 |
| --- | --- | --- |
| QB0：目标机行情探针 | 国金客户端/xtquant 兼容性、行情权限、标的覆盖、源时间、订阅/快照/日线及重连报告 | 独立验证行情 API；历史 `QMT_SERVER_NOT_ALLOWED` 记录不能推断现在行情可用或不可用；探针不调用交易写接口 |
| QB1：整体桥契约 | 更新 ADR-0002、Agent 能力配置、行情 scope、版本化上传协议及服务端接入设计 | 冻结认证、幂等、单位、复权、时间语义和资源预算；旧配置兼容、行情凭证越权测试通过 |
| QB2：行情采集与上传 | 在同一 Agent 内增加行情工作单元、缓冲、限流、续传；VPS data_center 校验与入库 | Fake SDK 端到端验证上传、确认、重复/乱序、断网/重启、部分失败与补采；不影响交易回报 |
| QB3：统一数据消费 | Provider 路由、freshness、failover、Publication 与现有策略读取链路 | 验证 stale 不洗白、一致性超限告警、备用源切换、全源失效阻断及全 universe 发布门禁 |
| QB4：统一运维与用户任务 | 更新同一安装包/运行手册，TUI 展示连接与分能力健康、采集延迟、补采进度、独立启停和恢复 | 用户能完成连接检查、仅启用行情、确认服务端数据、暂停及恢复；新任务默认进入 TUI，Classic 仅兼容或导流 |
| QB5：旁路验证与分级启用 | 目标机连续至少 5 个交易日旁路报告，含开收盘、延迟分布、覆盖率、缺口、故障切换及资源竞争证据 | 按 QB1 冻结的频率、延迟、覆盖率和恢复预算逐项达标，才提升对应行情能力的源优先级；实盘仍按原 WP0/WP7 验收 |

QB0 可与不依赖真实 SDK 的契约/Fake 实现分别推进；QB0 未通过不得将行情标记为生产可用。首期先完成行情单独运行，再验证行情与交易同时运行时的资源隔离。

### 25.5 回归、治理和交付证据

- 自动测试覆盖行情映射、源时间/单位/复权、空数据、旧数据、越权、批次幂等、积压恢复、重连，以及行情补采压力下订单/撤单/回报的独立性；真实客户端行为另保留目标机证据。
- 实现新增当前数据面时同步 `governance/current_data_contracts.json` 并运行 `python scripts/check_current_data_contracts.py`；新增 Celery 批量写入/新鲜度任务时读取专项规范，同步 `governance/celery_task_contracts.json` 并运行对应检查。本次仅记录计划，不登记尚不存在的运行时契约和测试。
- 实现 API 时读取外包交付规范，补状态码/Content-Type、认证与引用方契约测试；实现 TUI 时读取用户设计规范和 Web→TUI 迁移计划，同步 metadata/runtime、迁移配置并运行迁移检查。
- 生产 Python 改动运行增量 mypy 和债务上限门禁、相关格式与架构检查；涉及 TUI/SDK 的实现按范围运行仓库要求的最小回归包。原交易幂等、紧急停止、租约和对账回归必须通过。
- 最终交付同时列出已完成、未完成、自动测试、目标机证据与未验证风险；不得将 Fake 测试或 Agent 在线等同于生产行情稳定。

### 25.6 风险与回滚

主要外部风险为券商行情权限/使用范围、客户端兼容性、Windows 休眠或断网，以及行情与交易共享 SDK 的资源竞争。接入 QMT 只增加一个可治理的数据来源，不预先保证稳定性提升，结论以 QB5 证据为准。

上线前记录原 Provider 路由和 Agent 制品版本。回滚时关闭行情能力、恢复原数据源优先级、暂停补采并隔离不合格批次，保留审计和交易未决状态；不得为回滚行情清空交易 SQLite 或重报订单。旧数据源也不可用时保持决策阻断。需要回退安装包时先验证配置/本地状态兼容性，按既有未决订单恢复规则执行。

## 26. 整体桥首期仓库交付（2026-09-09）

- 用户/服务器绑定：登录用户创建一次性配对码，本地 Agent 绑定精确服务器地址；市场凭证独立签名、nonce 防重放、90 日到期、修复轮换和撤销，Windows 当前用户 DPAPI 加密保存。不向本地提供用户登录密码，不自动开启交易。
- 数据接入：管理员批准 QMT Provider、资产范围与成交量单位倍率；本地订阅快照并补采最多 31 日的不复权历史日线，HTTPS 上传至 `data_center` 的四层入口，事务保存规范事实、原始批次及幂等回执。
- 故障隔离：同一安装包中的市场/交易进程、凭证、暂停开关和 SQLite 分离；市场进程独占自己的状态目录，先排空有界缓冲再采集，丢失回执或重启不改变批次及源时间。安装支持 `-MarketOnly`，不要求交易账户，不覆盖已有交易配置。
- 用户入口：TUI“个人资料与交易设置”提供本人绑定、状态、暂停/恢复、重新配对与撤销；管理员“QMT 接入与设置”提供公共行情接入审批。无需新增 Classic 页面。
- 数据治理：服务端 QMT Provider 从桥事实读取，过期快照允许后续源接管；保留原有 current Publication 与决策门禁，不自动提高 QMT 优先级。首期批次采取全量校验、全事务成功或失败，不产生隐式部分成功；源同一身份内容冲突阻断，日线更正另走后续审计修复。

运行步骤和回滚见 [整体桥运行指南](../operations/qmt-integrated-bridge.md)。数据库新增迁移 `0074_qmtbridgebindingmodel_qmtbridgebatchmodel_and_more`。改动按服务端绑定/采集、本地制品、TUI 与治理文档分组交付，不混入其他数据源配置改动。

自动证据由 `tests/integration/data_center/test_qmt_bridge.py`、`tests/unit/data_center/test_qmt_bridge_domain.py`、`tests/unit/broker_execution/test_market_bridge.py`、`tests/unit/terminal/test_tui_qmt_bridge.py` 及原交易 Agent/TUI 回归维护。新增时效契约登记为 `data_center.qmt_bridge_observations`，不增加 Celery 任务。

未完成的外部验收：目标国金客户端行情权限与字段口径、连续至少 5 个交易日供数/延迟/覆盖率、真实交易与行情并行压力、生产迁移部署和用户浏览器操作验收。首期不自动激活全市场 Publication，不交付 Level-2、分钟线持久化或自动日线更正。生产启用前必须分别完成 QB0/QB5，交易仍遵守 WP0/WP7。

### 26.1 本次验证记录

- 桥专项集成、Domain、TUI、Windows DPAPI 与原交易 Agent/TUI 联合测试：51 passed；补充日线 SDK 字段适配后，市场工作单元测试 12 passed（其中 11 项已包含在联合集），共 52 项不同测试。包含真实本地 HTTP 配对→审批→采集配置→上传→数据库回执闭环；数据源采用 Fake SDK，不调用真实券商。
- 仓库要求的四组最小回归：`test_tui_workbench.py`、`test_terminal_agent_service.py`、`sdk/tests/test_sdk/test_client.py`、`test_internal_ssl_redirect.py`，373 passed。
- 增量 mypy：17 个变更 Python 文件零错误、零回归；全仓库 mypy 债务上限检查零错误。Black、isort、Ruff、PowerShell 脚本语法、Django system check、`makemigrations --check --dry-run` 通过。
- 当前数据契约：56 个 surface 通过；Web→TUI inventory 通过；架构全量扫描 3033 文件，零边界违规。测试采用隔离数据库的 `--no-migrations`，不代表已在生产执行迁移。
- Windows 制品：`artifacts/qmt-agent-bridge/agom-qmt-agent-windows-0.2.0-bridge.zip`，34 个归档条目，含整体桥指南；SHA-256：`d192f91f382dd5c874fd706e630cdd05c2b5311a52d24f73e2b2d2382af52fd0`。制品不包含本地配对凭证、SQLite 状态或券商客户端。
