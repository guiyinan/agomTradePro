...
| R6 | 简单基准不足、高级 artifact gate、七指标 qualification/政策反应诊断、assessment/lifecycle ledger，以及 monitoring Domain/Application/observation/assessment persistence | canonical owner adapters、DB-level immutable/signature depth；真正 activation/`stack[-2]` rollback | 真实 shortfall、PIT/OOS、稳定标签、政策目标、monitoring facts、owner authorization 与 Promotion |
...
| R8 | canonical snapshot、execution feedback、13 类 typed 输入、current baseline、可投资 universe、四市场约束、path drawdown、四候选比较、独立 canonical input receipt、result/lifecycle 与 fail-closed composition | 安全 registration façade与 DB 条件约束纵深；真实 Research/Portfolio owner provider 接线 | broker reconciliation、R3/R4/R5 晋级、真实 Portfolio snapshot、成本/容量/市场约束校准 |
