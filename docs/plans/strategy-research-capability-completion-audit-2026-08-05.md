...
| R2 | actor/series 双时间、proxy/measure semantics、PIT membership、描述性证据、版本化 expected-period calendar、完整 coverage、Publication exact gate 与 Promotion lifecycle | 两周期 Audit/Research 解释力 trial 与监控合同；不得把描述性 evidence 当预测信号 | 获批 taxonomy/calendar、owner authorization、两个市场周期、Production Publication 与真实 Audit outcome |
| R3 | 独立 App、exact PIT fact/manifest、historical-mean/FMP、concrete sklearn Lasso/OLS、nested temporal-CV、canonical artifact bytes、dated ledger、governed read 与 retirement lifecycle | canonical Data Center/Research/Regime owner adapters、真实 trial/monitoring composition；不得用 synthetic PIT attested ready | 宏观 vintage、代理资产/连续期货、真实 cost/benchmark、Regime/OOS trial、owner authorization 与 Promotion |
...
| R6 | 简单基准不足、高级 artifact gate、七指标 qualification/政策反应诊断、assessment/lifecycle ledger、raw-fact monitoring Phase A | monitoring observation/assessment persistence、strict codec/UoW/canonical adapters；Policy owner contract；真正 activation/`stack[-2]` rollback | 真实 shortfall、PIT/OOS、稳定标签、政策目标、owner authorization 与 Promotion |
...
| R8 | canonical snapshot、execution feedback、13 类 typed 输入、current baseline、可投资 universe、四市场约束、path drawdown、四候选比较、append-only result/Promotion/retirement/rollback lifecycle，以及缺 owner source 即 unavailable 的 production composition | 独立 append-only input receipt、canonical input-set exact provider 及真实 Research/Portfolio owner provider 接线；不得用 result payload 或 caller object 代替 | broker reconciliation、R3/R4/R5 晋级、真实 Portfolio snapshot、成本/容量/市场约束校准 |
