...
| R7 | 概率分栏、Brier/分箱、PIT 类比/路径、reminder/sample-policy/result/lifecycle ledger、物化审计 snapshot、无状态 production façade、post-promotion monitoring Phase A+B及独立family `stack[-2]` rollback Phase A+B ledger；Signal `0011` 已建 ForecastOutcome exact realization receipt，`0012` 已建 source-definition header/member registry并由 `0011` exact 读取 | production Risk Center、真实 source-metadata provider 与 family owner adapter；不在 fixture 上接 probability/current/consumer | 完整预测—复核—兑现历史、获批 sample policy、PIT 路径样本、完整 owner metadata、family/owner authorization 与合格 result |
| R8 | canonical snapshot、execution feedback、13 类 typed 输入、四候选比较、独立 canonical input receipt、exact lifecycle、post-promotion monitoring Phase A+B、Portfolio `0013` calendar registry、active-result/input-receipt adapters，以及Broker `0007` reconciliation period receipt/exact adapter | Research policy/Promotion与Portfolio feedback/raw-fact owner接线；Broker真实source registration与上线前PostgreSQL并发证据 | 真实 broker reconciliation、R3/R4/R5 晋级、Portfolio snapshot、成本/容量/市场约束校准 |
