...
- 五组测试统一改为在 fixture 内注入 `django_db_blocker.unblock()`，不依赖 pytest-django 自动建表，保持自建 schema 的 zero-seed 语义；使用 `--ds=tests.settings_account_actor_authority_source_v3 --confcutdir=tests/component/account` 重跑，合计 `38 passed`，其中 mutation-binding repository `4 passed`。
