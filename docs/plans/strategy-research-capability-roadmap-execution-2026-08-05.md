...
| R3 高频宏观因子与 nowcast | `blocked` | 已有 exact PIT runner、concrete sklearn Lasso/OLS、逐 fold baseline/FMP/nested-CV、Research-owned authoritative spec ledger/exact provider、独立 label-free inference/target calendar、完整 spec/request/manifest canonical seal、dated outputs、governed read 和 retirement lifecycle；仍需真实 definition provider composition、inference Publication/calendar、宏观 vintage/代理资产 PIT、benchmark/cost、Regime/OOS trial 与 exact Promotion | [R3/R4](macro-factor-r3-r4-readiness-and-staged-delivery-2026-08-05.md) |
...
| R7 情景概率校准 | `blocked` | 已有 source-separated Brier/分箱、PIT 类比、逐期路径证据、reminder/sample-policy/result/lifecycle append-only ledger、审计 snapshot、无状态 production façade与 post-promotion monitoring Phase A；仍需 monitoring Application/persistence、family rollback、完整 outcome 历史、真实 Risk Center/Forecast Ledger owner source、approved policy及合格 result/authorization | [R5—R8](strategy-research-r5-r8-readiness-and-staged-delivery-2026-08-05.md) |
