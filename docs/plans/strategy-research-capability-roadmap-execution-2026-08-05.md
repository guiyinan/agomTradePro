...
43. 完成 R5 monitoring Phase B：Research `0014` 三张 schema-only ledger、完整 Phase A owner graph replay、跨 clock winner 幂等、exact PIT、immutable signed audit、race/fork/rollback 与完整 ORM/Collector guard；production owner缺失时零写 blocked。
44. 完成 R7 production composition 能力面收口：三类 public builder只接受数据库别名，无状态 mutation façade不保存 provider/clock/writer/store/token；可注入写路径仅为非导出 test factory。
45. 完成 R3 authoritative runner-spec persistence：Research `0015` 单表 schema-only ledger、完整 strict codec、shared-UoW owner 双读、trusted server clock、动态 UoW 复核、exact PIT provider与first-winner replay；production definition provider缺失时零写 blocked。
46. 完成 R7 post-promotion monitoring Phase A：完整 lifecycle head/count replay、active result pre-period subjective/model projection、独立 Forecast Ledger realization owner、连续 exact calendar、source-separated coverage/Brier 和 PIT knowledge cutoff全部进入纯 Domain 合同；缺 calibration/analogy/path 证据保持 blocked，不发布概率或 current。
47. 完成 R2 explanatory trial/monitoring Phase B：Research `0016` 四本 schema-only ledger、strict codec、shared-UoW owner双读与append前完整再取证、server clock、exact PIT、winner/fork、signed audit及zero-seed正反迁移；production owner缺失时mutation/audit保持inert。
48. 完成 R7 post-promotion monitoring Phase B：Research `0017` 三本 schema-only ledger、完整 owner graph重放、assessment-scoped observations、exact PIT、first-winner/fork与signed immutable audit；production register/audit保持无状态 unavailable。
49. 完成 R7 family rollback Phase A：独立 family lifecycle 以本地 result lifecycle head/count attestation和三阶段owner/authorization重读支持PROMOTE/RETIRE/精确`stack[-2]` rollback，禁止重复入栈，不接现有terminal RETIRE persistence或consumer。
50. 收紧 R3/R6 production authority surface，并完成 R3 exact ledger完整性：public mutation façade无状态且不接caller provider/clock；`macro_factor.0003` 用cumulative commit和独立latest head检测output/artifact/lifecycle单侧或配对尾截断。
51. 完成 R3 governed-read 事务边界：五类 owner reader、trusted clock 与 atomic repository 使用动态 exact shared-UoW，完整 graph 双读且异常统一 fail closed。
52. 完成 R7 family lifecycle Phase B：Research `0018` 四本 schema-only ledger封存authorization/event/stream commit/audit snapshot，exact winner按首次ledger cutoff重放，canonical result与local authorization↔event graph现场恢复；production mutation/audit保持inert。

53. 关闭 readiness 静态假验收边界：只有显式 mechanism-attestable allowlist 中的 requirement 可由 governance manifest 验证，outcome、Portfolio snapshot、execution feedback 等真实证据类 requirement 在加载时直接拒绝。
54. 关闭 R1 baseline trial 评估的单读/回填边界：command 仅携 identity 与 PIT cutoff，trusted clock 产生 evaluated-at，spec/artifact/actual/Research 完整图在单一 shared UoW/atomic 中双读并在 append 前后复核，所有入口对象现场 exact-type/live 验证。
55. 完成 R4 服务端三候选构造：同一 sealed exposure/factor-covariance/asset-covariance/constraint/cost 图确定性生成 equal-weight、asset-RP 与 macro-factor-RP，宏观风险贡献误差必须落在版本化 tolerance 内，PSD/rank/bounds/liquidity/turnover/cost 异常统一 fail closed。

56. 完成 R1 Data Center actual owner 软件边界：`data_center.0068` 封存版本化 source definition 和 materialized manifest receipt，ID-only writer 只能在 shared UoW 中双读 canonical member/vintage/fact source 后构造完整 snapshot；production mutation inert，exact PIT provider 空库返回 `None`。
57. 完成 R7 Signal Forecast Ledger realization owner 软件边界：`signal.0011` 封存 manifest/member receipt，每个 member 动态 exact 重读 immutable ForecastOutcome/Entry；Research 只消费严格 PIT receipt，public append 无状态 unavailable，缺真实 receipt 时 monitoring 保持 `BLOCKED`。
58. 完成 R4 canonical monitoring owner 软件边界：Research `0019` 封存 policy/calendar owner definition，Portfolio `0012` 封存 raw-fact receipt；ID-only registration 使用 shared-UoW、trusted clock、双读和 exact PIT adapters，production mutation保持无状态 unavailable，缺真实 owner 时零写。
59. 完成 R1 Research trial prereg 软件边界：`research.0020` 只封存由 canonical definition、approved baseline spec/artifact 双读派生的 prereg evidence；public registration inert，Equity exact provider空库返回 `None`，不复用通用 ExperimentTrial 或事后 Promotion receipt冒充 authorization。
60. 完成 R7 Signal realization source-definition 软件边界：`signal.0012` 封存 versioned definition/member graph，既有 `0011` writer固定从该 registry exact读取；public registration inert，缺真实 source metadata 时零写且 monitoring保持 `BLOCKED`。
61. 完成 R2 trial-policy owner 软件边界：Research `0021` 单本 schema-only、zero-seed registry在selection前封存完整policy；ID-only/shared-UoW/trusted-clock双读注册和exact PIT provider已接既有evaluator，public mutation保持inert。
62. 完成 R5 research-control preflight：只读合并exact active lifecycle、fixed-income owner seal与最新完整monitoring assessment；调用方不能选择assessment/health，只有健康证据可进入人工consumer review，其余稳定`BLOCKED`，不发布current/decision/execution。
63. 完成 R8 monitoring calendar owner软件边界：Portfolio `0013` 单本schema-only、zero-seed calendar registry与active-result/input-receipt窄adapter已落地；public mutation inert，缺Research policy/Promotion及Portfolio/Broker反馈时monitoring继续`BLOCKED`。
...
70. 完成 R3 Data Center canonical PIT source软件边界：`data_center.0069`封存source/calendar/period/member/value-decoding与coverage规则，Macro Factor只从exact manifest/fact rows严格投影；缺字段、clock、coverage或同member多revision均返回`None`，不猜latest、不接runner写入。
71. 完成 R4 research-control preflight：以scope/as-of服务端选择active Promotion和latest-complete monitoring唯一winner，在shared-UoW/trusted-clock下双读完整六owner graph；仅发布人工consumer-review资格或`BLOCKED`，不接consumer/current/decision/execution。
...
77. 完成 R7 result canonical join：private owner runtime直接组装Signal calibration与Research `0025` analogy/path exact providers，任一owner缺失均在共享事务内零result写；public runtime仍using-only/inert。
...
|---|---|---|---|
| R1 行业经营驱动与盈利预测 | `blocked` | 已有版本化行业模板、Sector→Equity 持久 bridge、三情景六阶段/cash-flow seal、完整配对 trial、Research exact Promotion/lifecycle、Data Center `0068` actual ledger、Research `0020` trial prereg及production read-only evaluation preflight；仍需 QW-7 反馈、连续 PIT 事实、Production Publication、真实 fact/member/vintage与trial definition/spec/artifact owner、approved decision，且未接 Valuation | [R1/R2](strategy-research-r1-r2-readiness-plan-2026-08-05.md) |
| R2 市场结构与投资者资金流 | `blocked` | 已有 taxonomy/measure/proxy/PIT membership、版本化 calendar/coverage、Publication/Promotion、两周期 explanatory trial/monitoring合同、`0016` persistence与Research `0021` trial-policy registry/exact provider；仍需真实 Publication、两个周期真实 PIT 覆盖、owner authorization、Audit outcome 与其余 concrete owner providers | [R1/R2](strategy-research-r1-r2-readiness-plan-2026-08-05.md) |
| R3 高频宏观因子与 nowcast | `blocked` | 已有 exact PIT runner、concrete sklearn Lasso/OLS、逐 fold baseline/FMP/nested-CV、Research-owned authoritative spec ledger/exact provider、独立 label-free inference/target calendar、完整 spec/request/manifest canonical seal、dated outputs、shared-UoW/双读 governed read、retirement lifecycle及commit+latest-head完整性锚；production run mutation为无状态unavailable façade；仍需真实 definition provider composition、inference Publication/calendar、宏观 vintage/代理资产 PIT、benchmark/cost、Regime/OOS trial 与 exact Promotion | [R3/R4](macro-factor-r3-r4-readiness-and-staged-delivery-2026-08-05.md) |
| R4 宏观敞口与风险平价 | `blocked` | 已有 rolling/Regime/三方法同窗、Portfolio ledger/query、covariance diagnostics、Research 五表 ledger、scope/policy/trial/decision/lifecycle/active provider、monitoring Phase A+B、Research `0019` policy/calendar registry、Portfolio `0012` raw-fact receipt 与 canonical exact composition；仍需真实 R3 晋级、canonical inputs/owner facts、owner authorization、OOS trial 和下游 active consumption 验收 | [R3/R4](macro-factor-r3-r4-readiness-and-staged-delivery-2026-08-05.md) |
| R5 固收相对价值与久期 | `blocked` | 已有定价/久期/曲线/信用、fixed_income/Portfolio/Research owner ledgers、Promotion/retire/rollback、monitoring Phase A+B、只读preflight、Research `0022` policy/calendar及Portfolio `0014` raw-fact registry/private composition；仍需真实owner source provider、Publication/PIT/OOS、authorization、外部对账和下游验收 | [R5—R8](strategy-research-r5-r8-readiness-and-staged-delivery-2026-08-05.md) |
| R6 高级状态模型 | `blocked` | 已有简单基准不足、高级证据、qualification/lifecycle ledger、monitoring Phase A+B persistence 及 activation/`stack[-2]` rollback Phase A+B ledger；production monitoring mutation surface保持无状态unavailable；仍需真实不足/PIT/OOS/稳定标签/政策目标、canonical owner adapters、owner authorization、真实 Promotion 与生产 consumer 验收 | [R6](r6-simple-baseline-shortfall-and-state-model-staged-delivery-2026-08-05.md) |
| R7 情景概率校准 | `blocked` | 已有 source-separated Brier/分箱、PIT 类比、逐期路径证据、reminder/sample-policy/result/lifecycle append-only ledger、审计 snapshot、无状态 production façade、post-promotion monitoring Phase A+B、独立family `stack[-2]` rollback Phase A+B ledger，以及 Signal `0011` realization receipt、`0012` source-definition registry和Research exact adapter；仍需完整 outcome 历史、真实 source metadata provider、Risk Center owner source、approved policy及合格 family/result authorization | [R5—R8](strategy-research-r5-r8-readiness-and-staged-delivery-2026-08-05.md) |
| R8 多资产优化 | `blocked` | 已有 canonical snapshot/反馈 schema、13 类 typed 输入、四候选比较、独立 input receipt、exact lifecycle、monitoring Phase A+B、Portfolio `0013` calendar registry及Broker `0007` reconciliation receipt；仍缺Research policy/Promotion、Portfolio feedback/raw facts、真实 Broker source registration/snapshot/reconciliation、约束校准及 R3/R4/R5 晋级版本 | [R5—R8](strategy-research-r5-r8-readiness-and-staged-delivery-2026-08-05.md) |
...
2026-08-10 R5 monitoring Phase B 与 R7 production façade 验证：主代理独立复跑 R5 Phase A+persistence unit+component `26 passed`、Research `0014` migration `3 passed`，并完成 fresh forward/reverse/reforward 与三表 zero-seed；R7 capability unit/production zero-write/private-success focused `13 passed`。11 个生产文件增量 mypy 0 regression，Ruff/Black/isort、Django check、Research migration drift、governance consistency、架构扫描（2411 files / 0 violations）与模块循环均通过。Luna Max 最终普通复核高/中/低问题均为 0。软件证据不替代真实 Publication、monitoring facts、Risk Center owner、Promotion 或外部对账，R5/R7 继续 `blocked`。

2026-08-10 R3 runner-spec persistence 验证：主代理复跑 unit `4 passed`、component `13 passed`、migration `3 passed`；6 个生产文件增量 mypy 0 regression，Ruff/Black/isort、Django check、Research migration drift、governance consistency、架构扫描（2417 files / 0 violations）与模块循环均通过。Luna Max 复核关闭 shared-UoW/server-clock、owner二读、构造后UoW漂移和exact winner问题后无P0/P1。真实 definition provider、manifest/dataset、PIT数据、trial与Promotion仍缺，R3继续`blocked`。
...
2026-08-11 软件续批验证：readiness `47 passed`、R1 trusted-clock evaluation `54 passed`、R4 server-owned optimizer `21 passed`、R1 trial prereg unit+component `25 passed`、R7 source-definition/realization unit+component `40 passed`；R4 canonical owner 三个 component 边界逐项通过。Research `0020` 与 Signal `0012` 独立 SQLite forward/reverse/re-forward成功且zero-seed；本批四 App migration state无 drift。增量 mypy无回退，72个scoped Python文件Ruff/Black/isort通过，架构扫描`2477 files / 0 violations`，governance consistency `0 violations`。所有R1—R8状态仍为`blocked`，真实数据/PIT/OOS/Promotion/对账与consumer验收未被测试替代。

2026-08-11 第二批软件续批验证：R2 trial-policy registry unit+component `17 passed`、R5 research-control preflight unit+component `9 passed`、R8 monitoring calendar registry/owner composition unit+component `12 passed`。Research `0021` 与 Portfolio `0013` 在独立 SQLite 上均完成 forward/reverse/re-forward，新增账本每次均为zero-seed；两App migration state无 drift，三组生产文件增量mypy均为0 regression，scoped Ruff/Black/isort通过。该证据只关闭软件边界，不替代真实Publication、两个市场周期、monitoring facts、Promotion、broker reconciliation或consumer验收，R2/R5/R8继续`blocked`。
...
2026-08-12 第八批软件收口验证：R7 canonical result composition unit `8 passed`、empty-owner component `1 passed`；R2 preflight focused `15 passed`；R3 Regime exact-read composition focused `24 passed`；R6 qualification authority unit+repository+component合并 `7 passed`。相关生产文件增量mypy 0 regression、Ruff通过；无迁移/seed/current/decision/execution扩展。真实Publication/PIT/OOS/Promotion/owner/consumer验收仍未形成，各研究链继续`blocked`。
