...
| R7 情景概率校准 | `blocked` | 已有 source-separated Brier/分箱、PIT 类比、逐期路径证据、reminder/sample-policy/result/lifecycle append-only ledger、审计 snapshot 与无状态 production façade；仍需 post-promotion monitoring、family rollback、完整 outcome 历史、真实 Risk Center owner source/approved policy、真实合格 result/authorization | [R5—R8](strategy-research-r5-r8-readiness-and-staged-delivery-2026-08-05.md) |
