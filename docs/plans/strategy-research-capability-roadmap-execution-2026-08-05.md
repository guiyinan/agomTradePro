...
| R8 多资产优化 | `blocked` | 已有 canonical snapshot/反馈 schema、13 类 typed 输入、四候选比较、独立 input receipt、exact DB version/receipt constraints、ID-only shared-UoW/server-clock lifecycle、post-promotion monitoring Phase A、fail-closed composition 与无状态 production façades；仍缺 monitoring persistence/concrete owner、真实 Portfolio snapshot、broker reconciliation、约束校准及 R3/R4/R5 晋级版本 | [R5—R8](strategy-research-r5-r8-readiness-and-staged-delivery-2026-08-05.md) |
