...
> **机器证据再验证已收紧（2026-07-28）**：readiness checker 不再只相信 cutover evidence 中的投影字段。候选 commit 必须属于当前分支且其提交内矩阵 SHA 与当前范围一致；逐 route rollback 映射必须逐值等于矩阵并指向当前分支祖先；缺陷和生产遥测必须是生成器可重新解析的结构化 JSON，checker 会从 issue/task 原始记录重建投影并要求精确相等。生产遥测分母统一从 `classic_routes.task_key` 推导为 101 个可比较任务，不再误用包含 TUI-only action 的全量 `tui_task_keys`。
...
10. **完成切换保障**：使用 `backup_tui_registry` 在仓库外取得生产 registry JSON + SHA-256 sidecar，并以 `restore_tui_registry_backup` dry-run 验证可恢复性；记录 generation、graph/schema/runtime 版本、SHA-256、外部位置、保留期和验证人；owner 与独立 reviewer 对同一候选版本和证据快照签字。工具与集成测试已落地，但生产执行仍未完成。
