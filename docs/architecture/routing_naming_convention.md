...
本文档定义 AgomSAAF 项目的统一路由命名规范，确保前后端协作的一致性，降低维护成本。
...
# 生成: /account/api/portfolios/ (通过 core/urls.py 的 path('account/', include(...)) 挂载)
```
...
| `/dashboard/api/allocation/` | API | 需调整 | 应改为 `/api/dashboard/allocation/` |
| `/dashboard/api/v1/*` | API | 规范 | 版本化 API |
...
| `regime` | `/regime/api/` | `/regime/dashboard/` | 需调整 |
| `strategy` | `/strategy/api/` | `/strategy/` | 规范 |
| `simulated_trading` | `/simulated-trading/api/` | `/simulated-trading/*` | 规范 |
...
# 替换为新路由
# fetch('/account/api/portfolios/') → fetch('/api/account/portfolios/')
```
