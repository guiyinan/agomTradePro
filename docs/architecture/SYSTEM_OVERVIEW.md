...
- ✅ **一体化工作台**(2026-02-27 新增)
  - 双闸并行：Policy Gate (P0-P3) + Heat/Sentiment Gate (L0-L3)
  - 热点情绪事件不影响政策档位
  - 仅已生效事件计入当前档位
  - 审核流程（通过/拒绝/回滚/豁免）
  - SLA 监控（P2/P3: 2小时, 普通: 24小时）

**关键逻辑**:
- P2档位:暂停Regime信号24-48小时
- P3档位:全仓转现金或对冲,人工接管
- 热点情绪闸门(L0-L3)独立运行，不修改政策档位
...
**实然功能**:
- ✅ 策略配置已实现(StrategyConfigModel)
- ✅ 资产配置矩阵已实现(AllocationMatrix)
- ✅ 再平衡服务已实现(RebalanceService)
- ✅ 约束条件已实现(仓位限制、行业限制、地域限制)
- ✅ 配置服务已实现(allocation_service.py)

**核心功能**:
```python
# 资产配置计算
allocation = allocation_service.calculate_allocation(
...
└──────────────────────────┬──────────────────────────────────┘
                           ↓
┌─────────────────────────────────────────────────────────────┐
│ 8. 持仓管理 (account/simulated_trading)                     │
...
# core/settings/base.py - CELERY_BEAT_SCHEDULE
{
    ...
    # 实时价格监控定时任务 (新增)
    'realtime-update-after-close': {
        'task': 'apps.realtime.application.tasks.update_all_prices_after_close',
        'schedule': crontab(hour=16, minute=30, day_of_week='mon-fri'),  # 工作日16:30
    },
}
```
