...
## 八、Domain 层逻辑优化（2026-01-22）

### 8.1 Regime 通胀动量计算算法优化
...
**问题描述**:
通胀指标（CPI、PPI）使用相对动量计算时，低通胀时期数据严重扭曲：
- CPI 从 0.1% 涨到 0.3%，相对动量 = (0.3-0.1)/0.1 = **200%**
- 导致 Z-score 异常高，错误判定为"高通胀动量"
- 配合 PMI 的正动量，错误判定为"过热"象限

**经济现实**:
- CPI 0.3% 仍是低通胀（绝对水平低）
- 从 0.1% 涨到 0.3% 只是温和上升（+0.2pp）
- 不应该与 PMI 50.2 + CPI 3% 这种"真过热"相提并论

**解决方案**:
对通胀指标使用绝对差值动量（百分点差值）：

```python
# 新增函数（apps/regime/domain/services.py）
...
class RegimeCalculator:
    def __init__(
        self,
        ...
        use_absolute_inflation_momentum: bool = True  # 新增参数
    ):
        ...

    def calculate(self, growth_series, inflation_series, as_of_date):
        # 增长指标：相对动量
...
| 场景 | PMI | CPI | 修复前判定 | 修复后判定 |
|------|-----|-----|-----------|-----------|
| 2024 中国 | 50.2 | 0.3% | Overheat（Z通胀=+3.5） | Recovery（Z通胀=+0.3） |
| 真过热 | 52.0 | 3.0% | Overheat（Z通胀=+2.0） | Overheat（Z通胀=+1.8） |
...
**相关文档**:
- `docs/regime_calculation_logic.md` - 完整计算逻辑说明
- `docs/AgomSAAF_V3.4.md` - 更新主文档中的 Regime 章节
