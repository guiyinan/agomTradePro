...
Every screen intended to replace a classic Django page should define `default_action_key`. The renderer auto-runs that action when it has no unresolved required fields, and must expose screen-level `entry_state` metadata when the default action is blocked by operator input, so users never land on a generic empty task picker. Secondary actions stay in the left task panel.
...
- Published screens with a blocked `default_action_key` must provide `entry_state` (`auto_run | parameter_gate | dashboard`) plus operator-facing `entry_empty_copy` and `entry_help_steps`.
- Required `select` entry gates should open with a selector-first center pane; required free-text entry gates should open with a task-start card. Do not fall back to task-picker-only or a generic empty-state.
...
- Browser validation confirms no empty "task picker only" page, no generic empty-state at the default entry, no default `NO MATCHING ROWS`, and no console warning/error.
...
6. Publish to DB only with explicit approval:
   ...
   Re-running the same publish payload is now a no-op at the DB registry layer: if the latest `status=published` row for the same `registry_key` already has the same compacted payload hash, runtime history is reused and no new row is created.
