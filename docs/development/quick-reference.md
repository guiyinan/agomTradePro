...
| 版本 | 0.8.0 |
| 状态 | 生产就绪 |
| 阶段状态 | 见 `docs/VERSION.md` 与当前阶段计划 |
| 业务模块 | 见 `governance/governance_baseline.json` |
| 测试规模 | 见 `governance/governance_baseline.json` |
| Python版本 | 3.11+ |
| Django版本 | 5.x |

> 业务模块数、MCP 工具数、静态测试函数数及其他动态治理计数只写入
> `governance/governance_baseline.json`。本快速参考不维护数值副本。
...
- `/admin-console/`：管理控制台，统一承接管理员日常操作、集中风控、Agent 任务与提案、生产就绪和计划任务入口
- `/admin/`：Django Admin，仅保留给底层模型和未产品化功能
- 顶部导航分组：`研究`（宏观环境 / 策略研究 / 智能模块）、`执行`（决策工作台 / 账户与执行）、`平台`（设置 / 数据源 / AI 与自动化 / 运行校验）、`帮助`（文档 / API）、`运维`（管理员与值守）
- 经典 Dashboard `/dashboard/` 保留 Pulse 锚点，并为 staff 提供集中风控、实时监控、Agent Runtime、Qlib 配置入口；MCP 工具、生产就绪和计划任务按权限显示。

### 宏观数据页交互

- `/macro/data/` 图表默认先聚焦最近 `36` 个周期，底部缩放条可继续拖拽细选区间。
- 页面内置 `近12 / 近24 / 近36 / 近60 / 全部` 快捷区间按钮，适合长序列快速切换。
- `表格详情 / Markdown` 会打开右侧明细抽屉，可在“当前区间 / 全部记录”之间切换，并复制带单位、来源、发布时间与新鲜度字段的 Markdown 表格。
...
| 文档 | 说明 |
|------|------|
| `governance/governance_baseline.json` | 动态治理数据机器唯一真源 |
| `governance/SYSTEM_BASELINE.md` | 系统基线叙事索引 |
