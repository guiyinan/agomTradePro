...
- [外包测试与修复任务书（2026-02-22）](./outsourcing-task-book-2026-02-22.md)
...
| ID | 模块 | 文件 | 行号 | 问题描述 | 优先级 |
|----|------|------|------|----------|--------|
| #28 | Policy | repositories.py | 258-271 | `delete_event` 按日期删除会删除同日所有事件 | P1 |
| #29 | Policy | repositories.py | 118-124 | `get_event_by_date` 只返回第一个匹配事件 | P1 |
| #30 | Regime | interface/views.py | 25 | 数据源表为空时 `first()` 可能为 None | P1 |
