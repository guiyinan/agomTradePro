...
1. 管理员可以在数据源中台配置多条 `tushare`，例如官方 SDK、SDK 路径中转、统一中继；
   每条配置使用唯一名称和独立优先级
2. 填写 `api_key`
3. 填写 `http_url`
4. 按代理协议选择请求模式：
...
7. 每条配置只使用本行保存的 Token、URL 和请求模式；禁止从另一条 Tushare 配置读取模式
8. registry 首次按 `priority` 调度；一条线路成功后，在同一数据能力下优先复用该线路，
   失败则继续尝试其他激活线路
9. 已由既有健康记录器落库的最近成功时间，会在进程重启或 registry 刷新后恢复偏好；
   不同 `source_type` 之间的优先级槽位不会被同类线路粘滞重排
