...
| `/api/macro/datasources/` | GET/POST | 列出或创建统一数据源配置 |
| `/api/macro/datasources/{source_id}/` | GET/PATCH/PUT | 获取或更新指定数据源配置 |
| `/api/macro/datasources/{source_id}/test/` | POST | 触发连接探针并返回测试日志 |
...
# 测试指定数据源连接
POST /api/macro/datasources/1/test/
```
...
> **页面入口**: `/macro/datasources/#provider-status`（统一数据源中心中的运行状态与快速测试区域）
