...
1. [宏观与数据中台 (Macro & Data Center)](#1-宏观与数据中台-macro--data-center)
...
## 1. 宏观与数据中台 (Macro & Data Center)

### 1.1 宏观数据与治理 API

> **标准前缀**: `/api/data-center/`
> **说明**: 宏观事实、指标目录、量纲规则统一由 Data Center 提供；旧 macro API 已移除。

| 端点 | 方法 | 描述 |
|------|------|------|
| `/api/data-center/macro/series/` | GET | 查询宏观时序 |
| `/api/data-center/sync/macro/` | POST | 触发宏观同步 |
| `/api/data-center/indicators/` | GET/POST | 查询或创建指标目录 |
| `/api/data-center/indicators/{code}/` | GET/PATCH/DELETE | 管理单个指标 |
| `/api/data-center/indicators/{code}/unit-rules/` | GET/POST | 查询或创建量纲规则 |
| `/api/data-center/indicators/{code}/unit-rules/{rule_id}/` | GET/PATCH/DELETE | 管理单条量纲规则 |
...
```bash
# 查询宏观时序
GET /api/data-center/macro/series/?indicator_code=CN_PMI
...
## 1.5 数据中台模块 (Data Center)

> **标准前缀**: `/api/data-center/`
> **说明**: 统一 Provider 管理、标准事实表查询与同步入口。

| 端点 | 方法 | 描述 |
|------|------|------|
| `/api/data-center/providers/` | GET/POST | 列出或创建 Provider 配置 |
| `/api/data-center/providers/{id}/` | GET/PATCH/DELETE | 获取、更新或删除 Provider |
| `/api/data-center/providers/{id}/test/` | POST | 测试 Provider 连通性 |
| `/api/data-center/providers/status/` | GET | 获取 Provider 健康状态 |
| `/data-center/providers/` | GET | Provider 配置页 |
| `/data-center/monitor/` | GET | Provider 运行状态页 |
| `/api/data-center/assets/resolve/` | GET | 解析 canonical 资产代码 |
| `/api/data-center/macro/series/` | GET | 查询宏观时序 |
| `/api/data-center/prices/history/` | GET | 查询历史价格 |
| `/api/data-center/prices/quotes/` | GET | 查询最新行情 |
...
| `/api/data-center/sync/macro/` | POST | 同步宏观事实 |
| `/api/data-center/sync/prices/` | POST | 同步历史价格 |
| `/api/data-center/sync/quotes/` | POST | 同步最新行情 |
| `/api/data-center/sync/funds/nav/` | POST | 同步基金净值 |
| `/api/data-center/sync/financials/` | POST | 同步财务事实 |
| `/api/data-center/sync/valuations/` | POST | 同步估值事实 |
| `/api/data-center/sync/sectors/constituents/` | POST | 同步板块成分 |
| `/api/data-center/sync/news/` | POST | 同步新闻 |
| `/api/data-center/sync/capital-flows/` | POST | 同步资金流 |

#### 请求示例

```bash
GET /api/data-center/financials/?asset_code=000001.SZ
...
> **标准前缀**: `/api/data-center/`
> **页面入口**: `/data-center/providers/`、`/data-center/monitor/`

统一数据中台是系统唯一数据接入层。行情、新闻、资金流、宏观、基金、财务与估值都从这里对外提供，旧 market-data API 前缀已彻底下线。

| 端点 | 方法 | 描述 |
|------|------|------|
| `/api/data-center/prices/quotes/?asset_code={asset_code}` | GET | 获取单个资产最新行情快照 |
| `/api/data-center/prices/history/?asset_code={asset_code}&start={date}` | GET | 获取历史行情 |
| `/api/data-center/capital-flows/?asset_code={asset_code}&start={date}&end={date}` | GET | 获取个股资金流向 |
| `/api/data-center/news/?asset_code={asset_code}&limit={limit}` | GET | 获取个股相关新闻 |
| `/api/data-center/providers/status/` | GET | 获取 Provider 运行状态 |
| `/api/data-center/sync/quotes/` | POST | 同步最新行情到标准表 |
| `/api/data-center/sync/capital-flows/` | POST | 同步资金流向到标准表 |
| `/api/data-center/sync/news/` | POST | 同步新闻到标准表 |

#### 请求示例

```bash
# 获取单个资产最新行情
GET /api/data-center/prices/quotes/?asset_code=000001.SZ

# 获取近 10 天资金流向
GET /api/data-center/capital-flows/?asset_code=000001.SZ&start=2026-03-26

# 获取最近新闻
GET /api/data-center/news/?asset_code=000001.SZ&limit=10

# 查看 Provider 运行状态
GET /api/data-center/providers/status/

# 同步资金流向
POST /api/data-center/sync/capital-flows/
{
  "provider_id": 1,
...
# 同步新闻
POST /api/data-center/sync/news/
{
  "provider_id": 1,
  "asset_code": "000001.SZ",
  "limit": 20
...
// GET /api/data-center/prices/quotes/?asset_code=000001.SZ
{
  ...
  "turnover_rate": 3.2,
  "source": "eastmoney",
  "observed_at": "2026-04-05T09:35:00+08:00"
}
