...
- 2026-08-30 本个人项目的生效 [`single-owner 授权`](deployment/personal-project-single-owner-authorization-2026-08-30-80ea002b.json) 允许唯一真人 owner 承担 owner/root/reviewer/role-owner，不要求第二名自然人；技术事实和 fail-closed 门禁不豁免。`TUI-01/TUI-03` 已完成，successor `80ea002b…` / release `20260830215638` 已部署，retained source、10/10 UAT、108/108 cleanup、isolated rollback 和 Day 0 registry backup 均通过；动态汇总见 [`TUI-02 production Day 0`](deployment/tui02-production-day0-checkpoint-2026-08-30-80ea002b.json)。`TUI-02=active`，窗口 `2026-08-30..2026-09-13` 正在自然运行，readiness 仍为 `5/10 DENY`。
...
| 文档 | 说明 | 状态 |
|------|------|------|
| [active_plan_registry.json](../governance/active_plan_registry.json) | **活跃计划与 canonical closure backlog 机器真源（工作流 / owner / 状态 / 依赖 / 唯一退出门 / 文件归属 / 限期审查）** | 当前无 active repository unit，focus 显式为 null；`TUI-02=active` production observation，DATA/AUD/EVID/STRAT/TAR 仍按真实技术前置 fail-closed |
| [release-blocker-closure-execution-plan-2026-08-29.md](plans/release-blocker-closure-execution-plan-2026-08-29.md) | **发布阻塞清零综合实施方案（DATA/AUD/EVID/STRAT/TUI/TAR/AI/QMT 顺序、授权包、回滚点和停止线）** | 执行中；DATA-01 完成，DATA-02 已部署并完成 dry-run、仍受真实 authority/profile 前置阻断；TUI-02 正在自然观察，其余硬门保持 fail-closed |
...
| [web-to-tui-migration-plan-2026-07-25.md](plans/web-to-tui-migration-plan-2026-07-25.md) | **Web 界面 → TUI 整体迁移计划（M0-M5，195 模板去向矩阵 / 图表样板 / web 保留清单）** | 实施中；M0-M4、TUI-01/TUI-03 完成；TUI-02 successor Day 0 已通过，真实观察运行中且 final inventory DENY |
...
| [system-audit-log-consolidation-plan-2026-08-13.md](plans/system-audit-log-consolidation-plan-2026-08-13.md) | **系统级统一审计日志收口计划（统一事件账本 / 数据可靠性纵向链 / 指标告警 / TUI 观测）** | AUD-01 canonical publisher/runtime/authority 与 AUD-02 Data Reliability same-UOW/replay、cross-App corrective regression 均完成 repository exit；AUD-03 仍需候选绑定的生产 migration/rollback、PostgreSQL、backlog/metrics/alerts/TUI、archive/restore 与双签 |
...
- 当前执行波次规则已与 registry 的 `execution_focus=EVID-01` 对齐：TAR-01 至 TAR-04 repository 合同完成，生产 auto-collect 可并行，但不因健康或本地合同解除 Evidence、决策、TUI 或容量门禁。
