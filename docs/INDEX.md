...
| [evidence-governance-and-decision-hard-gate-remediation-plan-2026-08-12.md](plans/evidence-governance-and-decision-hard-gate-remediation-plan-2026-08-12.md) | **证据治理与决策硬闸改造计划** | 第一期 P0：M0 已冻结外部写面、10 个 Transition Plan writer、62 个显式输出、18 个动态面及 18 个 MCP P0 发布面；Terminal result bridge 与 6 个 Broker native read 已暂停（MCP disabled=7、integrated=0）；Equity research snapshot 已收敛为唯一语义链但仍未接 Evidence；M1 核心账本与 Data Center/Broker/Strategy DecisionResult+OrderIntent 首批 legacy adapters 已完成，均未授予执行权限 |
