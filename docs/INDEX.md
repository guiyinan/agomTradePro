...
| [evidence-governance-and-decision-hard-gate-remediation-plan-2026-08-12.md](plans/evidence-governance-and-decision-hard-gate-remediation-plan-2026-08-12.md) | **证据治理与决策硬闸改造计划** | 第一期 P0：已冻结 10 个 Transition writer、65 个显式/18 个动态输出及 18 个 MCP P0 read 面；Portfolio submit、Advisor draft 与 Broker create/approve/lease/submitting 均硬阻断；Broker current-data=49，order detail/catalog、audit、reconciliation 已作 actor-aware display-only/脱敏，execution authorization exact schema 已冻结但明确 inactive；MCP P0 read disabled=6/integrated=0，尚缺 Portfolio/Risk owner receipts、ledger/provider 与四节点重验 |
