...
| [evidence-governance-and-decision-hard-gate-remediation-plan-2026-08-12.md](plans/evidence-governance-and-decision-hard-gate-remediation-plan-2026-08-12.md) | **证据治理与决策硬闸改造计划** | 第一期 P0：已冻结 10 个 Transition writer、65 个显式/18 个动态输出及 18 个 MCP P0 read 面；Portfolio submit、Advisor draft 与 Broker create/approve/lease/submitting 均硬阻断；Broker current-data=49，order detail/catalog、audit、reconciliation 已分别作 actor-aware display-only closed projection或动态字段脱敏，MCP P0 read disabled=6/integrated=0，首批 adapters 均未授予执行权限 |
