...
- 2026-09-02 Web liveness incident 的仓库整改已补齐可选 server-side host watchdog：[`vps-web-watchdog.sh`](../scripts/vps-web-watchdog.sh) 只在 Docker `web` 连续 unhealthy 后按 cooldown/rolling budget 执行 `compose restart web`，不 kill shared PID、重启 Celery 或触碰数据；systemd service/timer 模板与四项行为测试已纳入部署包。该 watcher 尚未安装或在 VPS 执行，故不改变当前候选、TUI-02 观察或任何生产门禁。
