...
- Personal Auto Advisor is exposed as native MCP tools via the SDK MCP server:
  ...
  - `create_auto_advisor_weekly_report(account_id, as_of=None)` generates and persists the weekly report, investment diary, dashboard notification, and audit log.
  ...
  These tools do not execute trades and still rely on backend auth and role checks.
