...
17. 已核验 `realtime` price alert CRUD 属于错误遗留契约：当前 server build 没有 `/api/realtime/alerts/` canonical API，也没有 `PriceAlert` 实现；因此 SDK/raw MCP 入口必须显式 fail fast，而不是继续猜测不存在的 path。
...
36. 同一模块只允许迁移证据闭合的子集。Fund 已完成 persisted-only ranking、detail、NAV history、holdings 与 pure-compute screen；NAV governed schema 不发布 canonical API 未执行的 legacy `limit`，holdings 不发布兼容别名 `as_of_date`。score、compatibility list/recommendation 与其他 analysis 路径仍须分别补齐唯一契约和副作用分类；POST performance calculation 已确认会持久化快照，必须按本标准的 workflow/write 规则治理。
...
39. Alpha 的 provider status、universe catalog 与 health 可以作为零参数直接 GET 子集独立迁移，分别固定为 `alpha.read.provider_status`、`alpha.read.universe_catalog`、`alpha.read.health`。两条 staff-only 运维读取固定为 `alpha.read.inference_ops_overview` 与 `alpha.read.qlib_data_ops_overview`，均只发布零参数契约并解包 canonical `success + data`；manifest 必须声明 `required_roles=("staff",)`，服务端仍执行真实 staff 权限校验。Inference overview 不得在读取时清理 stale/completed cache lock，runtime Qlib 配置在 singleton 缺失时必须使用未持久化默认对象；Qlib data overview 只检查本地 calendar 文件和持久化任务结果，不得刷新文件或投递任务。trigger/refresh 继续属于 workflow/write；factor exposure 当前 SDK/raw 路径会直接启动本地 Django service，不得伪装成 canonical HTTP read。
...
56. Strategy read family 必须逐路径证明 canonical owner/staff scope。仅有 `IsAuthenticated` 不能替代对象范围授权；strategy、AI config、position rule 等读取必须由服务端 queryset 执行对象范围隔离。基础 strategy catalog/detail 已按第 68 条完成收口，AI config catalog/detail 已按第 69 条完成收口，position rule catalog/detail 已按第 70 条完成收口；script config、rule condition 和 assignment 在各自 queryset 完成同等范围校验前继续冻结。SDK 指向当前 ViewSet 不存在的 `performance/signals/positions` action 时同样必须冻结，禁止用猜测路由或 MCP fallback 掩盖缺失 endpoint。
...
59. Equity 估值只读子集固定为 `equity.read.valuation_repair_list`、`equity.read.valuation_freshness` 与 `equity.read.valuation_quality_latest`。repair list 只读取持久化修复快照，并归一化为 `repairs + total_count + query`；freshness 和 latest quality 只读取现有估值记录与质量快照，不得触发 provider sync、repair scan、validation 或快照创建。`get_stock_valuation` 在 SDK `as_of_date` 与 canonical `lookback_days` 输入统一前必须冻结；repair status/history 必须单独证明计算链纯读；score/detail/recommendation/composite analysis、scan/sync/validate 和配置 mutation 必须按缺 canonical owner、pure calculation 或 workflow/write 分流。估值配置读取在 staff 权限和无配置 fallback 语义闭合前不得迁移。
...
102. Sector 持久化轮动排名固定收口为 `sector.read.rotation_ranking`。Governed 输入只允许可选 `regime`、`5..120` 的 `lookback_days`、`SW1|SW2|SW3` 的 `level` 和 `1..50` 的 `top_n`；canonical GET 必须显式使用 `IsAuthenticated` 和严格 query serializer，未知参数必须返回 400。未提供 `regime` 时只能读取最新持久化 Regime 快照，不得触发实时 Regime 计算、宏观 provider 或快照保存。Sector GET 与 POST analyze 都不得因本地数据缺失调用 `UpdateSectorDataUseCase`；数据初始化和 provider 同步只能通过 staff-only 的显式 `POST /api/sector/update-data/` 执行。沪深 300 基准收益读取必须固定 `hydrate=False`，本地不足时只能使用无基准降级值，不得远端补数或写缓存。Focused API contract 必须证明 `SectorInfoModel`、`SectorIndexModel`、`SectorConstituentModel`、`SectorRelativeStrengthModel` 与 `SectorPreferenceConfigModel` 的记录和时间戳零变化，并以 fail-fast patch 证明 Sector provider sync 与基准行情远端 hydration 未调用。正式 SDK 统一发布 `get_rotation_ranking()`；legacy `list_sectors`、`get_sector_recommendations`、`get_hot_sectors` 仅作为显式参数兼容 alias，共同 replacement 到该 capability。Controlled fallback 只能调用正式 SDK，不得新增 raw `@server.tool()`。`get_sector_score`、`analyze_sector` 与 detail/performance 派生语义不得借排名证据一并迁移，必须另补 canonical contract。
