---
name: mcp-remote-agomsaaf
description: "Connect to remote VPS AgomSAAF system (141.11.211.21:8000) via MCP tools or direct API calls. Use when querying regime status, macro data, signals, backtest results, or managing account positions on the production system."
---

# MCP Remote AgomSAAF Connection

## Overview

Connect to the production AgomSAAF system on VPS using MCP (Model Context Protocol) or direct REST API calls. This skill enables querying macro regime, policy status, signals, backtest results, and managing account data on the remote system.

## Connection Info

- **VPS URL**: `http://141.11.211.21:8000`
...
curl -s -H "Authorization: Token 28c11b3441d70f581c7410faeb5baeb90901c005" \
  http://141.11.211.21:8000/api/regime/current/

# Policy status (returns current_level, level_name, recommendations)
curl -s -H "Authorization: Token 28c11b3441d70f581c7410faeb5baeb90901c005" \
  http://141.11.211.21:8000/api/policy/status/

# List signals
curl -s -H "Authorization: Token 28c11b3441d70f581c7410faeb5baeb90901c005" \
  http://141.11.211.21:8000/api/signals/

# Supported macro indicators (52 indicators available)
curl -s -H "Authorization: Token 28c11b3441d70f581c7410faeb5baeb90901c005" \
  http://141.11.211.21:8000/api/macro/supported-indicators/

# API root (lists all available endpoints)
curl -s -H "Authorization: Token 28c11b3441d70f581c7410faeb5baeb90901c005" \
  http://141.11.211.21:8000/api/
```
...
class AgomSAAFClient:
    """Client for AgomSAAF VPS API"""

    def __init__(self, base_url="http://141.11.211.21:8000",
                 token="28c11b3441d70f581c7410faeb5baeb90901c005"):
...
```json
{
  "mcpServers": {
    "agomsaaf-vps": {
      ...
      "env": {
        "AGOMSAAF_BASE_URL": "http://141.11.211.21:8000",
        "AGOMSAAF_API_TOKEN": "28c11b3441d70f581c7410faeb5baeb90901c005",
...
1. **Connection refused**
   ```bash
   ping 141.11.211.21
   curl -I http://141.11.211.21:8000/
   ```
