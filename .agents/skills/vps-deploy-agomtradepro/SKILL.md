...
- `dist/agomtradepro-vps-bundle-$tag.tar.gz`
...
- `dist/agomtradepro-vps-bundle-$tag-live.tar.gz`

Manual fallback remains:

```powershell
pwsh -File scripts/inject-sqlite-into-bundle.ps1 `
  -SourceBundle dist/agomtradepro-vps-bundle-$tag.tar.gz `
  -OutputTag "$tag-live"
```
...
rm -rf /opt/agomtradepro/current /opt/agomtradepro/releases/* || true
mkdir -p /opt/agomtradepro/releases
```
...
docker compose -p agomtradepro -f docker/docker-compose.vps.yml --env-file deploy/.env up -d web caddy
```
