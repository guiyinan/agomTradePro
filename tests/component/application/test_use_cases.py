...
class TestSyncMacroDataUseCase:
    ...
    def test_execute_with_empty_data(self):
        ...
        request = SyncMacroDataRequest(start_date=date(2024, 1, 1), indicators=["CN_PMI"])
    ...
    def test_unit_conversion_for_currency(self):
        ...
        request = SyncMacroDataRequest(start_date=date(2024, 1, 1), indicators=["CN_NEW_CREDIT"])
...
class TestGetLatestMacroDataUseCase:
    ...
    def test_execute_success(self):
        ...
        request = GetLatestMacroDataRequest(
            indicator_codes=["CN_PMI", "CN_CPI"], as_of_date=date(2024, 1, 31)
        )
    ...
    def test_execute_with_missing_data(self):
        ...
        request = GetLatestMacroDataRequest(indicator_codes=["CN_PMI", "CN_CPI"])  # CN_CPI 无数据
...
class TestCalculateRegimeUseCase:
    ...
    def test_execute_success(self):
        ...
        request = CalculateRegimeRequest(
            as_of_date=date(2024, 1, 1), growth_indicator="PMI", inflation_indicator="CPI"
        )
    ...
    def test_execute_insufficient_data_with_fallback(self):
        ...
        use_case = CalculateRegimeUseCase(
            repository=repository, regime_repository=regime_repository
        )
    ...
    def test_execute_with_pit_mode(self):
        ...
        request = CalculateRegimeRequest(as_of_date=date(2024, 1, 1), use_pit=True)  # 启用 PIT 模式
        ...
        repository.get_growth_series.assert_called_with(
            indicator_code="PMI", end_date=date(2024, 1, 1), use_pit=True, source=None
        )
    ...
    def test_fallback_count_reset_on_success(self):
        ...
        # 成功计算后计数器应被重置
        assert (
            use_case._consecutive_fallback_count == 0
        ), "Fallback counter should reset after success"
