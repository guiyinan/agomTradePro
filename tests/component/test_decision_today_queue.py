...
def test_today_queue_includes_adopted_conflict_blocking_plan_and_pending_approval():
    ...
    assert (
        next(item for item in items if item["type"] == "recommendation_adopted")["next_action"]
        == "生成计划"
    )
    assert (
        next(item for item in items if item["type"] == "recommendation_conflict")["next_action"]
        == "处理冲突"
    )
    assert (
        next(item for item in items if item["type"] == "transition_plan_blocking")["next_action"]
        == "补齐风控/证伪"
    )
    assert (
        next(item for item in items if item["type"] == "execution_approval_pending")["next_action"]
        == "审批执行"
    )
