...
from apps.account.interface.admin import MacroSizingConfigModelAdmin
...
def test_macro_sizing_admin_marks_json_fields_readonly():
    registered_admin = admin.site._registry[MacroSizingConfigModel]

    assert isinstance(registered_admin, MacroSizingConfigModelAdmin)
    assert {
        "regime_tiers_json",
        "pulse_tiers_json",
        "drawdown_tiers_json",
    }.issubset(registered_admin.readonly_fields)
