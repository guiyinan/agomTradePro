...
class CacheKeyBuilderTests(TestCase):
    ...
    def test_build_key_with_query_params(self):
        ...
        key = CacheKeyBuilder.build(prefix="test", request=request, vary_on=["date", "type"])
    ...
    def test_build_key_with_user(self):
        ...
        key = CacheKeyBuilder.build(prefix="test", request=request, include_user=True)
    ...
    def test_build_key_from_args(self):
        ...
        key = CacheKeyBuilder.build_from_args(
            prefix="myfunc", func_name="calculate", args=("arg1", "arg2"), kwargs={"param": "value"}
        )
    ...
    def test_long_key_hashing(self):
        ...
        key = CacheKeyBuilder.build(prefix="test", request=request, vary_on=["x" * 100])
...
class PrometheusMetricsTests(TestCase):
    ...
    def test_cache_hits_metric(self):
        ...
        self.assertGreater(
            cache_hits_total.labels(endpoint="test", key_prefix="test")._value._value, 0
        )
    ...
    def test_cache_misses_metric(self):
        ...
        self.assertEqual(
            cache_misses_total.labels(endpoint="test", key_prefix="test")._value._value, 3
        )
    ...
    def test_cache_stale_metric(self):
        ...
        self.assertGreater(cache_stale_total.labels(endpoint="test")._value._value, 0)
