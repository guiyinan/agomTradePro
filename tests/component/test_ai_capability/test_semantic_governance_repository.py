...
def test_repository_applies_set_and_remove_in_one_audited_batch(
    ...
    assert CapabilitySemanticAuditModel.objects.filter(batch_id=result.batch_id).count() == 2
...
def test_catalog_upsert_preserves_collected_and_effective_semantic_keys() -> None:
    ...
    DjangoCapabilityRepository().bulk_upsert(
        [capability],
        collected_semantic_keys={"mcp_tool.replay_events": "legacy.mcp.replay_events"},
    )

    model = CapabilityCatalogModel.objects.get(capability_key="mcp_tool.replay_events")
