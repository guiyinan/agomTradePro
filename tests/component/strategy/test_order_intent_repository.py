...
def _build_intent(
    strategy_id: int, portfolio_id: int, *, key: str, status: OrderStatus = OrderStatus.DRAFT
):
    return OrderIntent(
...
def test_get_pending_intents_filters_by_portfolio_and_status():
    ...
    draft = _build_intent(
        strategy.id, portfolio.id, key=f"k-{uuid.uuid4()}", status=OrderStatus.DRAFT
    )
    pending = _build_intent(
        strategy.id, portfolio.id, key=f"k-{uuid.uuid4()}", status=OrderStatus.PENDING_APPROVAL
    )
    approved = _build_intent(
        strategy.id, portfolio.id, key=f"k-{uuid.uuid4()}", status=OrderStatus.APPROVED
    )
    sent = _build_intent(
        strategy.id, portfolio.id, key=f"k-{uuid.uuid4()}", status=OrderStatus.SENT
    )
