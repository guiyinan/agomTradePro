...
def test_decision_repository_generates_a_persistable_domain_decision_id() -> None:
    ...
    assert GateDecisionModel._default_manager.filter(decision_id=saved.decision_id).exists()
...
def test_universe_repository_saves_the_domain_as_of_date() -> None:
    ...
    saved = VisibilityUniverseSnapshotModel._default_manager.get(snapshot_id=snapshot_id)
