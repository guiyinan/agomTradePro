...
class TestTerminalPermissionService:
    ...
    def _make_cmd(self, name, risk_level=TerminalRiskLevel.READ, requires_mcp=True, enabled=True):
...
class TestConfirmationTokenService:
    ...
    def test_wrong_params_rejected(self, token_service):
        ...
        valid, error = token_service.validate_token(
            token, 1, "cmd", {"a": 2}, "read", "confirm_each"
        )
        assert valid is False
...
class TestExecuteCommandGovernance:
    ...
    def test_confirm_each_returns_token_for_write(
        self, mock_repo, mock_execution_service, mock_audit_repo
    ):
        from apps.terminal.application.use_cases import ExecuteCommandRequest, ExecuteCommandUseCase
    ...
    def test_auto_confirm_executes_directly(
        self, mock_repo, mock_execution_service, mock_audit_repo
    ):
        from apps.terminal.application.use_cases import ExecuteCommandRequest, ExecuteCommandUseCase
