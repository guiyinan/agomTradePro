...
def test_path_normalization_semantics_keys_and_labels() -> None:
    ...
    assert _normalize_api_path(r"^api/items/(?P<ITEM_ID>\d+)/\Z$") == ("api/items/<item_id>/")
    assert _normalize_api_path("api/items.<(?P<format>json)>") is None
    assert _derive_semantic_key("api/data-center/items/<item_id>/") == ("data_center.items")
    collector = ApiCapabilityCollector()
    assert collector._create_capability_key("api/items/<id>/", "GET") == ("api.get.api.items.id")
...
def test_capability_creation_classifies_read_write_and_unsafe_routes() -> None:
    ...
    unsafe = collector._create_capability_for_view("api/items/", "POST", unsafe_view)
...
def test_view_metadata_docstrings_permissions_tags_and_schema() -> None:
    ...
    assert capability.input_schema["properties"]["name"]["description"] == ("Display name")
