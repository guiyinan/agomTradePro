...
def test_decision_builders_cover_capability_suggestion_and_chat(
    ...
    monkeypatch.setattr(
        facade, "_execute_capability", lambda *_args: {"reply": "ready"}
    )

    selected = facade._build_capability_decision(
        capability, candidates, 0.9, "status", context
    )
    ...
    suggestion = facade._build_suggestion_decision(
        capability, candidates, 0.7, "status", context
    )
...
def test_execute_capability_dispatches_non_builtin_sources(
    ...
    result = _facade()._execute_capability(
        _capability(source_type), "run", _context()
    )
...
def test_execute_capability_dispatches_builtin_and_unknown_handlers(
    ...
    assert facade._execute_builtin(_capability(handler="market_regime")) == {
        "reply": "regime"
    }
    assert "Unknown builtin" in facade._execute_builtin(
        _capability(handler="missing")
    )["reply"]
    unknown = SimpleNamespace(source_type="other")
    assert "Unknown capability" in facade._execute_capability(
        unknown, "run", _context()
    )["reply"]
...
def test_market_regime_formats_runtime_state(
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            get_current_policy_level=lambda: SimpleNamespace(value="neutral")
        ),
    )
...
def test_no_candidate_handler_delegates_logging_and_response(
    ...
    monkeypatch.setattr(
        facade, "_build_response", lambda *_args: {"decision": "chat"}
    )
    assert facade._handle_no_candidates("hello", "session-1", context) == {
        "decision": "chat"
    }
