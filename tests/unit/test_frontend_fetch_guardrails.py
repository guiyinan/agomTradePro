...
def test_terminal_provider_selector_is_bootstrapped_before_lazy_mermaid_loading():
    template = Path("core/templates/terminal/index.html").read_text(encoding="utf-8")
    script = Path("static/js/terminal.js").read_text(encoding="utf-8")

    assert 'json_script:"terminal-provider-bootstrap"' in template
    assert "mermaidScriptUrl" in template
    assert "<script src=\"{% static 'js/mermaid.min.js' %}\"></script>" not in template
    assert "readProviderBootstrap()" in script
    assert "ensureMermaidRenderer()" in script
    assert "fetchJsonWithTimeout" in script
...
def test_high_risk_templates_use_shared_fetch_helpers():
    ...
    for template in templates:
        ...
        for pattern in banned_patterns:
            ...
            ), f"{template} still contains unsafe JSON parsing pattern: {pattern}"
...
def test_exit_chain_templates_use_shared_dashboard_detail_urls():
    ...
    for template, placeholder in templates_with_expected_placeholders.items():
        ...
        ), f"{template} should use shared dashboard detail url placeholder"
        for pattern in banned_patterns:
            assert (
                pattern not in content
            ), f"{template} still hardcodes dashboard exit detail link: {pattern}"
...
def test_workspace_entrypoints_use_canonical_bridge_params_and_keep_legacy_compat():
    ...
    workspace_content = Path("core/templates/decision/workspace.html").read_text(encoding="utf-8")
    ...
    assert (
        "workspaceParams.get('security_code') || workspaceParams.get('asset_code') || ''"
        in workspace_content
    )
    assert (
        "workspaceParams.get('action') || workspaceParams.get('direction') || ''"
        in workspace_content
    )
    assert (
        "workspaceParams.get('execute_request') ? 'alpha-trigger-execute' : ''" in workspace_content
    )
...
def test_dashboard_and_equity_workspace_entrypoints_use_canonical_builders():
    main_workflow_content = Path("core/templates/dashboard/main_workflow_panel.html").read_text(
        encoding="utf-8"
    )
    alpha_ranking_content = Path("core/templates/dashboard/alpha_ranking.html").read_text(
        encoding="utf-8"
    )
    equity_screen_content = Path("core/templates/equity/screen.html").read_text(encoding="utf-8")
    ...
    assert (
        "/decision/workspace/?source=equity-screen&security_code=${encodeURIComponent(item.code || '')}"
        not in equity_screen_content
    )
...
def test_equity_detail_uses_single_asset_realtime_endpoint_and_renders_price_timestamp():
    ...
    assert (
        "document.getElementById('detailPriceTimestamp').textContent = '实时价格不可用'" in content
    )
    ...
    assert (
        "updatePriceTimestamp(latest.updated_at || latest.trade_date, '估值数据时间')"
        not in content
    )
    ...
    assert (
        "/api/equity/technical/${stockCode}/?timeframe=${timeframe}&lookback_days=${technicalLookbackDays[timeframe] || technicalLookbackDays.day}"
        in content
    )
    ...
    assert (
        'class="btn btn-secondary technical-timeframe-btn active" data-technical-timeframe="intraday"'
        not in content
    )
