...
def test_chain_missing_configuration_returns_explicit_failure_response() -> None:
    ...
    assert result.error_message == "Chain not found: 404"
...
def test_report_and_signal_facades_forward_provider_user_and_safe_fallbacks() -> None:
    chain_use_case = Mock()
    chain_use_case.execute.side_effect = [
        SimpleNamespace(
            final_output=None,
            total_tokens=8,
            total_cost=0.3,
        ...
        SimpleNamespace(final_output="BUY because momentum"),
    ]
    ...
    report_request = chain_use_case.execute.call_args_list[0].args[0]
    assert report_request.chain_id == 1
    ...
    assert signal.logic_desc == "BUY because momentum"
    signal_request = chain_use_case.execute.call_args_list[1].args[0]
    assert signal_request.chain_id == 2
