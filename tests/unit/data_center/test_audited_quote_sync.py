"""RED contract tests for the audited realtime-quote sync path."""
...
class _Provider:
    def __init__(
        self, quotes: list[QuoteSnapshot] | None = None, error: Exception | None = None
    ) -> None:
...
def test_success_correlates_quote_identity_raw_audit_and_publication_events() -> None:
    use_case, uow, writer, facts, raw, quality_recorder = _build([_quote()], publisher=_Publisher())
    ...
    assert uow.events[0] == "begin"
    assert uow.events[-1] == "commit"


def test_noop_emits_fetch_noop_and_no_publication() -> None:
...
def test_audit_writer_failure_rolls_back_and_is_not_hidden() -> None:
    use_case, uow, writer, _facts, _raw, quality_recorder = _build([_quote()])
    writer.fail = True
...
def test_publication_block_commits_blocked_event_then_reraises() -> None:
    use_case, _uow, writer, _facts, _raw, quality_recorder = _build(
        [_quote()], publisher=_Publisher(error=ValueError("publication blocked"))
    )
