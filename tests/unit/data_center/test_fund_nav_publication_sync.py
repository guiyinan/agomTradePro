from __future__ import annotations

from datetime import UTC, date, datetime

import pytest

from apps.data_center.application.publication_sync import PublishFundNavBatchUseCase
from apps.data_center.domain.contracts import DatasetKey, PublicationPolicy
from apps.data_center.domain.control_plane import CanonicalPublication, PublicationFactReference
from apps.data_center.domain.entities import FundNavFact
from apps.data_center.infrastructure.fundamental_fact_repositories import FundNavRepository
from apps.data_center.infrastructure.models import FundNavFactModel

PUBLISHED_AT = datetime(2026, 8, 4, 12, 0, tzinfo=UTC)


class _CandidateRepository:
    def __init__(self, references: list[PublicationFactReference]) -> None:
        self.references = references

    def list_publication_candidates(self, _facts):
        return list(self.references)


class _PolicyRepository:
    def get_active(self, _dataset_key: str):
        return PublicationPolicy(
            dataset=DatasetKey("fund.nav", "1.0", "1.0"),
            minimum_coverage_ratio=0.99,
            allow_partial=True,
            conflict_action="block",
            required_evidence=("source", "observed_at", "payload_hash"),
            retention_days=1825,
        )


class _PublicationRepository:
    def __init__(self) -> None:
        self.current: CanonicalPublication | None = None
        self.published: list[tuple[CanonicalPublication, tuple[object, ...]]] = []

    def get_current(self, _dataset_key: str, _publication_key: str):
        return self.current

    def publish_with_members(self, publication, members):
        self.current = publication
        self.published.append((publication, members))
        return publication


def _fact(fund_code: str, nav_date: date) -> FundNavFact:
    return FundNavFact(
        fund_code=fund_code,
        nav_date=nav_date,
        nav=1.234,
        source="provider-main",
        fetched_at=PUBLISHED_AT,
    )


def _reference(fund_code: str, nav_date: date, fact_pk: str) -> PublicationFactReference:
    return PublicationFactReference(
        natural_key=f"{fund_code}:{nav_date.isoformat()}:provider-main",
        source="provider-main",
        source_record_id=f"{fund_code}:{nav_date.isoformat()}",
        fact_table="data_center_fund_nav_fact",
        fact_pk=fact_pk,
        observed_at=datetime(nav_date.year, nav_date.month, nav_date.day, tzinfo=UTC),
        raw_payload_hash="d" * 64,
    )


def test_fund_nav_publication_uses_nav_date_as_of_and_exact_members() -> None:
    nav_date = date(2026, 8, 3)
    facts = [_fact("F000001", nav_date), _fact("F000002", nav_date)]
    repository = _PublicationRepository()
    use_case = PublishFundNavBatchUseCase(
        fact_repository=_CandidateRepository(
            [_reference("F000001", nav_date, "201"), _reference("F000002", nav_date, "202")]
        ),
        publication_repository=repository,
        policy_repository=_PolicyRepository(),
    )

    publication = use_case.execute(facts, provider_name="provider-main", published_at=PUBLISHED_AT)

    assert publication is not None
    assert publication.as_of == datetime(2026, 8, 3, tzinfo=UTC)
    assert publication.as_of != facts[0].fetched_at
    assert publication.coverage.selected_count == 2
    assert publication.coverage.missing_count == 0
    assert {member.fact_pk for member in repository.published[0][1]} == {"201", "202"}


def test_fund_nav_publication_is_idempotent_for_same_snapshot() -> None:
    nav_date = date(2026, 8, 3)
    facts = [_fact("F000001", nav_date)]
    repository = _PublicationRepository()
    use_case = PublishFundNavBatchUseCase(
        fact_repository=_CandidateRepository([_reference("F000001", nav_date, "201")]),
        publication_repository=repository,
        policy_repository=_PolicyRepository(),
    )

    first = use_case.execute(facts, provider_name="provider-main", published_at=PUBLISHED_AT)
    second = use_case.execute(facts, provider_name="provider-main", published_at=PUBLISHED_AT)

    assert first is second
    assert len(repository.published) == 1


def test_fund_nav_publication_fails_closed_below_coverage_policy() -> None:
    nav_date = date(2026, 8, 3)
    facts = [_fact("F000001", nav_date), _fact("F000002", nav_date)]
    use_case = PublishFundNavBatchUseCase(
        fact_repository=_CandidateRepository([_reference("F000001", nav_date, "201")]),
        publication_repository=_PublicationRepository(),
        policy_repository=_PolicyRepository(),
    )

    with pytest.raises(ValueError, match="coverage"):
        use_case.execute(facts, provider_name="provider-main", published_at=PUBLISHED_AT)


@pytest.mark.django_db
def test_fund_nav_repository_candidate_preserves_nav_date_observation() -> None:
    nav_date = date(2026, 8, 3)
    row = FundNavFactModel.objects.create(
        fund_code="F000001",
        nav_date=nav_date,
        nav=1.234,
        source="provider-main",
        source_record_id="nav-1",
        raw_payload_hash="e" * 64,
    )

    references = FundNavRepository().list_publication_candidates([_fact("F000001", nav_date)])

    assert len(references) == 1
    assert references[0].fact_pk == str(row.pk)
    assert references[0].source_record_id == "nav-1"
    assert references[0].raw_payload_hash == "e" * 64
    assert references[0].observed_at == datetime(2026, 8, 2, 16, tzinfo=UTC)
    assert references[0].observed_at != row.fetched_at
