...
def test_analyze_text_failure_creates_neutral_result_and_alert(
    ...
    monkeypatch.setattr(services, "get_sentiment_alert_repository", lambda: alert_repo)
    ...
    alert_payload = alert_repo.create_alert.call_args.kwargs
    assert "quota" not in str(alert_payload)
...
def test_parser_confidence_category_and_keyword_fallbacks() -> None:
    ...
    assert analyzer._parse_sentiment_score("no score") is None
    ...
    assert analyzer._categorize_sentiment(0.51) == SentimentCategory.POSITIVE
    assert analyzer._categorize_sentiment(-0.51) == SentimentCategory.NEGATIVE
    assert analyzer._categorize_sentiment(0.5) == SentimentCategory.NEUTRAL
    assert analyzer._categorize_sentiment(-0.5) == SentimentCategory.NEUTRAL
