...
def _internal_signature(*, secret: str, timestamp: str, method: str, path: str, user_id: int, username: str) -> str:
...
def test_terminal_internal_authentication_rejects_invalid_signature():
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        monkeypatch.setattr("apps.account.interface.authentication.time.time", lambda: 1700000000)
        with pytest.raises(exceptions.AuthenticationFailed, match="Invalid internal auth signature"):
