...
def test_current_advisor_sheet_requires_matching_preview_digest() -> None:
    ...
    creator = _OrderCreator()
    plan_creator = CreateLiveOrdersFromAdvisorExecutionPlanUseCase(
...
def test_authorized_trader_uses_real_account_owner_projection_for_draft() -> None:
    ...
    order = LiveOrderModel.objects.get(
...
def test_four_dimension_reconciliation_is_idempotent_and_auto_stops_on_p0() -> None:
    ...
    first = repository.generate_reconciliation_runs(
        account_projections={77: projection}
    )
    second = repository.generate_reconciliation_runs(
        account_projections={77: projection}
    )
    ...
    assert set(
        ReconciliationDifferenceModel.objects.filter(run=run).values_list(
            "dimension", flat=True
        )
