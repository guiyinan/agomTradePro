...
def test_broker_execution_repository_split_is_bounded_and_compatible() -> None:
    ...
    owners = _assert_owner_contract(
        ...
        owner_budgets={
            "apps.broker_execution.infrastructure.broker_repository_access": 800,
            "apps.broker_execution.infrastructure.broker_repository_order_control": 350,
            "apps.broker_execution.infrastructure.broker_repository_agent_runtime": 850,
            "apps.broker_execution.infrastructure.broker_repository_agent_administration": 500,
            "apps.broker_execution.infrastructure.broker_repository_reconciliation": 700,
        },
    )
    repository_type = broker_repositories.DjangoBrokerExecutionRepository
    expected_mixins = {
        "apps.broker_execution.infrastructure.broker_repository_access": (
            "BrokerExecutionAccessMixin"
        ),
        "apps.broker_execution.infrastructure.broker_repository_order_control": (
            "BrokerExecutionOrderControlMixin"
        ),
        "apps.broker_execution.infrastructure.broker_repository_agent_runtime": (
            "BrokerExecutionAgentRuntimeMixin"
        ),
        "apps.broker_execution.infrastructure.broker_repository_agent_administration": (
            "BrokerExecutionAgentAdministrationMixin"
        ),
        "apps.broker_execution.infrastructure.broker_repository_reconciliation": (
            "BrokerExecutionReconciliationMixin"
        ),
    }
    for module_name, owner in owners.items():
        assert issubclass(repository_type, getattr(owner, expected_mixins[module_name]))
    assert _non_empty_lines(broker_repositories) <= 300
...
def test_simulated_trading_repository_split_is_bounded_and_compatible() -> None:
    ...
    assert _non_empty_lines(simulated_repositories) <= 50
