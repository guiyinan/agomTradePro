...
def _payload_v2(*, row_counts: dict[str, int] | None = None) -> bytes:
    """Build the post-0055 authority inventory contract fixture."""

    counts = row_counts or dict.fromkeys(EVID_01_INVENTORY_TABLES_V2, 0)
    value: dict[str, Any] = {
        "version": "evid-01-authority-inventory-snapshot.v2",
        "environment": "production",
        "captured_at": "2026-08-24T12:10:43.123456Z",
        "candidate": {
            "stable_version": "20260824121000",
            "source_commit": "a28aa5fdeeaa11bb22cc33dd44ee55ff66778899",
            "release": "agomtradepro-web:20260824121000",
        },
        "read_only": True,
        "database": {
            "backend": "postgresql",
            ...
            "migrations": [
                {
                    "app": "account",
                    "name": name,
                    "applied_at": f"2026-08-24T05:32:{index:02d}.123456Z",
                }
                for index, name in enumerate(EVID_01_INVENTORY_MIGRATIONS_V2, start=7)
            ],
        },
        "row_counts": counts,
    }
...
def test_v2_inventory_includes_0054_0055_and_owner_tenant_ledger() -> None:
    """The v2 parser recognizes the new ledger without opening authority."""

    counts = dict.fromkeys(EVID_01_INVENTORY_TABLES_V2, 0)
    counts["account_owner_tenant_authority_v1"] = 1
    snapshot = parse_evid_01_authority_inventory_snapshot(_payload_v2(row_counts=counts))
    assert snapshot.format_version == "evid-01-authority-inventory-snapshot.v2"
    assert (
        tuple(item.name for item in snapshot.database.migrations) == EVID_01_INVENTORY_MIGRATIONS_V2
    )
    assert tuple(table for table, _ in snapshot.row_counts) == EVID_01_INVENTORY_TABLES_V2
    report = build_evid_01_authority_inventory_report(snapshot)
    assert report.outcome is Evid01AuthorityInventoryOutcome.BLOCKED_UNVERIFIED_AUTHORITY
    assert (
        b'"version":"evid-01-authority-inventory-report.v2"'
...
def test_v2_cli_dispatch_remains_dry_run_by_default(tmp_path: Path) -> None:
    ...
    input_path = tmp_path / "snapshot-v2.json"
    input_path.write_bytes(_payload_v2())
