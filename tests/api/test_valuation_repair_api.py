...
class TestValuationRepairStatusAPI:
    ...
    def test_status_endpoint_accepts_dot_in_stock_code(
        self, api_client, mock_stock_repo, sample_repair_status
    ):
        """
        ...
        with patch("apps.equity.interface.views.DjangoStockRepository") as MockRepo:
            ...
            mock_instance.get_stock_info.return_value = MagicMock(
                stock_code="000001.SZ", name="平安银行"
            )
            mock_instance.get_valuation_history.return_value = [
                MagicMock(
                    trade_date=date.today() - timedelta(days=i), pe=8.5 + i * 0.1, pb=0.9 + i * 0.01
                )
                for i in range(100)
            ]
    ...
    def test_status_endpoint_returns_canonical_success_contract(self, api_client):
        ...
        with patch("apps.equity.interface.views.GetValuationRepairStatusUseCase") as mock_use_case:
            mock_use_case.return_value.execute.return_value = MagicMock(
                success=True,
                data=payload,
            )

            response = api_client.get("/api/equity/valuation-repair/000001.SZ/?lookback_days=756")
...
class TestValuationRepairHistoryAPI:
    ...
    def test_history_endpoint_invalid_lookback_days_returns_400(self, api_client):
        ...
        response = api_client.get(
            "/api/equity/valuation-repair/000001.SZ/history/?lookback_days=abc"
        )
...
class TestValuationRepairScanAPI:
    ...
    def test_scan_endpoint_not_captured_by_status_route(self, api_client):
        ...
        """
        with (
            patch("apps.equity.interface.views.DjangoStockRepository") as MockStockRepo,
            ...
            response = api_client.post(
                "/api/equity/valuation-repair/scan/",
                {"universe": "all_active", "lookback_days": 756},
            )
    ...
    def test_scan_endpoint_blocked_when_quality_gate_not_passed(self, api_client):
        ...
        with patch("apps.equity.interface.views.ScanValuationRepairsUseCase") as MockUseCase:
            mock_use_case = MockUseCase.return_value
            mock_use_case.execute.return_value = MagicMock(
                success=False, error="valuation data quality gate not passed"
            )
    ...
    def test_scan_endpoint_does_not_expose_internal_failure(self, api_client):
        with patch("apps.equity.interface.views.ScanValuationRepairsUseCase") as mock_use_case:
            mock_use_case.return_value.execute.return_value = MagicMock(
                success=False,
                error="postgresql://user:secret@internal/valuation",
            )
        ...
        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert response.data["error"] == "估值修复扫描失败"
...
class TestValuationRepairListAPI:
    ...
    def test_list_endpoint_rejects_unknown_query_parameters(self, api_client):
        response = api_client.get("/api/equity/valuation-repair-list/?unexpected=true")
...
class TestValuationRepairContentType:
    ...
    def test_status_endpoint_returns_json(self, api_client):
        ...
        with patch("apps.equity.interface.views.DjangoStockRepository") as MockRepo:
            ...
            mock_instance.get_valuation_history.return_value = [
                MagicMock(trade_date=date.today() - timedelta(days=i), pe=8.5, pb=0.9)
                for i in range(100)
            ]

            response = api_client.get("/api/equity/valuation-repair/000001.SZ/")

            assert "application/json" in response.get(
                "Content-Type", ""
...
class TestValuationDataQualityAPI:
    ...
    def test_validate_endpoint_returns_snapshot(self, api_client):
        with patch(
            "apps.equity.interface.views.ValidateEquityValuationQualityUseCase"
        ) as MockUseCase:
            mock_use_case = MockUseCase.return_value
            mock_use_case.execute.return_value = MagicMock(
                success=True,
    ...
    def test_quality_latest_endpoint_returns_snapshot(self, api_client):
        with patch(
            "apps.equity.interface.views.GetLatestEquityValuationQualityUseCase"
        ) as MockUseCase:
            mock_use_case = MockUseCase.return_value
            mock_use_case.execute.return_value = MagicMock(
                success=True,
    ...
    def test_sync_endpoint_returns_statistics(self, api_client):
        with patch("apps.equity.interface.views.SyncEquityValuationUseCase") as MockUseCase:
            ...
            response = api_client.post(
                "/api/equity/valuation-data/sync/",
                {
                    "days_back": 1,
                    "primary_source": "akshare",
    ...
    def test_sync_endpoint_rejects_reversed_dates_before_use_case(self, api_client):
        with patch("apps.equity.interface.views.SyncEquityValuationUseCase") as mock_use_case:
            response = api_client.post(
                "/api/equity/valuation-data/sync/",
                {
    ...
    def test_sync_endpoint_does_not_expose_internal_failure(self, api_client):
        with patch("apps.equity.interface.views.SyncEquityValuationUseCase") as mock_use_case:
            mock_use_case.return_value.execute.return_value = MagicMock(
                success=False,
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data["error"] == "估值数据同步失败"
        assert "private-value" not in str(response.data)
