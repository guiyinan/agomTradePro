...
class TestDailyNetValueService(TestCase):
    ...
    def test_record_net_value_after_buy(self):
        ...
        self.net_value_service.record_and_update_performance(
            account_id=self.account.account_id, record_date=trade_date
        )
        ...
        self.assertAlmostEqual(
            float(net_value_record.market_value), float(account.current_market_value), places=2
        )

        # 验证累计收益率
        expected_cumulative_return = (
            (expected_net_value - float(self.account.initial_capital))
            / float(self.account.initial_capital)
        ) * 100
        self.assertAlmostEqual(
            net_value_record.cumulative_return, expected_cumulative_return, places=2
        )
    ...
    def test_equity_curve_consistency(self):
        ...
        for i, price in enumerate(price_sequence):
            ...
            if i == 0:
                ...
                position.unrealized_pnl_pct = (
                    (float(price_decimal) - float(position.avg_cost)) / float(position.avg_cost)
                ) * 100
                position.save()
                ...
                account_model.total_value = account_model.current_cash + Decimal(
            ...
            # 记录每日净值
            self.net_value_service.record_and_update_performance(
                account_id=self.account.account_id, record_date=current_date
            )
        ...
        expected_total_return = (
            (float(updated_account.total_value) - float(updated_account.initial_capital))
            / float(updated_account.initial_capital)
        ) * 100
    ...
    def test_max_drawdown_calculation(self):
        ...
        for point in equity_curve:
            ...
            drawdown = (
                ((peak_so_far - point["net_value"]) / peak_so_far * 100) if peak_so_far > 0 else 0
            )
    ...
    def test_auto_trading_engine_calls_performance_update(self):
        ...
        engine = AutoTradingEngine(
            ...
            price_provider=MockPriceProvider(prices={"000001.SZ": 10.50}),
        )
        ...
        trades_count = SimulatedTradeModel.objects.filter(
            account_id=self.account.account_id, execution_date=trade_date
        ).count()
    ...
    def test_daily_return_calculation(self):
        ...
        self.net_value_service.record_and_update_performance(
            account_id=self.account.account_id, record_date=day1
        )
        ...
        position.unrealized_pnl_pct = (
            (float(position.current_price) - float(position.avg_cost)) / float(position.avg_cost)
        ) * 100
        position.save()
        ...
        self.net_value_service.record_and_update_performance(
            account_id=self.account.account_id, record_date=day2
        )
        ...
        expected_daily_return = (
            (float(nv2.net_value) - float(nv1.net_value)) / float(nv1.net_value)
        ) * 100
    ...
    def test_drawdown_field_accuracy(self):
        ...
        for point in equity_curve:
            ...
            # 计算预期回撤
            expected_drawdown = (
                ((peak_so_far - current_value) / peak_so_far * 100) if peak_so_far > 0 else 0
            )
