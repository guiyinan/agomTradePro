...
[![MCP Tools](https://img.shields.io/badge/MCP_tools-321-orange.svg)](#ai-native-integration)
...
- Macro-governance and MCP docs are now aligned with the current local snapshot of `321` registered MCP tools
...
- **MCP Server (321 registered tools)** — plug directly into Claude, Cursor, or any MCP-compatible AI
...
**321 MCP tools** do not just expose a few query endpoints. They span macro, policy, signals, backtesting, accounts, portfolios, trading flows, AI capability routing, terminal commands, runtime orchestration, and system-level operations.
...
321   MCP tools (current local registration snapshot)
