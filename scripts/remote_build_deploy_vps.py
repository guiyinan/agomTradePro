...
OLD_TUSHARE_TOKEN=""
OLD_TUSHARE_HTTP_URL=""
OLD_TUSHARE_REQUEST_MODE=""
OLD_TUSHARE_GATEWAY_UPSTREAM=""
OLD_TUSHARE_GATEWAY_ALLOWED_IP=""
OLD_POSTGRES_DB=""
OLD_POSTGRES_USER=""
...
_read_old_keys() {
  ...
  [ -z "$OLD_TUSHARE_TOKEN" ] && OLD_TUSHARE_TOKEN="$(get_env_kv TUSHARE_TOKEN "$_src")"
  [ -z "$OLD_TUSHARE_HTTP_URL" ] && OLD_TUSHARE_HTTP_URL="$(get_env_kv TUSHARE_HTTP_URL "$_src")"
  [ -z "$OLD_TUSHARE_REQUEST_MODE" ] && OLD_TUSHARE_REQUEST_MODE="$(get_env_kv TUSHARE_REQUEST_MODE "$_src")"
  [ -z "$OLD_TUSHARE_GATEWAY_UPSTREAM" ] && OLD_TUSHARE_GATEWAY_UPSTREAM="$(get_env_kv TUSHARE_GATEWAY_UPSTREAM "$_src")"
  [ -z "$OLD_TUSHARE_GATEWAY_ALLOWED_IP" ] && OLD_TUSHARE_GATEWAY_ALLOWED_IP="$(get_env_kv TUSHARE_GATEWAY_ALLOWED_IP "$_src")"
...
EFFECTIVE_TUSHARE_TOKEN="${TUSHARE_TOKEN:-}"
if [ -z "$EFFECTIVE_TUSHARE_TOKEN" ] && [ -n "$OLD_TUSHARE_TOKEN" ]; then
  EFFECTIVE_TUSHARE_TOKEN="$OLD_TUSHARE_TOKEN"
fi
EFFECTIVE_TUSHARE_HTTP_URL="${TUSHARE_HTTP_URL:-}"
if [ -z "$EFFECTIVE_TUSHARE_HTTP_URL" ] && [ -n "$OLD_TUSHARE_HTTP_URL" ]; then
  EFFECTIVE_TUSHARE_HTTP_URL="$OLD_TUSHARE_HTTP_URL"
fi
EFFECTIVE_TUSHARE_REQUEST_MODE="${TUSHARE_REQUEST_MODE:-sdk_path}"
if [ -z "${TUSHARE_REQUEST_MODE:-}" ] && [ -n "$OLD_TUSHARE_REQUEST_MODE" ]; then
  EFFECTIVE_TUSHARE_REQUEST_MODE="$OLD_TUSHARE_REQUEST_MODE"
fi
set_env_kv "TUSHARE_TOKEN" "$EFFECTIVE_TUSHARE_TOKEN"
set_env_kv "TUSHARE_HTTP_URL" "$EFFECTIVE_TUSHARE_HTTP_URL"
set_env_kv "TUSHARE_REQUEST_MODE" "$EFFECTIVE_TUSHARE_REQUEST_MODE"
_persist_secrets_env "TUSHARE_TOKEN" "$EFFECTIVE_TUSHARE_TOKEN"
_persist_secrets_env "TUSHARE_HTTP_URL" "$EFFECTIVE_TUSHARE_HTTP_URL"
_persist_secrets_env "TUSHARE_REQUEST_MODE" "$EFFECTIVE_TUSHARE_REQUEST_MODE"

EFFECTIVE_TUSHARE_GATEWAY_UPSTREAM="${TUSHARE_GATEWAY_UPSTREAM:-}"
if [ -z "$EFFECTIVE_TUSHARE_GATEWAY_UPSTREAM" ] && [ -n "$OLD_TUSHARE_GATEWAY_UPSTREAM" ]; then
  EFFECTIVE_TUSHARE_GATEWAY_UPSTREAM="$OLD_TUSHARE_GATEWAY_UPSTREAM"
fi
EFFECTIVE_TUSHARE_GATEWAY_ALLOWED_IP="${TUSHARE_GATEWAY_ALLOWED_IP:-}"
if [ -z "$EFFECTIVE_TUSHARE_GATEWAY_ALLOWED_IP" ] && [ -n "$OLD_TUSHARE_GATEWAY_ALLOWED_IP" ]; then
  EFFECTIVE_TUSHARE_GATEWAY_ALLOWED_IP="$OLD_TUSHARE_GATEWAY_ALLOWED_IP"
fi
if [ -n "$EFFECTIVE_TUSHARE_GATEWAY_UPSTREAM" ]; then
  set_env_kv "TUSHARE_GATEWAY_UPSTREAM" "$EFFECTIVE_TUSHARE_GATEWAY_UPSTREAM"
  _persist_secrets_env "TUSHARE_GATEWAY_UPSTREAM" "$EFFECTIVE_TUSHARE_GATEWAY_UPSTREAM"
fi
if [ -n "$EFFECTIVE_TUSHARE_GATEWAY_ALLOWED_IP" ]; then
  set_env_kv "TUSHARE_GATEWAY_ALLOWED_IP" "$EFFECTIVE_TUSHARE_GATEWAY_ALLOWED_IP"
  _persist_secrets_env "TUSHARE_GATEWAY_ALLOWED_IP" "$EFFECTIVE_TUSHARE_GATEWAY_ALLOWED_IP"
fi
