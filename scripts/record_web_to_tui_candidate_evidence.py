...
UAT_REPORT_VERSION = "web-to-tui-candidate-uat-report.v1"
...
def validate_rollback_report(report: object, *, binding: CandidateBinding) -> None:
    ...
    transition_counts = {key: 0 for key in ("added", "modified", "deleted", "unchanged")}
...
def record_uat(
    *,
    ...
    validate_uat_report(report, binding=context.binding, routes=context.routes)
...
def _run_uat_suite(args: argparse.Namespace, junit_path: Path) -> list[dict[str, Any]]:
    """Execute the fixed managed live-server UAT suite and parse JUnit."""

    if os.environ.get("AGOM_M5_EXTERNAL_AI_UAT", "").lower() not in TRUE_VALUES:
        raise CandidateEvidenceError(
...
def main() -> int:
    ...
    report_path = (
        args.report.resolve()
        if args.report
        else (DEFAULT_REPORT_DIR / f"web_to_tui_{args.kind}_candidate.v1.json").resolve()
    )
    try:
        ...
        with tempfile.TemporaryDirectory(prefix=f"agom-m5-{args.kind}-") as temp_dir:
            ...
            if args.kind == "uat":
                cases = _run_uat_suite(args, junit_path)
                record_uat(
                    context=context,
                    evidence_path=evidence_path,
