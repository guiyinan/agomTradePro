...
ViewDecorator = Callable[[ViewFunction], ViewFunction]
...
def staff_required(view_func: ViewFunction) -> ViewFunction:
    """
...
def superuser_required(view_func: ViewFunction) -> ViewFunction:
    """
    要求用户为超级用户的装饰器
...
def ajax_required(view_func: ViewFunction) -> ViewFunction:
    """
    ...
    def wrapper(request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponseBase:
        if not request.headers.get("X-Requested-With") and not is_htmx(request):
            return JsonResponse({"success": False, "message": "此接口仅支持 AJAX 请求"}, status=400)
        return view_func(request, *args, **kwargs)

    return wrapper


def htmx_only(view_func: ViewFunction) -> ViewFunction:
    """
    要求请求为 HTMX 的装饰器
    ...
    def wrapper(request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponseBase:
        if not is_htmx(request):
            return JsonResponse({"success": False, "message": "此接口仅支持 HTMX 请求"}, status=400)
        return view_func(request, *args, **kwargs)
...
def conditionally(
    condition_decorator: bool,
) -> Callable[[ViewDecorator], ViewDecorator]:
    """
    条件装饰器工厂
    ...
    def decorator(decorator_func: ViewDecorator) -> ViewDecorator:
        def wrapper(view_func: ViewFunction) -> ViewFunction:
...
def htmx_view(view_func: ViewFunction) -> ViewFunction:
    """
    HTMX 视图装饰器
...
def htmx_trigger(event_name: str) -> ViewDecorator:
    """
    HTMX 事件触发装饰器工厂
...
def htmx_redirect(view_func: ViewFunction) -> ViewFunction:
    """
    HTMX 重定向装饰器
    ...
    def wrapper(request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponseBase:
        ...
        if is_htmx(request) and redirect_url is not None:
...
def success_message(message: str) -> ViewDecorator:
    """
    成功消息装饰器工厂
...
def error_message(message: str) -> ViewDecorator:
    """
    错误消息装饰器工厂
...
def cache_per_user(timeout: int = 300) -> ViewDecorator:
    """
    基于用户的缓存装饰器
    ...
    def decorator(view_func: ViewFunction) -> ViewFunction:
        ...
        def wrapper(request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponseBase:
            ...
            if cached_response is not None:
                return cast(HttpResponseBase, cached_response)
...
def admin_htmx_view(view_func: ViewFunction) -> ViewFunction:
    """
    管理员 HTMX 视图组合装饰器
...
def login_htmx_view(view_func: ViewFunction) -> ViewFunction:
    """
    登录用户 HTMX 视图组合装饰器
