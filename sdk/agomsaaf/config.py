...
def _load_env_config() -> dict:
    ...
    if username := os.getenv("AGOMSAAF_USERNAME"):
        config["username"] = username

    if password := os.getenv("AGOMSAAF_PASSWORD"):
        config["password"] = password

    if timeout := os.getenv("AGOMSAAF_TIMEOUT"):
        try:
            config["timeout"] = int(timeout)
        except ValueError:
            raise ConfigurationError(f"Invalid AGOMSAAF_TIMEOUT: {timeout}")

    if max_retries := os.getenv("AGOMSAAF_MAX_RETRIES"):
        try:
            config["max_retries"] = int(max_retries)
        except ValueError:
            raise ConfigurationError(f"Invalid AGOMSAAF_MAX_RETRIES: {max_retries}")

    if verify_ssl := os.getenv("AGOMSAAF_VERIFY_SSL"):
...
def _load_file_config() -> dict:
    ...
    支持的配置文件（按优先级）：
    1. .agomsaaf.json（当前目录）
    2. ~/.agomsaaf/config.json（用户目录）
    """
    config_files = [
        Path.cwd() / ".agomsaaf.json",
        Path.home() / ".agomsaaf" / "config.json",
    ]
