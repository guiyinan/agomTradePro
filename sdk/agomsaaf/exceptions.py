...
class AgomSAAFAPIError(Exception):
    """
    AgomSAAF API 基础异常类
...
class AuthenticationError(AgomSAAFAPIError):
    """
    认证失败异常 (401/403)
...
class RateLimitError(AgomSAAFAPIError):
    """
    请求频率限制异常 (429)
...
class ValidationError(AgomSAAFAPIError):
    """
    数据验证失败异常 (400)
...
class NotFoundError(AgomSAAFAPIError):
    """
    资源未找到异常 (404)
...
class ConflictError(AgomSAAFAPIError):
    """
    资源冲突异常 (409)
...
class ServerError(AgomSAAFAPIError):
    """
    服务器错误异常 (5xx)
...
class ConnectionError(AgomSAAFAPIError):
    """
    网络连接异常
    ...
    def __init__(
        self,
        message: str = "Failed to connect to AgomSAAF server.",
    ) -> None:
        super().__init__(message)


class TimeoutError(AgomSAAFAPIError):
    """
    请求超时异常
...
class ConfigurationError(AgomSAAFAPIError):
    """
    配置错误异常
