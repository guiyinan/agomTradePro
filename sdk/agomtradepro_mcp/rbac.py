"""RBAC for AgomTradePro MCP server."""
...
def _role() -> str:
    ...
    env_role = os.getenv("AGOMTRADEPRO_MCP_ROLE")
    if env_role and env_role.strip():
        return _normalize_role(env_role)

    source = os.getenv("AGOMTRADEPRO_MCP_ROLE_SOURCE", "backend").strip().lower()
...
def enforce_tool_access(tool_name: str) -> None:
    ...
    allowed = _split_csv_env("AGOMTRADEPRO_MCP_ALLOWED_TOOLS")
    denied = _split_csv_env("AGOMTRADEPRO_MCP_DENIED_TOOLS")
...
def enforce_resource_access(resource_uri: str) -> None:
    ...
    allowed = _split_csv_env("AGOMTRADEPRO_MCP_ALLOWED_RESOURCES")
    denied = _split_csv_env("AGOMTRADEPRO_MCP_DENIED_RESOURCES")
...
def enforce_prompt_access(prompt_name: str) -> None:
    ...
    allowed = _split_csv_env("AGOMTRADEPRO_MCP_ALLOWED_PROMPTS")
    denied = _split_csv_env("AGOMTRADEPRO_MCP_DENIED_PROMPTS")
