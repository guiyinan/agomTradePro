---
name: agomsaaf-agent-operator
description: Use when an AI agent needs to autonomously operate the AgomSAAF system through MCP tools first, then the Python SDK, with docs-backed setup, RBAC-aware execution, read-before-write verification, and direct API fallback only for debugging or unsupported flows.
---

# AgomSAAF Agent Operator

Use this skill when the user wants the agent to operate AgomSAAF directly, not just explain it. This skill is designed around the local MCP server in `sdk/agomsaaf_mcp`, the Python SDK in `sdk/agomsaaf`, and the project docs in `docs/mcp` and `docs/sdk`.
...
5. Respect RBAC and audit constraints. Do not try to bypass `AGOMSAAF_MCP_ENFORCE_RBAC`.
...
- MCP server registration: `sdk/agomsaaf_mcp/server.py`
