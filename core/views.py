...
def terminal_view(request):
    ...
    context = {
        ...
        "regime_confidence": (
            f"{current_regime.confidence * 100:.1f}%" if current_regime.confidence else "N/A"
        ),
...
def decision_workspace_view(request):
    ...
    try:
        ...
        context["valuation_repair_config_summary"] = get_valuation_repair_config_summary(
            use_cache=False
        )
...
def settings_center_view(request):
    ...
    items_by_key = {
        item["key"]: item for section in context["sections"] for item in section["items"]
    }
    context["featured_items"] = [items_by_key[key] for key in featured_order if key in items_by_key]
