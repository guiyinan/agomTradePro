...
DiagnosticStatus = Literal["success", "error", "warning"]
...
class DiagnosticResult(TypedDict):
    """One persisted data-connection diagnostic observation."""
...
class DataConnectionTester:
    ...
    def __init__(
        ...
        *,
        results_path: Path | None = Path("docs/data_test_results.json"),
    ) -> None:
        self.stdout = stdout
    ...
    def log_exception(
        ...
        """Record a redacted exception without persisting credential values."""

        detail = _SENSITIVE_DETAIL_PATTERN.sub(r"\1\2<redacted>", str(exc))
        self.log_result(category, test_name, "error", detail)
    ...
    def test_database_connection(self) -> bool:
        ...
        try:
            ...
            self.log_exception("Database", "数据库连接", exc)
            return False

    def test_account_data(self) -> bool:
        """测试账户数据"""
        self.stdout.write("\n" + "=" * 60)
        ...
        try:
            ...
            self.log_exception("Account", "账户数据测试", exc)
            return False

    def test_macro_data_update(self) -> bool:
        """测试宏观数据更新"""
        self.stdout.write("\n" + "=" * 60)
        ...
        try:
            ...
            self.log_result(
                "Macro", "数据源配置", "success", f"共 {source_count} 个源，{active_sources} 个激活"
            )
            ...
            try:
                ...
                self.log_exception("Macro", "PMI数据更新", exc)

            return True
        except Exception as exc:
            self.log_exception("Macro", "宏观数据测试", exc)
            return False

    def test_regime_calculation(self) -> bool:
        """测试Regime判定"""
        self.stdout.write("\n" + "=" * 60)
        ...
        try:
            ...
            try:
                ...
                self.log_exception("Regime", "Regime计算", exc)

            return True
        except Exception as exc:
            self.log_exception("Regime", "Regime测试", exc)
            return False

    def test_policy_events(self) -> bool:
        """测试政策事件"""
        self.stdout.write("\n" + "=" * 60)
        ...
        try:
            ...
            self.log_result(
                "Policy", "RSS源配置", "success", f"共 {rss_count} 个源，{active_rss} 个激活"
            )

            return True
        except Exception as exc:
            self.log_exception("Policy", "政策测试", exc)
            return False

    def test_investment_signals(self) -> bool:
        """测试投资信号"""
        self.stdout.write("\n" + "=" * 60)
        ...
        try:
            ...
            if regime_matched > 0:
                ...
                self.log_result("Signal", "Regime匹配", "warning", "暂无与当前Regime匹配的信号")

            return True
        except Exception as exc:
            self.log_exception("Signal", "信号测试", exc)
            return False
    ...
    def test_dashboard_data(self) -> bool:
        ...
        try:
            ...
            self.log_result(
                ...
                "用户基本信息",
                "success",
                f"用户: {data.username or user['username']}, " f"显示名: {data.display_name}",
            )
            ...
            self.log_result(
                ...
                "持仓信息",
                "success",
                f"持仓数: {data.position_count}, " f"Regime匹配度: {data.regime_match_score:.2f}",
            )
            ...
            if data.current_regime:
                self.log_result(
                    ...
                    "宏观环境",
                    "success",
                    f"当前Regime: {data.current_regime}, " f"政策档位: {data.current_policy_level}",
                )

            return True
        except Exception as exc:
            self.log_exception("Dashboard", "仪表板测试", exc)
            return False
    ...
    def test_data_consistency(self) -> bool:
        ...
        try:
            ...
            missing_metadata = count_missing_asset_metadata(list_signal_diagnostic_asset_codes())
            ...
            if latest_regime_observed_at:
                ...
                if macro_exists:
                    ...
                    self.log_result(
                        "Consistency", "Regime-宏观数据", "warning", "Regime日期之前无宏观数据"
                    )

            return True
        except Exception as exc:
            self.log_exception("Consistency", "一致性测试", exc)
            return False
    ...
    def run_all_tests(self) -> bool:
        ...
        self.stdout.write("=" * 60)
        self.stdout.write(f"测试时间: {datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        for name, test_func in tests:
            ...
            try:
                declared_success = test_func()
                new_results = self.results[result_start:]
                if not new_results:
                    self.log_result(
                        "Runner",
                        name,
                    ...
                    failed += 1
                    continue
                has_error = any(result["status"] == "error" for result in new_results)
                if declared_success and not has_error:
                    passed += 1
                else:
                    failed += 1
            except Exception as exc:
                failed += 1
                self.log_exception("Runner", name, exc)
    ...
    def save_results(self) -> None:
        ...
        temporary_path = self.results_path.with_suffix(f"{self.results_path.suffix}.tmp")
        temporary_path.write_text(
            json.dumps(summary, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        temporary_path.replace(self.results_path)
        self.stdout.write(f"\n📄 测试结果已保存到: {self.results_path}")
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        """Register bounded result-output controls."""

        parser.add_argument(
            "--output",
            type=Path,
            default=Path("docs/data_test_results.json"),
            help="JSON evidence output path.",
        )
        parser.add_argument(
            "--no-write",
            action="store_true",
            help="Run diagnostics without writing a JSON evidence file.",
        )

    def handle(self, *args: Any, **options: Any) -> None:
        """Run all diagnostics and return a failing process status on errors."""

        output_path = None if options["no_write"] else options["output"]
        if output_path is not None and not isinstance(output_path, Path):
            raise CommandError("output must be a filesystem path")
        tester = DataConnectionTester(self.stdout, results_path=output_path)
        if not tester.run_all_tests():
            raise CommandError("One or more data connection diagnostics failed")
