...
AgomTradePro 是一个**决策辅助系统**，不是自动交易系统。系统负责分析宏观环境并给出投资建议，用户根据建议**手动执行交易**。
...
**A**: 不会。AgomTradePro 是决策辅助系统，只提供建议。您需要：
1. 在系统中查看建议
2. 在券商 APP 手动执行交易
3. 在系统中记录持仓
...
'''
        ...
        for doc_data in initial_docs:
            ...
            if existing:
                ...
                # 创建新文档
                doc = DocumentationModel._default_manager.create(**doc_data)
                created_count += 1
