...
class Command(BaseCommand):
    ...
    def handle(self, *args: str, **options: Any) -> None:
        ...
        self.stdout.write(
            f"  - Top-level categories: {AssetCategoryModel._default_manager.filter(level=1).count()}"
        )
        self.stdout.write(
            f"  - Sub-categories: {AssetCategoryModel._default_manager.filter(level=2).count()}"
        )
