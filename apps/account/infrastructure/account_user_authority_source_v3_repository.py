"""Closed-world Django repository for Account user-authority raw source v3."""

from __future__ import annotations

import hashlib
import json
from collections.abc import Iterator
from contextlib import contextmanager
from dataclasses import dataclass
from datetime import UTC, datetime
from typing import Protocol, TypeVar

from django.db import IntegrityError, transaction
from django.utils import timezone

from apps.account.application.account_actor_authority_raw_source_primitives_v3 import (
    AccountActorAuthorityRawSourceV3Conflict,
    AccountActorAuthorityRawSourceV3Corruption,
    AccountActorAuthorityRawSourceV3Recorder,
    AccountActorAuthorityRawSourceV3Unavailable,
)
from apps.account.application.account_user_authority_source_v3 import (
    PersistedAccountUserAuthoritySourceV3,
)
from apps.account.domain.account_user_authority_source_v3 import (
    root_claim_hash_for_account_user_authority_source_v3,
    validate_account_user_authority_source_v3_successor,
)
from apps.account.infrastructure.account_actor_authority_raw_source_models_v3 import (
    AccountUserAuthoritySourceV3AnchorModel,
    AccountUserAuthoritySourceV3Model,
    _activate_account_actor_authority_raw_source_v3_uow,
    _claim_account_actor_authority_raw_source_v3_insert,
)
from apps.account.infrastructure.account_user_authority_source_v3_codec import (
    AccountUserAuthoritySourceV3CodecError,
    decode_account_user_authority_source_v3,
    encode_account_user_authority_source_v3,
)


class AccountUserAuthoritySourceV3Clock(Protocol):
    """Provide the repository decision clock."""

    def now(self) -> datetime:
        """Return one aware server time."""

        ...


class DjangoAccountUserAuthoritySourceV3Clock:
    """Use Django's timezone-aware clock."""

    def now(self) -> datetime:
        """Return the current aware server time."""

        return timezone.now()


@dataclass(frozen=True, slots=True)
class _World:
    anchors: tuple[AccountUserAuthoritySourceV3AnchorModel, ...]
    records: tuple[
        tuple[AccountUserAuthoritySourceV3Model, PersistedAccountUserAuthoritySourceV3], ...
    ]


_ValueT = TypeVar("_ValueT")


class DjangoAccountUserAuthoritySourceV3Repository:
    """Persist one guarded, append-only, single-root chain per user source id."""

    def __init__(
        self, *, clock: AccountUserAuthoritySourceV3Clock | None = None, using: str = "default"
    ) -> None:
        if type(using) is not str or not using or using.strip() != using:
            raise ValueError("using must be an exact database alias")
        self._clock = clock or DjangoAccountUserAuthoritySourceV3Clock()
        self._using = using
        self._uow: object | None = None

    @contextmanager
    def atomic(self) -> Iterator[None]:
        """Open the private, non-nestable write unit of work on the configured alias."""

        if self._uow is not None:
            raise AccountActorAuthorityRawSourceV3Conflict("nested user-authority UOW")
        token = object()
        self._uow = token
        try:
            with (
                transaction.atomic(using=self._using),
                _activate_account_actor_authority_raw_source_v3_uow(token),
            ):
                yield
        finally:
            self._uow = None

    def now(self) -> datetime:
        """Return the checked repository clock."""

        value = self._clock.now()
        if not _is_aware(value):
            raise AccountActorAuthorityRawSourceV3Corruption("repository clock is naive")
        return value

    def get_winner(
        self, *, source_id: str, source_version: str, as_of: datetime
    ) -> PersistedAccountUserAuthoritySourceV3 | None:
        """Return the immutable first winner knowable at the cutoff."""

        self._cutoff(as_of)
        matches = tuple(
            record
            for _, record in self._closed_world(lock=False).records
            if (record.source.identity.source_id, record.source.identity.source_version)
            == (source_id, source_version)
            and record.source.clock.recorded_at <= as_of
        )
        return _single(matches, "user-authority identity")

    def get_current_head(
        self, *, source_id: str, as_of: datetime
    ) -> PersistedAccountUserAuthoritySourceV3 | None:
        """Return the final knowable head, including expired or terminal heads."""

        self._cutoff(as_of)
        matches = tuple(
            record
            for _, record in self._closed_world(lock=False).records
            if record.source.identity.source_id == source_id
            and record.source.clock.recorded_at <= as_of
        )
        return max(matches, key=lambda item: item.source.clock.recorded_at) if matches else None

    def get_exact_by_hash(
        self,
        *,
        source_id: str,
        source_version: str,
        expected_content_hash: str,
        as_of: datetime,
    ) -> PersistedAccountUserAuthoritySourceV3 | None:
        """Return one exact permanent version after its knowledge cutoff."""

        self._cutoff(as_of)
        anchors = tuple(
            record
            for _, record in self._closed_world(lock=False).records
            if (record.source.identity.source_id, record.source.identity.source_version)
            == (source_id, source_version)
            or record.source.content_hash == expected_content_hash
        )
        matches = tuple(
            record
            for record in anchors
            if (
                record.source.identity.source_id,
                record.source.identity.source_version,
                record.source.content_hash,
            )
            == (source_id, source_version, expected_content_hash)
            and record.source.clock.recorded_at <= as_of
        )
        if anchors and (len(anchors) != 1 or len(matches) > 1):
            raise AccountActorAuthorityRawSourceV3Corruption(
                "user-authority exact anchors disagree"
            )
        return matches[0] if matches else None

    def append(
        self,
        record: PersistedAccountUserAuthoritySourceV3,
        *,
        expected_predecessor_hash: str | None,
        recorded_at: datetime,
    ) -> PersistedAccountUserAuthoritySourceV3:
        """Append inside a whole-operation savepoint so failures cannot strand anchors."""

        with transaction.atomic(using=self._using):
            return self._append(
                record,
                expected_predecessor_hash=expected_predecessor_hash,
                recorded_at=recorded_at,
            )

    def _append(
        self,
        record: PersistedAccountUserAuthoritySourceV3,
        *,
        expected_predecessor_hash: str | None,
        recorded_at: datetime,
    ) -> PersistedAccountUserAuthoritySourceV3:
        checked = _record(record)
        token = self._require_uow()
        source = checked.source
        if recorded_at != source.clock.recorded_at:
            raise AccountActorAuthorityRawSourceV3Conflict("persisted_at differs")
        root_hash = root_claim_hash_for_account_user_authority_source_v3(
            source_id=source.identity.source_id,
            user_id=source.user_id,
            actor_id=source.actor_id,
        )
        anchor = self._lock_anchor(
            source_id=source.identity.source_id,
            root_claim_hash=root_hash,
            created_at=recorded_at,
            token=token,
        )
        world = self._closed_world(lock=True, permitted_empty_anchor=_pk(anchor))
        collisions = tuple(
            existing
            for _, existing in world.records
            if (
                existing.source.identity.source_id,
                existing.source.identity.source_version,
            )
            == (source.identity.source_id, source.identity.source_version)
            or existing.source.identity_hash == source.identity_hash
            or existing.source.record_seal == source.record_seal
            or existing.source.content_hash == source.content_hash
        )
        if collisions:
            if len(collisions) == 1 and collisions[0] == checked:
                return collisions[0]
            raise AccountActorAuthorityRawSourceV3Conflict("user-authority winner differs")
        chain = tuple(
            item
            for item in world.records
            if item[1].source.identity.source_id == source.identity.source_id
        )
        previous_row: AccountUserAuthoritySourceV3Model | None = None
        previous: PersistedAccountUserAuthoritySourceV3 | None = None
        if chain:
            previous_row, previous = max(chain, key=lambda item: item[1].source.clock.recorded_at)
        actual_hash = previous.source.content_hash if previous is not None else None
        if expected_predecessor_hash != actual_hash:
            raise AccountActorAuthorityRawSourceV3Conflict("predecessor CAS differs")
        try:
            if previous is None:
                if source.chain.root_claim_hash != root_hash:
                    raise ValueError("root differs from anchor")
            else:
                validate_account_user_authority_source_v3_successor(previous.source, source)
            values = _values(
                checked, anchor_pk=_pk(anchor), predecessor_pk=_optional_pk(previous_row)
            )
            with transaction.atomic(using=self._using):
                self._insert(AccountUserAuthoritySourceV3Model, values, token)
        except IntegrityError as error:
            exact = tuple(
                existing
                for _, existing in self._closed_world(
                    lock=True, permitted_empty_anchor=_pk(anchor)
                ).records
                if existing == checked
            )
            if len(exact) == 1:
                return exact[0]
            raise AccountActorAuthorityRawSourceV3Conflict(
                "concurrent user-authority winner differs"
            ) from error
        except (TypeError, ValueError) as error:
            raise AccountActorAuthorityRawSourceV3Conflict(
                "invalid user-authority chain append"
            ) from error
        restored = self.get_exact_by_hash(
            source_id=source.identity.source_id,
            source_version=source.identity.source_version,
            expected_content_hash=source.content_hash,
            as_of=recorded_at,
        )
        if restored != checked:
            raise AccountActorAuthorityRawSourceV3Corruption("restore mismatch")
        return restored

    def _lock_anchor(
        self,
        *,
        source_id: str,
        root_claim_hash: str,
        created_at: datetime,
        token: object,
    ) -> AccountUserAuthoritySourceV3AnchorModel:
        query = AccountUserAuthoritySourceV3AnchorModel._base_manager.using(self._using)
        if query.filter(source_id=source_id).first() is None:
            values: dict[str, object] = {
                "source_id": source_id,
                "root_claim_hash": root_claim_hash,
                "created_at": created_at,
            }
            try:
                with transaction.atomic(using=self._using):
                    self._insert(AccountUserAuthoritySourceV3AnchorModel, values, token)
            except IntegrityError:
                pass
        anchor = query.select_for_update().filter(source_id=source_id).first()
        if anchor is None:
            raise AccountActorAuthorityRawSourceV3Conflict("anchor unavailable")
        if anchor.root_claim_hash != root_claim_hash:
            raise AccountActorAuthorityRawSourceV3Conflict("anchor root claim differs")
        return anchor

    def _closed_world(self, *, lock: bool, permitted_empty_anchor: int | None = None) -> _World:
        anchors_query = AccountUserAuthoritySourceV3AnchorModel._base_manager.using(
            self._using
        ).all()
        rows_query = (
            AccountUserAuthoritySourceV3Model._base_manager.using(self._using)
            .select_related("anchor", "predecessor")
            .all()
        )
        if lock:
            anchors_query = anchors_query.select_for_update()
            rows_query = rows_query.select_for_update()
        anchors = tuple(anchors_query.order_by("pk"))
        anchor_ids = {_pk(anchor): anchor for anchor in anchors}
        records: list[
            tuple[AccountUserAuthoritySourceV3Model, PersistedAccountUserAuthoritySourceV3]
        ] = []
        for row in rows_query.order_by("pk"):
            anchor = anchor_ids.get(row.anchor_id)
            if anchor is None:
                raise AccountActorAuthorityRawSourceV3Corruption("ledger has no anchor")
            record = _restore(row)
            if record.source.identity.source_id != anchor.source_id:
                raise AccountActorAuthorityRawSourceV3Corruption("anchor source differs")
            records.append((row, record))
        self._validate_graph(anchors, tuple(records), permitted_empty_anchor)
        return _World(anchors, tuple(records))

    def _validate_graph(
        self,
        anchors: tuple[AccountUserAuthoritySourceV3AnchorModel, ...],
        records: tuple[
            tuple[AccountUserAuthoritySourceV3Model, PersistedAccountUserAuthoritySourceV3], ...
        ],
        permitted_empty_anchor: int | None,
    ) -> None:
        for anchor in anchors:
            chain = tuple(item for item in records if item[0].anchor_id == _pk(anchor))
            if not chain:
                if permitted_empty_anchor != _pk(anchor):
                    raise AccountActorAuthorityRawSourceV3Corruption("orphan anchor")
                continue
            roots = tuple(item for item in chain if item[0].predecessor_id is None)
            if len(roots) != 1:
                raise AccountActorAuthorityRawSourceV3Corruption("chain must have one root")
            root_row, root = roots[0]
            if (
                root.source.chain.root_claim_hash != anchor.root_claim_hash
                or root.source.clock.recorded_at != anchor.created_at
            ):
                raise AccountActorAuthorityRawSourceV3Corruption("anchor binding differs")
            successors: dict[
                int,
                list[
                    tuple[AccountUserAuthoritySourceV3Model, PersistedAccountUserAuthoritySourceV3]
                ],
            ] = {}
            for item in chain:
                if item[0].predecessor_id is not None:
                    successors.setdefault(item[0].predecessor_id, []).append(item)
            visited = {_pk(root_row)}
            current_row, current = root_row, root
            while following := successors.get(_pk(current_row), []):
                if len(following) != 1:
                    raise AccountActorAuthorityRawSourceV3Corruption("chain forks")
                next_row, next_record = following[0]
                if _pk(next_row) in visited:
                    raise AccountActorAuthorityRawSourceV3Corruption("chain cycles")
                try:
                    validate_account_user_authority_source_v3_successor(
                        current.source, next_record.source
                    )
                except (TypeError, ValueError) as error:
                    raise AccountActorAuthorityRawSourceV3Corruption(
                        "successor is invalid"
                    ) from error
                visited.add(_pk(next_row))
                current_row, current = next_row, next_record
            if len(visited) != len(chain):
                raise AccountActorAuthorityRawSourceV3Corruption("chain has orphan or cycle")

    def _insert(
        self,
        model_type: (
            type[AccountUserAuthoritySourceV3AnchorModel] | type[AccountUserAuthoritySourceV3Model]
        ),
        values: dict[str, object],
        token: object,
    ) -> None:
        with _claim_account_actor_authority_raw_source_v3_insert(
            token=token, model_type=model_type, expected_values=values
        ):
            model_type._default_manager.using(self._using).create(**values)

    def _require_uow(self) -> object:
        if self._uow is None:
            raise AccountActorAuthorityRawSourceV3Conflict("append requires private UOW")
        return self._uow

    def _cutoff(self, as_of: datetime) -> None:
        if not _is_aware(as_of):
            raise AccountActorAuthorityRawSourceV3Unavailable("as_of must be timezone-aware")
        if as_of > self.now():
            raise AccountActorAuthorityRawSourceV3Unavailable("future as_of is forbidden")


def _values(
    record: PersistedAccountUserAuthoritySourceV3,
    *,
    anchor_pk: int,
    predecessor_pk: int | None,
) -> dict[str, object]:
    source, recorder = record.source, record.recorded_by
    payload = encode_account_user_authority_source_v3(source)
    values: dict[str, object] = {
        "anchor_id": anchor_pk,
        "predecessor_id": predecessor_pk,
        "owner": source.owner,
        "artifact_type": source.artifact_type,
        "schema": source.schema,
        "permission": source.permission,
        "status": source.status,
        "must_not_execute": source.must_not_execute,
        "execution_allowed": source.execution_allowed,
        "source_id": source.identity.source_id,
        "source_version": source.identity.source_version,
        "observed_at": source.clock.observed_at,
        "recorded_at": source.clock.recorded_at,
        "valid_until": source.clock.valid_until,
        "root_claim_hash": source.chain.root_claim_hash,
        "supersedes_content_hash": source.chain.supersedes_content_hash,
        "user_id": source.user_id,
        "actor_id": source.actor_id,
        "is_active": source.is_active,
        "is_staff": source.is_staff,
        "is_superuser": source.is_superuser,
        "authority_state": source.authority_state,
        "identity_hash": source.identity_hash,
        "user_seal": source.user_seal,
        "facts_seal": source.facts_seal,
        "clock_seal": source.clock_seal,
        "chain_seal": source.chain_seal,
        "fixed_authority_seal": source.fixed_authority_seal,
        "record_seal": source.record_seal,
        "content_hash": source.content_hash,
        "recorded_by_service_id": recorder.service_id,
        "recorded_by_role": recorder.role,
        "recorded_by_kind": recorder.kind,
        "recorded_by_is_automated": recorder.is_automated,
        "canonical_payload": payload,
        "persisted_at": source.clock.recorded_at,
    }
    recorder_payload = {
        "service_id": recorder.service_id,
        "role": recorder.role,
        "kind": recorder.kind,
        "is_automated": recorder.is_automated,
    }
    values["recorder_binding_seal"] = _hash(
        {"content_hash": source.content_hash, "recorder": recorder_payload}
    )
    values["ledger_seal"] = _hash(
        {
            "anchor_pk": anchor_pk,
            "content_hash": source.content_hash,
            "predecessor_pk": predecessor_pk,
            "record_seal": source.record_seal,
            "recorder_binding_seal": values["recorder_binding_seal"],
            "persisted_at": _time(source.clock.recorded_at),
        }
    )
    return values


def _restore(row: AccountUserAuthoritySourceV3Model) -> PersistedAccountUserAuthoritySourceV3:
    try:
        source = decode_account_user_authority_source_v3(row.canonical_payload)
        recorder = AccountActorAuthorityRawSourceV3Recorder(
            service_id=row.recorded_by_service_id,
            role=row.recorded_by_role,
            kind=row.recorded_by_kind,
            is_automated=row.recorded_by_is_automated,
        )
        record = PersistedAccountUserAuthoritySourceV3(source, recorder)
        expected = _values(record, anchor_pk=row.anchor_id, predecessor_pk=row.predecessor_id)
    except (AccountUserAuthoritySourceV3CodecError, TypeError, ValueError) as error:
        raise AccountActorAuthorityRawSourceV3Corruption("ledger payload is corrupt") from error
    for name, value in expected.items():
        if getattr(row, name) != value:
            raise AccountActorAuthorityRawSourceV3Corruption(
                f"user-authority ledger mismatch: {name}"
            )
    return record


def _record(value: object) -> PersistedAccountUserAuthoritySourceV3:
    if type(value) is not PersistedAccountUserAuthoritySourceV3:
        raise AccountActorAuthorityRawSourceV3Corruption("record type substitution")
    try:
        value.__post_init__()
    except (TypeError, ValueError) as error:
        raise AccountActorAuthorityRawSourceV3Corruption("record is corrupt") from error
    return value


def _hash(value: object) -> str:
    return hashlib.sha256(
        json.dumps(
            value,
            allow_nan=False,
            ensure_ascii=False,
            separators=(",", ":"),
            sort_keys=True,
        ).encode("utf-8")
    ).hexdigest()


def _time(value: datetime) -> str:
    return value.astimezone(UTC).isoformat(timespec="microseconds").replace("+00:00", "Z")


def _single(values: tuple[_ValueT, ...], label: str) -> _ValueT | None:
    if len(values) > 1:
        raise AccountActorAuthorityRawSourceV3Corruption(f"{label} is ambiguous")
    return values[0] if values else None


def _pk(value: AccountUserAuthoritySourceV3AnchorModel | AccountUserAuthoritySourceV3Model) -> int:
    if value.pk is None:
        raise AccountActorAuthorityRawSourceV3Corruption("row has no database identity")
    return int(value.pk)


def _optional_pk(value: AccountUserAuthoritySourceV3Model | None) -> int | None:
    return None if value is None else _pk(value)


def _is_aware(value: object) -> bool:
    return type(value) is datetime and value.tzinfo is not None and value.utcoffset() is not None


__all__ = [
    "AccountUserAuthoritySourceV3Clock",
    "DjangoAccountUserAuthoritySourceV3Clock",
    "DjangoAccountUserAuthoritySourceV3Repository",
]
