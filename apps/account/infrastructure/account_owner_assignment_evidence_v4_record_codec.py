"""Canonical authenticated record codec for Account owner evidence v4."""
...
class AccountOwnerAssignmentEvidenceV4RecordCodecError(ValueError):
    """A persisted evidence record is malformed or not canonical v4."""
...
def _mapping(value: object, expected: set[str], name: str) -> dict[str, object]:
    ...
    if type(value) is not dict:
        raise AccountOwnerAssignmentEvidenceV4RecordCodecError(f"{name} must be an exact mapping")
    data = cast(dict[str, object], value)
    if any(type(key) is not str for key in data) or set(data) != expected:
        raise AccountOwnerAssignmentEvidenceV4RecordCodecError(
...
def _text(value: object, name: str) -> str:
    ...
    if type(value) is not str:
        raise AccountOwnerAssignmentEvidenceV4RecordCodecError(f"{name} must be an exact string")
    return value
...
def _integer(value: object, name: str) -> int:
    ...
    if type(value) is not int:
        raise AccountOwnerAssignmentEvidenceV4RecordCodecError(f"{name} must be an exact integer")
    return value
...
def _boolean(value: object, name: str) -> bool:
    ...
    if type(value) is not bool:
        raise AccountOwnerAssignmentEvidenceV4RecordCodecError(f"{name} must be an exact boolean")
    return value
...
def _clock(value: object, name: str) -> datetime:
    ...
    try:
        ...
        raise AccountOwnerAssignmentEvidenceV4RecordCodecError(f"{name} is invalid") from error
...
def encode_account_owner_assignment_evidence_v4_record(
    ...
    if type(value) is not PersistedAccountOwnerAssignmentEvidenceV4:
        raise AccountOwnerAssignmentEvidenceV4RecordCodecError(
            "expected exact PersistedAccountOwnerAssignmentEvidenceV4"
        )
    try:
        value.__post_init__()
        return {
            "evidence": encode_account_owner_assignment_evidence_v4(value.evidence),
            "authority": _authority_payload(value.authority),
        }
    except (AttributeError, TypeError, ValueError) as error:
        raise AccountOwnerAssignmentEvidenceV4RecordCodecError(
            "evidence v4 record cannot be encoded"
        ) from error


def decode_account_owner_assignment_evidence_v4_record(
    payload: object,
) -> PersistedAccountOwnerAssignmentEvidenceV4:
    """Decode a closed-world v4 record and require exact canonical roundtrip."""

    try:
        data = _mapping(payload, {"evidence", "authority"}, "record")
        result = PersistedAccountOwnerAssignmentEvidenceV4(
            evidence=decode_account_owner_assignment_evidence_v4(data["evidence"]),
            authority=_decode_authority(data["authority"]),
        )
        if encode_account_owner_assignment_evidence_v4_record(result) != data:
            raise AccountOwnerAssignmentEvidenceV4RecordCodecError(
                "evidence v4 record is not canonical"
            )
        return result
    except AccountOwnerAssignmentEvidenceV4RecordCodecError:
        raise
    except (AttributeError, KeyError, TypeError, ValueError) as error:
        raise AccountOwnerAssignmentEvidenceV4RecordCodecError(
            "invalid authenticated evidence v4 record"
        ) from error
