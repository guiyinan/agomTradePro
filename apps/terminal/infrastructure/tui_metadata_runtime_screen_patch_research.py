...
RUNTIME_SCREEN_PATCHES_RESEARCH: dict[str, dict[str, Any]] = {'research.asset-lab': {'dashboard_panels': [{'key': 'asset-pool-summary',
                                             ...
                                             {'key': 'asset-weight-configs',
                                              ...
                                              'target_screen': 'research.asset-lab'}]},
 'research.alpha-triggers': {'default_action_key': 'auto.api.get.api.alpha-triggers.candidates.actionable',
                             'business_context': {'objective': '先看可操作候选，再核对活跃触发器和观察列表，判断今天应该推进哪些研究动作。',
                                                  'decision_output': 'Alpha '
                                                                     '触发结论：立即跟进、继续观察、等待触发或复核绩效。',
                                                  'checkpoints': ['先看可操作候选，确认今天最值得推进的标的。',
                                                                  '再看活跃触发器和观察列表，确认触发背景是否延续。',
                                                                  '需要复核历史效果时再看触发器绩效。']},
                             'dashboard_panels': [{'key': 'alpha-triggers-actionable',
                                                   'title': '一、可操作候选',
                                                   'kind': 'datagrid',
...
RUNTIME_REDUNDANT_SCREEN_ACTION_KEYS_RESEARCH: dict[str, set[str]] = {'research.alpha-triggers': {'auto.api.get.api.alpha-triggers'}}
