...
def _command_to_payload(command) -> dict:
    ...
    payload["prompt_template"] = (
        int(prompt_template) if str(prompt_template).isdigit() else prompt_template
    )
...
class TerminalCommandViewSet(viewsets.ViewSet):
    ...
    def _update_command(self, request, pk=None, *, partial: bool):
        ...
        request_obj = UpdateCommandRequest(
            ...
            prompt_template_id=(
                _normalize_prompt_template_id(data.get("prompt_template"))
            ...
            response_jq_filter=(
                (data.get("response_jq_filter") or None) if "response_jq_filter" in data else None
            ),
            parameters=data.get("parameters"),
    ...
    def execute(self, request, pk=None):
        ...
        serializer = ExecuteCommandSerializer(data={"name": command.name, **request.data})
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
