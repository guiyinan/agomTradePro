...
from .queued_runtime_views import (
...
urlpatterns = [
    ...
    # TAR-02 durable admission/worker boundary.  The view remains fail-closed
    # when either explicit production queue flag is false.
    path("runs/", TerminalQueuedRunView.as_view(), name="terminal-run-create"),
    path("runs/queue/", TerminalQueuedRunView.as_view(), name="terminal-run-queue"),
    path(
        "runs/<str:run_id>/events/",
        TerminalQueuedRunEventsView.as_view(),
        name="terminal-run-events",
    ),
    path(
        "runs/<str:run_id>/cancel/",
        TerminalQueuedRunCancelView.as_view(),
        name="terminal-run-cancel",
    ),
    path(
        "runs/<str:run_id>/",
        TerminalQueuedRunView.as_view(),
