...
def _is_authenticated(actor: Any) -> bool:
    return bool(actor is not None and getattr(actor, "is_authenticated", False))


def ensure_can_view_qlib_center(actor: Any) -> None:
...
def ensure_can_manage_qlib_runtime(actor: Any) -> None:
...
def ensure_can_manage_qlib_training_profiles(actor: Any) -> None:
...
def ensure_can_trigger_qlib_training(actor: Any) -> None:
