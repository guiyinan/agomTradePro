...
class DecisionReplayBacktestUseCase:
    ...
    def __init__(self, *, trade_repo=None, backtest_repo=None, recommendation_repo=None) -> None:
    ...
    def _simulate(
        ...
        *,
        ...
        equity_curve: list[dict[str, Any]] = [{"date": start_date.isoformat(), "value": float(cash)}]
        warnings: list[str] = [
            "Decision replay uses imported execution prices only; missing market history is not inferred."
        ]
        ...
        for tx in transactions:
            ...
            last_price[tx.asset_code] = price

            if action == "buy":
                cash -= notional + Decimal(str(tx.commission or 0))
                positions[tx.asset_code] = positions.get(tx.asset_code, Decimal("0")) + shares
            elif action == "sell":
                sell_shares = min(shares, positions.get(tx.asset_code, Decimal("0")))
                cash += sell_shares * price - Decimal(str(tx.commission or 0))
                positions[tx.asset_code] = positions.get(tx.asset_code, Decimal("0")) - sell_shares
            trades.append(
                {
                    "trade_date": trade_date.isoformat(),
                    "asset_class": tx.asset_code,
                    "action": action,
                    "shares": float(shares),
                    "price": float(price),
                    "notional": float(notional),
                    "cost": float(tx.commission or 0),
        ...
        equity_curve.append({"date": end_date.isoformat(), "value": float(final_capital)})
        return {
