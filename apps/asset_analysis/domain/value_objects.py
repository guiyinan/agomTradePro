    ...
    def __post_init__(self) -> None:
        ...
        for name, value in [
            ...
            if not math.isfinite(value) or value < 0:
                raise ValueError(f"{name} 必须为非负数且为有限值，当前为 {value}")

        # 2. 再验证权重总和为 1.0（容差 0.01）
        total = self.regime_weight + self.policy_weight + self.sentiment_weight + self.signal_weight
...
class ScoreContext:
    ...
    active_signals: Sequence[object]  # 激活的投资信号列表
    ...
    def __post_init__(self) -> None:
        ...
        if not -3.0 <= self.sentiment_index <= 3.0:
            raise ValueError(
                f"sentiment_index 必须在 -3.0 到 +3.0 之间，当前为 {self.sentiment_index}"
            )
        ...
        if self.current_regime not in valid_regimes:
            raise ValueError(
                f"current_regime 必须是 {valid_regimes} 之一，当前为 {self.current_regime}"
            )
        ...
        if self.policy_level not in valid_policies:
            raise ValueError(
                f"policy_level 必须是 {valid_policies} 之一，当前为 {self.policy_level}"
            )
