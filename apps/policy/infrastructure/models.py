...
class PolicyLog(models.Model):
    ...
    _previous_policy_level: str | None
    ...
    ai_confidence = models.FloatField(
        null=True, blank=True, verbose_name="AI置信度", help_text="0.0-1.0，高于阈值自动通过"
    )
    ...
    rss_item_guid = models.CharField(
        max_length=500, blank=True, db_index=True, verbose_name="RSS条目GUID"
    )
    ...
    risk_impact = models.CharField(
        max_length=20, choices=RISK_IMPACT_CHOICES, default="unknown", verbose_name="风险影响"
    )
    is_blacklist = models.BooleanField(
        default=False, verbose_name="黑名单标记", help_text="标记为黑名单的政策"
    )
    is_whitelist = models.BooleanField(
        default=False, verbose_name="白名单标记", help_text="标记为白名单的政策"
    )
    ...
    gate_effective = models.BooleanField(
        default=False, db_index=True, verbose_name="闸门已生效", help_text="是否已审核通过并生效"
    )
    ...
    rollback_reason = models.TextField(
        blank=True, verbose_name="回滚原因", help_text="回滚生效时填写的原因"
    )
...
class RSSHubGlobalConfig(models.Model):
    ...
    enabled = models.BooleanField(
        default=False, verbose_name="启用 RSSHub", help_text="是否启用 RSSHub 全局配置"
    )
    ...
    @classmethod
    def get_config(cls) -> RSSHubGlobalConfig:
        """
        获取全局 RSSHub 配置（单例模式）
        ...
        """
        config, _created = cls._default_manager.get_or_create(
            singleton_id=1,
            defaults={
    ...
    def get_full_url(self, route_path: str, format: str | None = None) -> str:
        """
        构建完整的 RSSHub URL
...
class RSSSourceConfigModel(models.Model):
    ...
    category = models.CharField(
        max_length=20, choices=CATEGORY_CHOICES, default="other", verbose_name="分类"
    )
    ...
    extract_content = models.BooleanField(
        default=False, verbose_name="提取完整内容", help_text="是否从RSS链接中提取文章正文内容"
    )
    ...
    proxy_type = models.CharField(
        max_length=10, choices=PROXY_TYPE_CHOICES, default="http", verbose_name="代理类型"
    )

    # RSS解析器配置
    parser_type = models.CharField(
        max_length=20, choices=PARSER_TYPE_CHOICES, default="feedparser", verbose_name="解析器类型"
    )
...
class PolicyAuditQueue(models.Model):
    ...
    policy_log = models.OneToOneField(
        PolicyLog, on_delete=models.CASCADE, related_name="audit_queue", verbose_name="关联政策日志"
    )
    ...
    due_date = models.DateTimeField(null=True, blank=True, verbose_name="期望完成时间")
    auto_rejection_reason = models.TextField(blank=True, verbose_name="自动拒绝原因")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
...
@receiver(pre_save, sender=PolicyLog)
def capture_previous_policy_level(
    sender: type[PolicyLog],
    instance: PolicyLog,
    **kwargs: Any,
) -> None:
    """Capture the persisted level so post-save can detect a real transition."""

    del sender, kwargs
    previous_level: str | None = None
    if instance.pk is not None:
        previous_level = (
            PolicyLog._default_manager.filter(pk=instance.pk)
            .values_list("level", flat=True)
            .first()
        )
    instance._previous_policy_level = previous_level


@receiver(post_save, sender=PolicyLog)
def on_policy_level_change(
    sender: type[PolicyLog],
    instance: PolicyLog,
    created: bool,
    **kwargs: Any,
    ...
    try:
        previous_level = getattr(instance, "_previous_policy_level", None)
        if previous_level is None or previous_level == instance.level:
            return
        if instance.level not in {"P0", "P1", "P2", "P3"}:
            logger.warning(
                "Skipping policy reevaluation for unsupported level: level=%s",
                instance.level,
            )
            return
        ...
        logger.info(
            "Policy level changed from %s to %s; scheduling signal reevaluation",
            previous_level,
            instance.level,
        )
        transaction.on_commit(
            lambda: trigger_signal_reevaluation.delay(
                new_level=new_level,
        ...
        logger.error(
            "Failed to schedule policy signal reevaluation: error_type=%s",
            type(exc).__name__,
            exc_info=True,
        )
...
class HedgePositionModel(models.Model):
    ...
    execution_price = models.DecimalField(
        max_digits=20, decimal_places=4, null=True, blank=True, verbose_name="执行价格"
    )
    ...
    opening_cost = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="开仓成本"
    )
    closing_cost = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="平仓成本"
    )
    total_cost = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="总成本"
    )

    # 效果评估
    beta_before = models.FloatField(null=True, blank=True, verbose_name="对冲前Beta")
    beta_after = models.FloatField(null=True, blank=True, verbose_name="对冲后Beta")
    hedge_profit = models.DecimalField(
        max_digits=20, decimal_places=2, null=True, blank=True, verbose_name="对冲盈亏"
    )
...
class PolicyIngestionConfig(models.Model):
    ...
    auto_approve_enabled = models.BooleanField(
        default=False, verbose_name="启用自动生效", help_text="是否启用高置信度事件自动生效"
    )
    ...
    p23_sla_hours = models.IntegerField(
        default=2, verbose_name="P2/P3 SLA（小时）", help_text="P2/P3 事件审核超时时间"
    )
    normal_sla_hours = models.IntegerField(
        default=24, verbose_name="普通 SLA（小时）", help_text="P0/P1 事件审核超时时间"
    )

    # 版本控制
    version = models.IntegerField(default=1, verbose_name="配置版本")
    updated_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="更新人"
    )
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新时间")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    ...
    @classmethod
    def get_config(cls) -> PolicyIngestionConfig:
        """
        获取配置（单例模式）
        """
        config, _created = cls._default_manager.get_or_create(
...
class SentimentGateConfig(models.Model):
    ...
    asset_class = models.CharField(
        max_length=20, choices=ASSET_CLASS_CHOICES, unique=True, verbose_name="资产分类"
    )

    # 热度阈值
    heat_l1_threshold = models.FloatField(
        default=30.0, verbose_name="热度 L1 阈值", help_text="热度达到此值触发 L1 关注"
    )
    heat_l2_threshold = models.FloatField(
        default=60.0, verbose_name="热度 L2 阈值", help_text="热度达到此值触发 L2 警戒"
    )
    heat_l3_threshold = models.FloatField(
        default=85.0, verbose_name="热度 L3 阈值", help_text="热度达到此值触发 L3 严控"
    )

    # 情绪阈值（负值，绝对值越大越悲观）
    sentiment_l1_threshold = models.FloatField(
        default=-0.3, verbose_name="情绪 L1 阈值", help_text="情绪低于此值触发 L1 关注（如 -0.3）"
    )
    sentiment_l2_threshold = models.FloatField(
        default=-0.6, verbose_name="情绪 L2 阈值", help_text="情绪低于此值触发 L2 警戒"
    )
    sentiment_l3_threshold = models.FloatField(
        default=-0.8, verbose_name="情绪 L3 阈值", help_text="情绪低于此值触发 L3 严控"
    )
...
class GateActionAuditLog(models.Model):
    ...
    event = models.ForeignKey(
        PolicyLog, on_delete=models.CASCADE, related_name="audit_logs", verbose_name="关联事件"
    )
    ...
    before_state = models.JSONField(
        default=dict, blank=True, verbose_name="操作前状态", help_text="操作前的关键字段快照"
    )
    after_state = models.JSONField(
        default=dict, blank=True, verbose_name="操作后状态", help_text="操作后的关键字段快照"
    )

    # 原因与版本
    reason = models.TextField(
        blank=True, verbose_name="操作原因", help_text="拒绝/回滚/豁免时填写的原因"
    )
    rule_version = models.CharField(
        max_length=50, blank=True, verbose_name="规则版本", help_text="执行时的规则版本号"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
...
class InAppNotification(models.Model):
    ...
    channel = models.CharField(
        max_length=20, choices=CHANNEL_CHOICES, default="in_app", verbose_name="通知渠道"
    )
    priority = models.CharField(
        max_length=20,
    ...
    is_global = models.BooleanField(
        default=False, verbose_name="全局通知", help_text="是否为全局通知（所有用户可见）"
    )
    ...
    metadata = models.JSONField(
        default=dict, blank=True, verbose_name="元数据", help_text="额外的结构化信息"
    )
