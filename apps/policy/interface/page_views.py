...
class RSSReaderView(LoginRequiredMixin, ListView):  # type: ignore[type-arg]
    """RSS阅读器页面 - 显示抓取的文章"""
...
class PolicyEventsPageView(LoginRequiredMixin, ListView):  # type: ignore[type-arg]
...
class RSSSourceCreateView(StaffRequiredMixin, FormView):  # type: ignore[type-arg]
    ...
    def form_valid(self, form: RSSSourceForm) -> HttpResponse:
...
class PolicyKeywordCreateView(StaffRequiredMixin, FormView):  # type: ignore[type-arg]
    ...
    def form_valid(self, form: PolicyKeywordForm) -> HttpResponse:
...
class WorkbenchView(LoginRequiredMixin, ListView):  # type: ignore[type-arg]
    """
    工作台页面视图
