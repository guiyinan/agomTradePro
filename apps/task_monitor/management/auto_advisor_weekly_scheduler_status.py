...
def _resolve_auto_advisor_weekly_schedule_config(
    *,
    ...
    day_of_week = str(
        (effective_schedule or {}).get("day_of_week")
        or EXPECTED_AUTO_ADVISOR_WEEKLY_DAY_OF_WEEK
    )
    timezone = str(
        (effective_schedule or {}).get("timezone")
        or EXPECTED_SCHEDULE_TIMEZONE
    )
    hour = _parse_single_crontab_number(str((effective_schedule or {}).get("hour") or ""))
    minute = _parse_single_crontab_number(
        str((effective_schedule or {}).get("minute") or "")
    )
    return {
