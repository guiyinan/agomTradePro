...
from apps.task_monitor.management import scheduler_status_utils
    ...
    *,
    ...
    issues.extend(
        _prefix_scheduler_issues(
            scheduler_status_utils.build_run_control_safety_issues(
...
def _build_quote_pre_readiness_schedule_safety_issue(
    *,
    ...
    minute = scheduler_status_utils.parse_single_crontab_number(
    ...
    if scheduled_minutes < MIN_QUOTE_PRE_READINESS_POST_CLOSE_MINUTES:
        return {
            ...
            "message": (
                "Pre-readiness quote refresh should run after the 15:00 market close."
            ),
        }
    return None
