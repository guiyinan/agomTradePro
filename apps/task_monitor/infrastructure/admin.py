...
class TaskExecutionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("基本信息", {
            "fields": ("task_id", "task_name", "status", "priority")
        }),
        ("时间信息", {
            "fields": ("started_at", "finished_at", "runtime_seconds")
        }),
        ("执行参数", {
            "fields": ("args", "kwargs")
        }),
        ("执行结果", {
            "fields": ("result", "exception", "traceback")
        }),
        ("配置信息", {
            "fields": ("queue", "worker", "retries")
        }),
        ("元数据", {
...
class TaskAlertAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("告警信息", {
            "fields": ("level", "title", "message")
        }),
        ("任务信息", {
            "fields": ("task_id", "task_name")
        }),
        ("异常信息", {
            "fields": ("exception", "traceback")
        }),
        ("发送状态", {
            "fields": ("is_sent", "sent_at", "send_error")
        }),
        ("元数据", {
            "fields": ("metadata",)
        }),
