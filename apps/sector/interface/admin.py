...
class SectorInfoAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'sector_code', 'sector_name', 'level',
        'parent_code', 'is_active', 'created_at'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('sector_code', 'sector_name', 'level')
        }),
        ('层级关系', {
            'fields': ('parent_code',)
        }),
...
class SectorIndexAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('sector_code', 'trade_date')
        }),
        ('价格数据', {
            'fields': ('open_price', 'high', 'low', 'close', 'change_pct')
        }),
        ('成交数据', {
...
class SectorConstituentAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'enter_date', 'exit_date', 'is_current'
    ]
...
class SectorRelativeStrengthAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('相对强弱指标', {
            'fields': ('relative_strength', 'momentum', 'momentum_window', 'beta')
        }),
        ('时间戳', {
            'fields': ('created_at',),
            'classes': ('collapse',)
