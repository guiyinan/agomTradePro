...
class StockInfoModel(models.Model):
    ...
    stock_code = models.CharField(
        max_length=10, unique=True, db_index=True, verbose_name="股票代码"
    )
...
class StockDailyModel(models.Model):
    ...
    ma5 = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="5日均线"
    )
    ma20 = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="20日均线"
    )
    ma60 = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="60日均线"
    )

    # MACD
    macd = models.FloatField(null=True, blank=True, verbose_name="MACD")
    macd_signal = models.FloatField(null=True, blank=True, verbose_name="MACD 信号线")
    macd_hist = models.FloatField(null=True, blank=True, verbose_name="MACD 柱状图")

    # RSI
    rsi = models.FloatField(null=True, blank=True, verbose_name="RSI（14日）")
...
class FinancialDataModel(models.Model):
    ...
    report_type = models.CharField(
        max_length=10,
        choices=[("1Q", "一季报"), ("2Q", "中报"), ("3Q", "三季报"), ("4Q", "年报")],
        verbose_name="报告类型",
    )
...
class ValuationModel(models.Model):
    ...
    source_provider = models.CharField(
        max_length=32, default="unknown", db_index=True, verbose_name="数据提供方"
    )
    source_updated_at = models.DateTimeField(null=True, blank=True, verbose_name="源端更新时间")
    fetched_at = models.DateTimeField(default=timezone.now, db_index=True, verbose_name="抓取时间")
    pe_type = models.CharField(
        max_length=16, default="dynamic", db_index=True, verbose_name="PE口径"
    )
    is_valid = models.BooleanField(default=True, db_index=True, verbose_name="是否有效")
    quality_flag = models.CharField(
        max_length=32, default="ok", db_index=True, verbose_name="质量标记"
    )
    quality_notes = models.CharField(
        max_length=255, default="", blank=True, verbose_name="质量说明"
    )
    raw_payload_hash = models.CharField(
        max_length=64, default="", blank=True, verbose_name="原始载荷哈希"
    )
...
class ScoringWeightConfigModel(models.Model):
    ...
    description = models.CharField(max_length=500, blank=True, verbose_name="配置描述")
    is_active = models.BooleanField(default=True, verbose_name="是否启用")

    # 评分维度权重（总和应为 1.0）
    growth_weight = models.FloatField(default=0.4, verbose_name="成长性评分权重")
    profitability_weight = models.FloatField(default=0.4, verbose_name="盈利能力评分权重")
    valuation_weight = models.FloatField(default=0.2, verbose_name="估值评分权重")

    # 成长性内部权重（总和应为 1.0）
    revenue_growth_weight = models.FloatField(default=0.5, verbose_name="营收增长率权重")
    profit_growth_weight = models.FloatField(default=0.5, verbose_name="净利润增长率权重")
    ...
    def clean(self) -> None:
        ...
        # 检查维度权重总和
        total_dimension = self.growth_weight + self.profitability_weight + self.valuation_weight
        ...
        # 检查成长性内部权重总和
        total_growth = self.revenue_growth_weight + self.profit_growth_weight
        ...
        for field_name in [
            ...
            if not (0.0 <= value <= 1.0):
                raise ValidationError({field_name: f"权重必须在 [0.0, 1.0] 范围内，当前为 {value}"})
    ...
    def to_domain_entity(self) -> "ScoringWeightConfig":
        """转换为 Domain 层实体"""
        from apps.equity.domain.entities import ScoringWeightConfig
...
class StockPoolSnapshot(models.Model):
    ...
    avg_roe = models.FloatField(null=True, blank=True, verbose_name="平均 ROE")
    avg_pe = models.FloatField(null=True, blank=True, verbose_name="平均 PE")
...
class ValuationRepairTrackingModel(models.Model):
    ...
    # 股票标识
    stock_code = models.CharField(max_length=10, db_index=True, verbose_name="股票代码")
    stock_name = models.CharField(max_length=100, default="", blank=True, verbose_name="股票名称")

    # 日期信息
    as_of_date = models.DateField(db_index=True, verbose_name="数据截止日期")
    repair_start_date = models.DateField(null=True, blank=True, verbose_name="修复开始日期")
    repair_start_percentile = models.FloatField(
        null=True, blank=True, verbose_name="修复起始分位数"
    )

    # 修复状态
    current_phase = models.CharField(max_length=32, db_index=True, verbose_name="当前修复阶段")
    signal = models.CharField(max_length=32, default="none", db_index=True, verbose_name="交易信号")

    # 分位数指标
    composite_percentile = models.FloatField(verbose_name="综合分位数")
    pe_percentile = models.FloatField(null=True, blank=True, verbose_name="PE分位数")
    pb_percentile = models.FloatField(verbose_name="PB分位数")

    # 修复进度指标
    repair_progress = models.FloatField(null=True, blank=True, verbose_name="修复进度(%)")
    repair_speed_per_30d = models.FloatField(
        null=True, blank=True, verbose_name="修复速度(每30天百分点)"
    )
    estimated_days_to_target = models.IntegerField(
        null=True, blank=True, verbose_name="预计到达目标天数"
    )

    # 停滞检测
    is_stalled = models.BooleanField(default=False, db_index=True, verbose_name="是否停滞")
    stall_start_date = models.DateField(null=True, blank=True, verbose_name="停滞开始日期")
    ...
    lowest_percentile_date = models.DateField(null=True, blank=True, verbose_name="最低分位数日期")

    # 配置参数
    target_percentile = models.FloatField(default=0.5, verbose_name="目标分位数")
    composite_method = models.CharField(
        max_length=20, default="pb_only", verbose_name="综合分位数方法"
    )
    confidence = models.FloatField(default=0.0, verbose_name="置信度")

    # 股票池来源
    source_universe = models.CharField(
        max_length=32, default="all_active", db_index=True, verbose_name="来源股票池"
    )
...
class ValuationDataQualitySnapshotModel(models.Model):
    ...
    as_of_date = models.DateField(unique=True, db_index=True, verbose_name="数据日期")
    ...
    primary_source = models.CharField(max_length=32, default="akshare", verbose_name="主数据源")
    fallback_used_count = models.IntegerField(default=0, verbose_name="备源使用数")

    is_gate_passed = models.BooleanField(default=False, db_index=True, verbose_name="是否通过门禁")
    gate_reason = models.CharField(max_length=255, default="", blank=True, verbose_name="门禁原因")
...
class ValuationRepairConfigModel(models.Model):
    ...
    is_active = models.BooleanField(default=False, db_index=True, verbose_name="是否激活（生效中）")
    effective_from = models.DateTimeField(null=True, blank=True, verbose_name="生效时间")

    # ============== 历史数据要求 ==============
    min_history_points = models.IntegerField(default=120, verbose_name="最小历史样本数")
    default_lookback_days = models.IntegerField(default=756, verbose_name="默认回看交易日数")

    # ============== 修复确认参数 ==============
    confirm_window = models.IntegerField(default=20, verbose_name="修复确认窗口（交易日）")
    min_rebound = models.FloatField(default=0.05, verbose_name="最小反弹幅度（百分位）")

    # ============== 停滞检测参数 ==============
    stall_window = models.IntegerField(default=40, verbose_name="停滞检测窗口（交易日）")
    stall_min_progress = models.FloatField(default=0.02, verbose_name="停滞最小进展阈值")

    # ============== 阶段判定阈值 ==============
    target_percentile = models.FloatField(default=0.50, verbose_name="目标百分位")
    undervalued_threshold = models.FloatField(default=0.20, verbose_name="低估阈值")
    near_target_threshold = models.FloatField(default=0.45, verbose_name="接近目标阈值")
    overvalued_threshold = models.FloatField(default=0.80, verbose_name="高估阈值")
    ...
    confidence_sample_threshold = models.IntegerField(default=252, verbose_name="置信度样本数阈值")
    confidence_sample_bonus = models.FloatField(default=0.2, verbose_name="置信度样本数奖励")
    confidence_blend_bonus = models.FloatField(default=0.15, verbose_name="置信度 pe_pb_blend 奖励")
    confidence_repair_start_bonus = models.FloatField(
        default=0.15, verbose_name="置信度修复起点奖励"
    )
    confidence_not_stalled_bonus = models.FloatField(default=0.1, verbose_name="置信度非停滞奖励")

    # ============== 其他阈值 ==============
    repairing_threshold = models.FloatField(default=0.10, verbose_name="REPAIRING 阶段阈值")
    eta_max_days = models.IntegerField(default=999, verbose_name="ETA 最大天数")

    # ============== 审计字段 ==============
    change_reason = models.TextField(default="", blank=True, verbose_name="变更原因")
    created_by = models.CharField(max_length=64, default="", blank=True, verbose_name="创建人")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新时间")
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not self.pk:
            max_version = (
                ValuationRepairConfigModel.objects.aggregate(max_v=models.Max("version"))["max_v"]
                or 0
            )
        ...
        # 如果设置为激活，先停用其他配置
        if self.is_active:
            ValuationRepairConfigModel.objects.filter(is_active=True).exclude(pk=self.pk).update(
                is_active=False
            )
    ...
    @classmethod
    def get_active_config(cls) -> Self | None:
        """获取当前激活的配置"""
        return cls.objects.filter(is_active=True).first()

    def to_domain_config(self) -> "ValuationRepairConfig":
        """转换为 Domain 层配置对象"""
        from apps.equity.domain.entities_valuation_repair import ValuationRepairConfig
...
class StockScreeningRuleConfigModel(models.Model):
    ...
    min_market_cap = models.DecimalField(
        max_digits=20, decimal_places=2, default=0, verbose_name="最低市值（元）"
    )

    # 行业偏好（JSON 数组）
    sector_preference = models.JSONField(default=list, blank=True, verbose_name="偏好行业列表")
