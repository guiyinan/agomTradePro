...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if existing_count > 0:
            self.stdout.write(self.style.WARNING(f"数据库中已存在 {existing_count} 个评分权重配置"))
        ...
        for config_data in default_configs:
            ...
            if ScoringWeightConfigModel._default_manager.filter(name=name).exists():
                self.stdout.write(self.style.WARNING(f'配置 "{name}" 已存在，跳过创建'))
                continue

            try:
                ScoringWeightConfigModel._default_manager.create(**config_data)
                created_count += 1
                self.stdout.write(self.style.SUCCESS(f"成功创建配置: {name}"))
            except Exception:
                self.stdout.write(self.style.ERROR(f'创建配置 "{name}" 失败'))
        ...
        if created_count > 0:
            ...
            self.stdout.write(self.style.WARNING("没有创建新配置"))
