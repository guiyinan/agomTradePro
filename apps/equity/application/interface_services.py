...
def get_active_valuation_repair_config_payload() -> dict[str, Any]:
    ...
    active_config = get_equity_valuation_repair_config_repository().get_active_model_if_available()
...
def init_stock_screening_rules(*, rules: list[dict[str, Any]]) -> None:
    ...
    repository = get_equity_bootstrap_config_repository()
    for rule_data in rules:
        repository.upsert_stock_screening_rule(rule_data)
...
def init_sector_preferences(*, preferences: list[dict[str, Any]]) -> None:
    ...
    for preference in preferences:
        repository.upsert_sector_preference(preference)
...
def init_fund_type_preferences(*, preferences: list[dict[str, Any]]) -> None:
    ...
    repository = get_equity_bootstrap_config_repository()
    for preference in preferences:
        repository.upsert_fund_type_preference(preference)
