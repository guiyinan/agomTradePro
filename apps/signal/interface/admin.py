...
class InvestmentSignalAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'asset_code', 'asset_class', 'direction', 'status',
        'target_regime', 'created_at'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('asset_code', 'asset_class', 'direction', 'status')
        }),
        ('逻辑描述', {
            'fields': ('logic_desc', 'invalidation_logic', 'invalidation_threshold')
        }),
        ('配置', {
            'fields': ('target_regime',)
        }),
        ('审核', {
