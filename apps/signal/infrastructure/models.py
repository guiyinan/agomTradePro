...
class InvestmentSignalModel(models.Model):
    ...
    invalidation_rule_json = models.JSONField(
        null=True, blank=True, help_text="结构化证伪规则 (InvalidationRule.to_dict())"
    )
    invalidation_description = models.TextField(blank=True, help_text="证伪逻辑的人类可读描述")
    ...
    invalidation_threshold = models.FloatField(
        null=True, blank=True, help_text="[DEPRECATED] 请使用 invalidation_rule_json"
    )
    invalidation_rules = models.JSONField(
        default=dict, blank=True, null=True, help_text="[DEPRECATED] 请使用 invalidation_rule_json"
    )
    # ==========================================================
    ...
    invalidation_details = models.JSONField(null=True, blank=True, help_text="证伪时的详细数据")
    ...
    backtest_count = models.IntegerField(
        default=0, verbose_name="回测次数", help_text="该信号参与回测的次数"
    )
    ...
    def clean(self) -> None:
        ...
        if (
            self.backtest_performance_score is not None
            and not 0 <= self.backtest_performance_score <= 100
        ):
            numeric_errors["backtest_performance_score"] = "必须在 0 到 100 之间"
        if self.backtest_count < 0:
            numeric_errors["backtest_count"] = "不能为负数"
        if numeric_errors:
            raise ValidationError(numeric_errors)
    ...
    def _validate_new_format(self, rule_dict: object) -> None:
        """验证新格式的规则结构"""
        if not isinstance(rule_dict, dict):
            raise ValueError("规则必须是字典格式")
        InvalidationRule.from_dict(cast(dict[str, Any], rule_dict))

    def _validate_rules(self, rules: object) -> None:
        """验证旧格式的规则结构（兼容性）"""
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        """Validate all model and business invariants before persistence."""

        self.full_clean(validate_unique=False, validate_constraints=False)
        super().save(*args, **kwargs)

    def to_domain_entity(self) -> InvestmentSignal:
        """转换为 Domain 实体"""

        invalidation_rule = None
        if self.invalidation_rule_json:
            try:
                invalidation_rule = InvalidationRule.from_dict(
                    cast(dict[str, Any], self.invalidation_rule_json)
                )
            except (KeyError, TypeError, ValueError) as exc:
                raise ValueError(
                    f"persisted signal {self.pk} has an invalid invalidation rule"
                ) from exc
    ...
    @classmethod
    def from_domain_entity(cls, signal: InvestmentSignal) -> InvestmentSignalModel:
        ...
        return cls(
            ...
            invalidation_rule_json=(
                signal.invalidation_rule.to_dict() if signal.invalidation_rule else None
            ),
            invalidation_description=signal.invalidation_description or "",
...
class UnifiedSignalModel(models.Model):
    ...
    signal_source = models.CharField(
        max_length=20, choices=SIGNAL_SOURCE_CHOICES, db_index=True, verbose_name="信号来源"
    )
    signal_type = models.CharField(
        max_length=20, choices=SIGNAL_TYPE_CHOICES, db_index=True, verbose_name="信号类型"
    )
    ...
    asset_name = models.CharField(max_length=100, blank=True, verbose_name="资产名称")
    ...
    priority = models.IntegerField(
        default=5, choices=PRIORITY_CHOICES, verbose_name="优先级", help_text="1-10，数字越大越重要"
    )
    ...
    action_required = models.CharField(
        max_length=200, blank=True, verbose_name="所需操作", help_text="建议采取的操作"
    )

    # 额外数据（JSON格式，用于存储特定模块的额外信息）
    extra_data = models.JSONField(
        default=dict, blank=True, verbose_name="额外数据", help_text="特定模块的额外信息"
    )

    # 关联到原始信号ID（如果有）
    related_signal_id = models.CharField(
        max_length=100, blank=True, verbose_name="关联信号ID", help_text="原始模块中的信号ID"
    )
    ...
    def mark_executed(self) -> None:
        ...
        self.save(update_fields=["is_executed", "executed_at", "updated_at"])
