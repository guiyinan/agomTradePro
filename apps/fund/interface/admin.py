...
class FundInfoAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'investment_style', 'management_company',
        'setup_date', 'fund_scale_display', 'is_active'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('fund_code', 'fund_name', 'fund_type', 'investment_style')
        }),
        ('机构信息', {
            'fields': ('management_company', 'custodian')
        }),
        ('规模信息', {
            'fields': ('setup_date', 'fund_scale')
        }),
        ('状态', {
            'fields': ('is_active',)
        }),
...
class FundManagerAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'fund_code', 'manager_name',
        'tenure_start', 'tenure_end',
        'total_tenure_days', 'fund_return',
        'is_current'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('fund_code', 'manager_name')
        }),
        ('任期信息', {
            'fields': ('tenure_start', 'tenure_end', 'total_tenure_days')
        }),
        ('业绩表现', {
            'fields': ('fund_return',)
        }),
...
class FundNetValueAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'fund_code', 'nav_date',
        'unit_nav', 'accum_nav', 'daily_return'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('fund_code', 'nav_date')
        }),
        ('净值数据', {
            'fields': ('unit_nav', 'accum_nav', 'daily_return')
        }),
        ('时间戳', {
            'fields': ('created_at',),
            'classes': ('collapse',)
...
class FundHoldingAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'holding_ratio', 'holding_value_display'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('fund_code', 'report_date')
        }),
        ('持仓信息', {
            'fields': ('stock_code', 'stock_name', 'holding_amount', 'holding_value', 'holding_ratio')
        }),
        ('时间戳', {
            'fields': ('created_at',),
            'classes': ('collapse',)
...
class FundSectorAllocationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('配置信息', {
            'fields': ('sector_name', 'allocation_ratio')
        }),
        ('时间戳', {
            'fields': ('created_at',),
            'classes': ('collapse',)
...
class FundPerformanceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('fund_code', 'start_date', 'end_date')
        }),
        ('收益指标', {
            'fields': ('total_return', 'annualized_return')
        }),
        ('风险指标', {
            'fields': ('volatility', 'max_drawdown')
        }),
        ('风险调整收益', {
            'fields': ('sharpe_ratio', 'alpha', 'beta')
        }),
        ('时间戳', {
            'fields': ('created_at',),
            'classes': ('collapse',)
