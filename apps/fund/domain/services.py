...
class FundScreener:
    ...
    def screen_by_regime(
        ...
        for fund_info, fund_perf, _sector_alloc in all_funds:
            ...
            if (
                preferred_styles
                and fund_info.investment_style
                and fund_info.investment_style not in preferred_styles
            ):
                continue
    ...
    def rank_funds(
        ...
        for fund_info, fund_perf, _sector_alloc in funds_data:
            ...
            perf_score = self._normalize_score(
                ...
                -20.0,
                50.0,  # 假设年化收益范围 -20% 到 50%
            )

            # 2. Regime 适配度评分
            # 根据基金类型和风格匹配 Regime 权重
            regime_fit = self._calculate_regime_fit_score(
                fund_info.fund_type, fund_info.investment_style or "", regime_weights
            )

            # 3. 风险评分（回撤越小越好，夏普越大越好）
            # 最大回撤映射（假设范围 0% 到 50%，越小越好）
            drawdown_score = 100 - self._normalize_score(fund_perf.max_drawdown or 0, 0, 50)
            # 夏普比率映射（假设范围 -1 到 3）
            sharpe_score = self._normalize_score(fund_perf.sharpe_ratio or 0, -1, 3)
    ...
    def _calculate_fund_score(self, fund_info: FundInfo, fund_perf: FundPerformance) -> float:
        """计算基金单一评分（简化版）
    ...
    def _calculate_regime_fit_score(
        self, fund_type: str, investment_style: str, regime_weights: dict[str, float]
    ) -> float:
        """计算 Regime 适配度评分
...
class FundStyleAnalyzer:
    ...
    def analyze_holding_style(self, holdings: list[FundHolding]) -> dict[str, float]:
        """分析持仓风格
    ...
    def analyze_sector_concentration(
        self, sector_alloc: list[FundSectorAllocation]
    ) -> dict[str, float]:
        """分析行业集中度
        ...
        return {"herfindahl_index": herfindahl, "top3_concentration": top3_concentration}
...
class FundPerformanceCalculator:
    ...
    def calculate_total_return(self, nav_series: list[FundNetValue]) -> float:
        """计算区间收益率
    ...
    def calculate_sharpe_ratio(
        self, annualized_return: float, volatility: float, risk_free_rate: float = 3.0
    ) -> float:
        """计算夏普比率
    ...
    def calculate_max_drawdown(self, nav_series: list[FundNetValue]) -> float:
        """计算最大回撤
