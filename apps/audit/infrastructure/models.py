...
class AttributionReport(models.Model):
    ...
    # 归因分析结果
    regime_timing_pnl = models.FloatField(
        verbose_name="Regime 择时贡献", help_text="因 Regime 判断正确/错误产生的收益/损失"
    )
    asset_selection_pnl = models.FloatField(
        verbose_name="资产选择贡献", help_text="因资产选择正确/错误产生的收益/损失"
    )
    interaction_pnl = models.FloatField(verbose_name="交互效应", help_text="择时与选股的交互作用")
    total_pnl = models.FloatField(verbose_name="总收益")

    # Regime 准确性
    regime_accuracy = models.FloatField(
        verbose_name="Regime 预测准确率", help_text="实际 Regime 与预测 Regime 的匹配度（0-1）"
    )
...
class LossAnalysis(models.Model):
    ...
    loss_source = models.CharField(
        max_length=50, choices=LOSS_SOURCE_CHOICES, verbose_name="损失来源"
    )

    impact = models.FloatField(verbose_name="影响金额", help_text="该因素造成的收益/损失")

    impact_percentage = models.FloatField(verbose_name="影响占比", help_text="占总损失的百分比")

    description = models.TextField(verbose_name="详细描述", help_text="损失产生的具体原因和情境")

    # 可改进措施
    improvement_suggestion = models.TextField(verbose_name="改进建议", blank=True)
...
class ExperienceSummary(models.Model):
    ...
    lesson = models.TextField(verbose_name="经验教训", help_text="从本次回测中学到的教训")

    recommendation = models.TextField(verbose_name="改进建议", help_text="针对性的改进措施")
...
class IndicatorPerformanceModel(models.Model):
    ...
    indicator_code = models.CharField(max_length=50, db_index=True, verbose_name="指标代码")
    ...
    # 建议
    recommended_action = models.CharField(max_length=20, null=True, verbose_name="建议操作")
    ...
    def __str__(self) -> str:
        f1_display = f"{self.f1_score:.3f}" if self.f1_score is not None else "N/A"
        return f"{self.indicator_code}: F1={f1_display}"
...
class IndicatorThresholdConfigModel(models.Model):
    ...
    indicator_code = models.CharField(max_length=50, unique=True, verbose_name="指标代码")
    ...
    base_weight = models.FloatField(
        default=1.0, verbose_name="基础权重", help_text="指标的基础权重（0-1）"
    )
    ...
    decay_threshold = models.FloatField(
        default=0.2, verbose_name="衰减阈值", help_text="F1 分数低于此值视为衰减"
    )
...
class ValidationSummaryModel(models.Model):
    ...
    validation_run_id = models.CharField(max_length=100, unique=True, verbose_name="验证运行ID")
    ...
    # 影子模式标记
    is_shadow_mode = models.BooleanField(default=False, verbose_name="是否为影子模式")
...
class ConfidenceConfigModel(models.Model):
    ...
    day_0_coefficient = models.FloatField(
        default=0.6, verbose_name="发布当天系数", help_text="数据发布当天的置信度系数"
    )
    day_7_coefficient = models.FloatField(
        default=0.5, verbose_name="发布1周后系数", help_text="数据发布1周后的置信度系数"
    )
    day_14_coefficient = models.FloatField(
        default=0.4, verbose_name="发布2周后系数", help_text="数据发布2周后的置信度系数"
    )
    day_30_coefficient = models.FloatField(
        default=0.3, verbose_name="发布1月后系数", help_text="数据发布1个月后的置信度系数"
    )

    # 数据类型加成
    daily_data_bonus = models.FloatField(
        default=0.2, verbose_name="日度数据加成", help_text="有日度数据支持时的置信度加成"
    )
    weekly_data_bonus = models.FloatField(
        default=0.1, verbose_name="周度数据加成", help_text="有周度数据支持时的置信度加成"
    )
    daily_consistency_bonus = models.FloatField(
        default=0.1, verbose_name="日度一致性加成", help_text="日度数据与月度数据一致时的加成"
    )

    # 基础置信度
    base_confidence = models.FloatField(
        default=0.5, verbose_name="基础置信度", help_text="默认的基础置信度"
    )

    # 信号冲突解决阈值
    daily_persist_threshold = models.IntegerField(
        default=10, verbose_name="日度持续阈值", help_text="日度信号持续多少天后采用日度信号"
    )
    hybrid_weight_daily = models.FloatField(
        default=0.3, verbose_name="混合日度权重", help_text="混合模式中日度信号的权重"
    )
    hybrid_weight_monthly = models.FloatField(
        default=0.7, verbose_name="混合月度权重", help_text="混合模式中月度信号的权重"
    )

    # 权重动态调整参数
    decay_threshold = models.FloatField(
        default=0.2, verbose_name="衰减阈值", help_text="F1分数低于此值视为衰减"
    )
    decay_penalty = models.FloatField(
        default=0.5, verbose_name="衰减惩罚系数", help_text="衰减后权重乘以该系数"
    )
    improvement_threshold = models.FloatField(
        default=0.1, verbose_name="改进阈值", help_text="F1提升超过此值给予奖励"
    )
    improvement_bonus = models.FloatField(
        default=1.2, verbose_name="改进奖励系数", help_text="改进后权重乘以该系数"
    )
    ...
    def to_domain_config(self) -> "ConfidenceConfig":
...
class OperationLogModel(models.Model):
    ...
    id = models.UUIDField(
        primary_key=True, default=uuid.uuid4, editable=False, verbose_name="日志ID"
    )
    request_id = models.CharField(
        max_length=64, db_index=True, verbose_name="链路追踪ID", help_text="用于关联请求链路"
    )

    # 操作者身份
    user_id = models.IntegerField(null=True, db_index=True, verbose_name="用户ID")
    username = models.CharField(max_length=150, default="anonymous", verbose_name="用户名")
    ip_address = models.GenericIPAddressField(null=True, blank=True, verbose_name="IP地址")
    user_agent = models.CharField(max_length=500, blank=True, verbose_name="User Agent")

    # 来源与租户
    source = models.CharField(
        max_length=20, default="MCP", db_index=True, verbose_name="来源", help_text="MCP/SDK/API"
    )
    client_id = models.CharField(max_length=100, blank=True, db_index=True, verbose_name="客户端ID")
    ...
    module = models.CharField(
        max_length=50, db_index=True, verbose_name="模块", help_text="signal/policy/backtest/..."
    )
    action = models.CharField(
        max_length=50, verbose_name="动作", help_text="CREATE/READ/UPDATE/DELETE/EXECUTE"
    )
    resource_type = models.CharField(max_length=50, blank=True, verbose_name="资源类型")
    resource_id = models.CharField(
        max_length=100, null=True, blank=True, db_index=True, verbose_name="资源ID"
    )

    # MCP 特定字段
    mcp_tool_name = models.CharField(
        max_length=120, null=True, blank=True, db_index=True, verbose_name="MCP工具名"
    )
    mcp_client_id = models.CharField(max_length=100, blank=True, verbose_name="MCP客户端ID")
    mcp_role = models.CharField(max_length=30, blank=True, verbose_name="MCP角色")
    sdk_version = models.CharField(max_length=50, blank=True, verbose_name="SDK版本")

    # 请求详情（params 为脱敏后）
    request_method = models.CharField(max_length=10, default="MCP", verbose_name="请求方法")
    request_path = models.CharField(max_length=500, blank=True, verbose_name="请求路径")
    request_params = models.JSONField(
        default=dict, encoder=DjangoJSONEncoder, verbose_name="请求参数", help_text="已脱敏"
    )
    ...
    response_text = models.TextField(
        blank=True, verbose_name="响应文本快照", help_text="完整或截断后的文本响应"
    )
    response_status = models.IntegerField(default=200, db_index=True, verbose_name="响应状态码")
    ...
    exception_traceback = models.TextField(blank=True, verbose_name="异常堆栈")
    ...
    checksum = models.CharField(
        max_length=64, blank=True, verbose_name="校验和", help_text="SHA-256"
    )
