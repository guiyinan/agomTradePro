...
class IndicatorThresholdConfigModelAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'indicator_code', 'indicator_name', 'category', 'level_low', 'level_high',
        'base_weight', 'is_active'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('indicator_code', 'indicator_name', 'category', 'description')
        }),
        ('阈值配置', {
            'fields': ('level_low', 'level_high')
        }),
        ('权重配置', {
            'fields': ('base_weight', 'min_weight', 'max_weight')
        }),
        ('验证阈值', {
            'fields': (
                'decay_threshold', 'decay_penalty',
                'improvement_threshold', 'improvement_bonus'
            )
        }),
        ('高级配置', {
...
class IndicatorPerformanceModelAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'indicator_code', 'evaluation_period_end', 'f1_score', 'stability_score',
        'recommended_action', 'recommended_weight', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        ('领先时间', {
            'fields': ('lead_time_mean', 'lead_time_std')
        }),
        ('稳定性分析', {
            'fields': (
                'pre_2015_correlation', 'post_2015_correlation', 'stability_score'
        ...
        ('信号特征', {
            'fields': ('decay_rate', 'signal_strength')
        }),
        ('建议', {
            'fields': (
                'recommended_action', 'recommended_weight', 'confidence_level'
...
class ValidationSummaryModelAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'validation_run_id', 'run_date', 'total_indicators',
        'approved_indicators', 'rejected_indicators', 'pending_indicators',
        'avg_f1_score', 'avg_stability_score', 'status', 'is_shadow_mode'
    ]
    list_filter = ['status', 'is_shadow_mode', 'run_date']
    search_fields = ['validation_run_id', 'overall_recommendation']
    readonly_fields = [
        'validation_run_id', 'run_date',
        'evaluation_period_start', 'evaluation_period_end',
        'total_indicators', 'approved_indicators',
        'rejected_indicators', 'pending_indicators',
        'avg_f1_score', 'avg_stability_score',
        'overall_recommendation', 'status', 'is_shadow_mode',
        'error_message'
    ]
    ...
    fieldsets = (
        ...
        ('统计摘要', {
            'fields': ('avg_f1_score', 'avg_stability_score')
        }),
        ('总体建议', {
            'fields': ('overall_recommendation',)
        }),
        ('状态', {
            'fields': ('status', 'is_shadow_mode', 'error_message')
        }),
    )
