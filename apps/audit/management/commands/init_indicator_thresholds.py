...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        """Register indicator-threshold initialization options."""
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        """Create or refresh the governed indicator threshold seed rows."""

        refresh = options.get("refresh", False)
        if not isinstance(refresh, bool):
            raise CommandError("refresh must be a boolean flag")
        ...
        # 分段验证配置（用于稳定性分析）
        default_validation_periods: list[dict[str, str]] = [
            {
                "name": "刚兑时期",
                "start": "2005-01-01",
                "end": "2017-12-31",
        ...
        # 默认行为阈值
        default_action_thresholds: dict[str, float] = {
            "keep_min_f1": 0.6,
            "reduce_min_f1": 0.4,
            "remove_max_f1": 0.3,
        }
        ...
        for config_data in default_configs:
            ...
            try:
                ...
                if refresh:
                    ...
                    existing.save()
                    updated_count += 1
                    self.stdout.write(self.style.WARNING(f"Updated: {indicator_code}"))
                else:
                    skipped_count += 1
                    self.stdout.write(self.style.WARNING(f"Skipped (exists): {indicator_code}"))
                ...
                created_count += 1
                self.stdout.write(self.style.SUCCESS(f"Created: {indicator_code}"))
        ...
        self.stdout.write(
            'Run "python manage.py init_indicator_thresholds --refresh" to update existing configs.'
        )
