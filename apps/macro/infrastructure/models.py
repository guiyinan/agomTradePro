...
class MacroIndicator(models.Model):
    ...
    original_unit = models.CharField(
        max_length=50, default="", help_text="原始单位（数据源返回的单位，用于展示）"
    )
    ...
    @property
    def is_term_data(self) -> bool:
        """是否为期限类数据（如 3M、10Y 等利率）"""
        duration = self.period_type[:-1]
        suffix = self.period_type[-1:]
        return bool(duration) and duration.isdigit() and suffix in {"M", "Y"}
...
class ExchangeRateModel(models.Model):
    ...
    rate = models.DecimalField(max_digits=16, decimal_places=6, verbose_name="汇率")
    effective_date = models.DateField(verbose_name="生效日期", db_index=True)
    source = models.CharField(max_length=30, default="akshare", verbose_name="数据来源")
...
class IndicatorConfigModel(models.Model):
    ...
    code = models.CharField(max_length=50, unique=True, primary_key=True, verbose_name="指标代码")
    name = models.CharField(max_length=100, verbose_name="指标名称")
    name_en = models.CharField(max_length=100, blank=True, verbose_name="英文名称")
    category = models.CharField(max_length=20, choices=CATEGORY_CHOICES, verbose_name="指标分类")
    unit = models.CharField(max_length=10, verbose_name="单位")

    # 可配置的阈值
    threshold_bullish = models.FloatField(null=True, blank=True, verbose_name="看涨阈值")
    threshold_bearish = models.FloatField(null=True, blank=True, verbose_name="看跌阈值")

    # 数据源配置
    data_source = models.CharField(max_length=20, default="akshare", verbose_name="数据源")
