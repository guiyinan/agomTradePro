...
class MacroIndicatorAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("数据", {"fields": ("code", "value", "reporting_period", "period_type")}),
        (
            "元数据",
            {"fields": ("published_at", "publication_lag_days", "source", "revision_number")},
        ),
...
class IndicatorUnitConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("基本信息", {"fields": ("indicator_code", "source", "original_unit")}),
        ("设置", {"fields": ("is_currency", "priority", "is_active")}),
