...
class HedgePairModel(models.Model):
    ...
    target_hedge_weight = models.FloatField(default=0.3, verbose_name="目标对冲权重")

    # Rebalance triggers
    rebalance_trigger = models.FloatField(default=0.05, verbose_name="调仓触发阈值")
    correlation_window = models.IntegerField(default=60, verbose_name="相关性计算窗口（天）")
    ...
    correlation_alert_threshold = models.FloatField(default=0.2, verbose_name="相关性告警阈值")

    # Risk limits
    max_hedge_cost = models.FloatField(default=0.05, verbose_name="最大对冲成本")
    beta_target = models.FloatField(null=True, blank=True, verbose_name="目标Beta")
...
class CorrelationHistoryModel(models.Model):
    ...
    asset1 = models.CharField(max_length=20, db_index=True, verbose_name="资产1")
    asset2 = models.CharField(max_length=20, db_index=True, verbose_name="资产2")
    ...
    correlation_trend = models.CharField(
        max_length=20, default="neutral", verbose_name="相关性趋势"
    )
    correlation_ma = models.FloatField(default=0.0, verbose_name="相关性均线")

    # Alert information
    alert = models.TextField(blank=True, verbose_name="告警信息")
    alert_type = models.CharField(max_length=50, blank=True, verbose_name="告警类型")
    ...
    def to_domain(self) -> CorrelationMetric:
        """Convert to a domain correlation metric."""
        ...
        return CorrelationMetric(
            ...
            correlation_ma=self.correlation_ma,
            alert=self.alert or None,
            alert_type=self.alert_type or None,
        )
...
class HedgePortfolioSnapshotModel(models.Model):
    ...
    pair = models.ForeignKey(
        HedgePairModel, on_delete=models.CASCADE, related_name="snapshots", verbose_name="对冲对"
    )
    ...
    def to_domain(self) -> HedgePortfolio:
        """Convert to domain entity"""
...
class HedgeAlertModel(models.Model):
    ...
    pair_name = models.CharField(max_length=100, db_index=True, verbose_name="对冲对名称")
    ...
    def to_domain(self) -> HedgeAlert:
        ...
        return HedgeAlert(
            ...
            alert_type=alert_type_map.get(self.alert_type, HedgeAlertType.UNKNOWN),
...
class HedgePerformanceModel(models.Model):
    ...
    correlation_stability = models.FloatField(default=0.0, verbose_name="相关性稳定性")
    ...
    def to_domain(self) -> HedgePerformance:
        """Convert to a domain hedge-performance result."""
...
class HedgingInstrumentConfigModel(models.Model):
    ...
    instrument_code = models.CharField(max_length=50, unique=True, verbose_name="工具代码")

    instrument_name = models.CharField(max_length=100, verbose_name="工具名称")

    instrument_type = models.CharField(
        max_length=20, choices=INSTRUMENT_TYPE_CHOICES, verbose_name="工具类型"
    )

    # 对冲参数
    hedge_ratio = models.FloatField(
        default=1.0, verbose_name="对冲比例", help_text="如 0.95 表示需要对冲95%的敞口"
    )

    underlying_index = models.CharField(
        max_length=50, blank=True, verbose_name="标的指数", help_text="如 000300.SH 表示沪深300"
    )

    # 成本参数
    cost_bps = models.FloatField(
        default=5.0, verbose_name="对冲成本（基点）", help_text="如 5 表示 0.05%"
    )
