...
class HedgePairSeed(TypedDict, total=False):
    """One governed hedge-pair seed payload."""
...
class Command(BaseCommand):
    ...
    def handle(self, *args: str, **options: Any) -> None:
        ...
        if not isinstance(reset, bool):
            raise CommandError("--reset must be boolean")
    ...
    def init_hedge_pairs(self) -> None:
        """Initialize hedge pair configurations"""
        hedge_pairs: list[HedgePairSeed] = [
            {
                "name": "股债对冲",
        ...
        for pair_data in hedge_pairs:
            pair, created = HedgePairModel._default_manager.get_or_create(
                name=pair_data["name"], defaults=pair_data
            )
            if created:
