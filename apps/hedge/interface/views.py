...
class HedgeActionViewSet(viewsets.ViewSet):
        ...
        return Response(
            {
                "actions": [
                    {
                        "endpoint": "/hedge/actions/calculate-correlation/",
                        "method": "POST",
                        "description": "Calculate correlation between two assets",
                    },
                    {
                        "endpoint": "/hedge/actions/check-hedge-ratio/",
                        "method": "POST",
                        "description": "Calculate hedge ratio for a pair",
                    },
                    {
                        "endpoint": "/hedge/actions/get-correlation-matrix/",
                        "method": "POST",
                        "description": "Get correlation matrix for multiple assets",
                    },
                ]
            }
        )
    ...
    def check_hedge_ratio(self, request: Request) -> Response:
        ...
        if result is None:
            return Response(
                {"error": f"Hedge pair not found: {pair_name}"}, status=status.HTTP_404_NOT_FOUND
            )
...
def activate_pair_view(request: HttpRequest, pair_id: int) -> JsonResponse:
    ...
    if response.success:
        return JsonResponse({"success": True, "message": response.message})
    else:
        return JsonResponse({"success": False, "error": response.message}, status=404)
