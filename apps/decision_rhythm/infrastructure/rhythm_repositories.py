...
class QuotaRepository:
    ...
    def get_quota(self, period: QuotaPeriod, account_id: str = "default") -> DecisionQuota | None:
        """
        获取配额
        ...
        """
        try:
            model = self.model.objects.filter(account_id=account_id, period=period.value).first()
    ...
    def reset_quota(self, period: QuotaPeriod, account_id: str = "default") -> bool:
        """
        重置配额
        ...
        """
        try:
            updated = self.model.objects.filter(account_id=account_id, period=period.value).update(
                used_decisions=0,
                used_executions=0,
            )

            logger.info(f"Reset {updated} quotas for account={account_id}, period={period.value}")
...
class CooldownRepository:
    ...
    def get_active_cooldown(
        ...
        """
        model = self.model.objects.filter(asset_code=asset_code).order_by("-created_at").first()
    ...
    def save(self, cooldown: CooldownPeriod) -> CooldownPeriod:
        ...
        """
        # 检查是否已存在
        existing = self.model.objects.filter(asset_code=cooldown.asset_code).first()
...
class DecisionRequestRepository:
    ...
    def save_request(self, request: DecisionRequest) -> DecisionRequest:
        ...
        """
        # 检查是否已存在
        existing = self.model.objects.filter(request_id=request.request_id).first()
    ...
    def save_response(
        ...
        try:
            ...
            # 检查是否已存在响应
            existing_response = self.response_model.objects.filter(request=request_model).first()
    ...
    def get_pending_for_execution(self) -> list[DecisionRequest]:
        ...
        """
        models = self.model.objects.filter(execution_status="PENDING").order_by("-requested_at")
    ...
    def get_by_candidate_id(self, candidate_id: str) -> DecisionRequest | None:
        ...
        try:
            model = (
                self.model.objects.filter(candidate_id=candidate_id)
