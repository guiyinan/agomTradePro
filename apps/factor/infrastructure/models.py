...
if TYPE_CHECKING:
    from apps.factor.domain.entities import FactorDefinition, FactorPortfolioConfig


class FactorDefinitionModel(models.Model):
    """Factor definition table"""

    code = models.CharField(max_length=50, unique=True, db_index=True, verbose_name="因子代码")
    ...
    update_frequency = models.CharField(max_length=20, default="daily", verbose_name="更新频率")
...
class FactorExposureModel(models.Model):
    ...
    stock_code = models.CharField(max_length=20, db_index=True, verbose_name="股票代码")
    trade_date = models.DateField(db_index=True, verbose_name="交易日期")
    factor_code = models.CharField(max_length=50, db_index=True, verbose_name="因子代码")
    factor_value = models.DecimalField(max_digits=18, decimal_places=6, verbose_name="因子值")
    percentile_rank = models.DecimalField(max_digits=5, decimal_places=4, verbose_name="百分位排名")
    z_score = models.DecimalField(max_digits=10, decimal_places=6, verbose_name="Z得分")
    normalized_score = models.DecimalField(
        max_digits=6, decimal_places=2, default=0.0, verbose_name="标准化得分"
    )
...
class FactorPortfolioConfigModel(models.Model):
    ...
    universe = models.CharField(max_length=20, default="all_a", verbose_name="股票池")
    min_market_cap = models.DecimalField(
        max_digits=18, decimal_places=2, null=True, blank=True, verbose_name="最小市值（亿）"
    )
    max_market_cap = models.DecimalField(
        max_digits=18, decimal_places=2, null=True, blank=True, verbose_name="最大市值（亿）"
    )
    max_pe = models.DecimalField(
        max_digits=10, decimal_places=4, null=True, blank=True, verbose_name="最大PE"
    )
    min_pe = models.DecimalField(
        max_digits=10, decimal_places=4, null=True, blank=True, verbose_name="最小PE"
    )
    max_pb = models.DecimalField(
        max_digits=10, decimal_places=4, null=True, blank=True, verbose_name="最大PB"
    )
    max_debt_ratio = models.FloatField(null=True, blank=True, verbose_name="最大资产负债率（%）")
    ...
    weight_method = models.CharField(max_length=50, default="equal_weight", verbose_name="权重方式")
    max_sector_weight = models.FloatField(default=0.4, verbose_name="最大行业权重")
    max_single_stock_weight = models.FloatField(default=0.05, verbose_name="最大单股权重")
    is_active = models.BooleanField(default=True, verbose_name="是否启用")
    ...
    def to_domain(self) -> FactorPortfolioConfig:
        """Convert to domain entity"""
...
class FactorPortfolioHoldingModel(models.Model):
    ...
    weight = models.DecimalField(max_digits=5, decimal_places=4, verbose_name="权重")
    factor_score = models.DecimalField(max_digits=10, decimal_places=4, verbose_name="因子得分")
    rank = models.IntegerField(verbose_name="排名")
    sector = models.CharField(max_length=50, blank=True, verbose_name="行业")
