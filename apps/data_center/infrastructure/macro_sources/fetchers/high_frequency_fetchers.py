"""
Data Center 高频指标数据获取器（Regime 滞后性改进方案 Phase 1）。
...
_BOND_SOURCE_COLUMNS = {
    "CN_BOND_2Y": "中国国债收益率2年",
    "CN_BOND_5Y": "中国国债收益率5年",
...
def _require_columns(
    ...
    *,
    ...
    """Reject source schema drift instead of guessing columns by position."""
...
class HighFrequencyIndicatorFetcher:
    ...
    def _get_bond_yields(self) -> pd.DataFrame | None:
        ...
        try:
            ...
            df = df.copy()
            required_source_columns = ("日期", *_BOND_SOURCE_COLUMNS.values())
            _require_columns(
                df,
                required_source_columns,
                dataset_name="中美国债收益率",
            )
            df["date"] = pd.to_datetime(df["日期"], errors="coerce")
            for indicator_code, source_column in _BOND_SOURCE_COLUMNS.items():
                df[indicator_code] = pd.to_numeric(df[source_column], errors="coerce")
        ...
        try:
            ...
            for _, row in df[["date", "spread"]].dropna().iterrows():
                try:
                    ...
                    spread_bp = parse_required_float(row["spread"]) * 100
                    point = MacroDataPoint(
                        code=indicator_code,
                        value=spread_bp,
                        observed_at=row["date"].date(),
    ...
    def fetch_nhci(self, start_date: date, end_date: date) -> list[MacroDataPoint]:
        """获取南华商品指数
        ...
        logger.warning(
            "%s: macro_china_commodity_price_index 提供中国大宗商品价格指数，"
            "不是南华商品指数；为防止错误口径入库，本数据源不发布该指标",
            indicator_code,
        )
        return []
    ...
    def fetch_fx_center_rate(
        ...
        logger.warning(
            "%s: fx_spot_quote 提供即期买卖报价，不是人民币中间价；"
            "为防止错误口径入库，本数据源不发布该指标",
            indicator_code,
        )
        return []

    def fetch_us_bond_10y(self, start_date: date, end_date: date) -> list[MacroDataPoint]:
        """获取美国10年期国债收益率
    ...
    def fetch_credit_spread(self, start_date: date, end_date: date) -> list[MacroDataPoint]:
        """计算信用利差
    ...
    def fetch_corp_bond_yield(
        self, rating: str, start_date: date, end_date: date
    ) -> list[MacroDataPoint]:
        """获取企业债收益率
    ...
    def fetch_usd_index(self, start_date: date, end_date: date) -> list[MacroDataPoint]:
        """获取美元指数
    ...
    def fetch_vix_index(self, start_date: date, end_date: date) -> list[MacroDataPoint]:
        """获取VIX波动率指数
