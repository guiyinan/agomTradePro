...
class RetentionPolicyRepository:
    ...
    def activate(self, policy: RetentionPolicy) -> RetentionPolicy:
        ...
        RetentionPolicyModel._default_manager.filter(dataset_key=policy.dataset_key, active=True).update(active=False)
    ...
    def get_active(self, dataset_key: str) -> RetentionPolicy | None:
        ...
        model = RetentionPolicyModel._default_manager.filter(dataset_key=dataset_key, active=True).order_by("-version").first()
...
class StorageHoldRepository:
    ...
    def has_active_hold(self, resource_type: str, resource_key: str, *, now: datetime | None = None) -> bool:
        ...
        return StorageHoldModel._default_manager.filter(
            resource_type=resource_type,
            resource_key=resource_key,
            released_at__isnull=True,
        ).filter(Q(expires_at__isnull=True) | Q(expires_at__gt=moment)).exists()
...
class ArchiveManifestRepository:
    ...
    def has_verified_for_dataset(self, dataset_key: str, *, now: datetime | None = None) -> bool:
        ...
        moment = now or datetime.now(UTC)
        return ArchiveManifestModel._default_manager.filter(
            dataset_key=dataset_key,
            state=ArchiveState.VERIFIED.value,
            verified_at__isnull=False,
        ).filter(
            Q(retention_until__isnull=True) | Q(retention_until__gt=moment)
        ).exists()


__all__ = ["ArchiveManifestRepository", "RetentionPolicyRepository", "StorageHoldRepository"]
