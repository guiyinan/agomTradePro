...
from . import public_published_queries as _published_queries  # noqa: E402

_PublicP = ParamSpec("_PublicP")
_PublicR = TypeVar("_PublicR")


def _sync_published_query_dependencies(target_name: str) -> None:
    """Keep the legacy public monkeypatch surface across the split module."""

    split_globals = vars(_published_queries)
    for name, value in tuple(globals().items()):
        if name == target_name or name not in split_globals:
            continue
        if getattr(value, "_published_query_target", None) is not None:
            continue
        setattr(_published_queries, name, value)


def _bind_published_query(
    target_name: str,
    target: Callable[_PublicP, _PublicR],
) -> Callable[_PublicP, _PublicR]:
    """Bind a split query implementation while retaining its public signature."""

    @wraps(target)
    def _bound(*args: _PublicP.args, **kwargs: _PublicP.kwargs) -> _PublicR:
        _sync_published_query_dependencies(target_name)
        return target(*args, **kwargs)

    setattr(_bound, "_published_query_target", target_name)  # noqa: B010
    return _bound


get_current_publication_freshness_gate = _bind_published_query(
    "get_current_publication_freshness_gate",
    _published_queries.get_current_publication_freshness_gate,
)
get_decision_publication_gate = _bind_published_query(
    "get_decision_publication_gate", _published_queries.get_decision_publication_gate
)
get_published_quote_payloads = _bind_published_query(
    "get_published_quote_payloads", _published_queries.get_published_quote_payloads
)
get_published_latest_quote_payload = _bind_published_query(
    "get_published_latest_quote_payload", _published_queries.get_published_latest_quote_payload
)
get_published_quote_series = _bind_published_query(
    "get_published_quote_series", _published_queries.get_published_quote_series
)
get_published_price_bar_series = _bind_published_query(
    "get_published_price_bar_series", _published_queries.get_published_price_bar_series
)
get_published_sector_memberships = _bind_published_query(
    "get_published_sector_memberships", _published_queries.get_published_sector_memberships
)
get_published_market_news = _bind_published_query(
    "get_published_market_news", _published_queries.get_published_market_news
)
get_published_capital_flow_series = _bind_published_query(
    "get_published_capital_flow_series", _published_queries.get_published_capital_flow_series
)
...
list_active_data_sources = _bind_published_query(
    "list_active_data_sources", _published_queries.list_active_data_sources
)
save_data_source_configuration = _bind_published_query(
    "save_data_source_configuration", _published_queries.save_data_source_configuration
)
update_asset_display_name = _bind_published_query(
    "update_asset_display_name", _published_queries.update_asset_display_name
)
save_macro_facts = _bind_published_query("save_macro_facts", _published_queries.save_macro_facts)
list_macro_facts_by_original_unit = _bind_published_query(
    "list_macro_facts_by_original_unit", _published_queries.list_macro_facts_by_original_unit
)
get_macro_runtime_metadata = _bind_published_query(
    "get_macro_runtime_metadata", _published_queries.get_macro_runtime_metadata
)
get_provider_settings_payload = _bind_published_query(
    "get_provider_settings_payload", _published_queries.get_provider_settings_payload
)
list_macro_indicator_codes = _bind_published_query(
    "list_macro_indicator_codes", _published_queries.list_macro_indicator_codes
)
get_market_breadth_snapshot = _bind_published_query(
    "get_market_breadth_snapshot", _published_queries.get_market_breadth_snapshot
)
get_active_stock_fact_coverage_payload = _bind_published_query(
    "get_active_stock_fact_coverage_payload",
    _published_queries.get_active_stock_fact_coverage_payload,
)
get_decision_data_readiness_payload = _bind_published_query(
    "get_decision_data_readiness_payload", _published_queries.get_decision_data_readiness_payload
)
get_decision_provider_capability_health_payload = _bind_published_query(
    "get_decision_provider_capability_health_payload",
    _published_queries.get_decision_provider_capability_health_payload,
)
resolve_asset_payload = _bind_published_query(
    "resolve_asset_payload", _published_queries.resolve_asset_payload
)
list_active_stock_codes = _bind_published_query(
    "list_active_stock_codes", _published_queries.list_active_stock_codes
)
list_price_covered_codes = _bind_published_query(
    "list_price_covered_codes", _published_queries.list_price_covered_codes
)
...
get_latest_quote_payloads = _bind_published_query(
    "get_latest_quote_payloads", _published_queries.get_latest_quote_payloads
)
get_price_bar_series = _bind_published_query(
    "get_price_bar_series", _published_queries.get_price_bar_series
)
get_current_publication = _bind_published_query(
    "get_current_publication", _published_queries.get_current_publication
)
get_publication_as_of = _bind_published_query(
