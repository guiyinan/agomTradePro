...
class RotationTemplateSpec(TypedDict):
    """Seed definition for one risk-tolerance template."""
...
class Command(BaseCommand):
    ...
    def init_asset_classes(self) -> None:
        ...
        for asset_data in assets:
            asset, created = AssetClassModel._default_manager.get_or_create(
                code=asset_data["code"], defaults=asset_data
            )
            if created:
    ...
    def init_rotation_configs(self) -> None:
        """Initialize rotation configurations"""
        ...
        for config_data in configs:
            config, created = RotationConfigModel._default_manager.get_or_create(
                name=config_data["name"], defaults=config_data
            )
            if created:
    ...
    def init_risk_templates(self) -> None:
        """
        初始化预设风险模板。
        ...
        def get_codes(
            ...
            qs = (
                AssetClassModel._default_manager.filter(category=category, is_active=True)
                .order_by("code")
        ...
        templates_spec: list[RotationTemplateSpec] = [
            {
                "key": "conservative",
                "name": "保守型",
