...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument("--pmi", type=float, default=None, help="PMI 阈值（扩张/收缩分界线）")
        parser.add_argument("--cpi-high", type=float, default=None, help="CPI 高通胀阈值")
        parser.add_argument("--cpi-low", type=float, default=None, help="CPI 低通胀阈值")
        parser.add_argument("--ppi-high", type=float, default=None, help="PPI 高通胀阈值")
        parser.add_argument("--activate", action="store_true", help="激活当前或最新配置")

    def handle(self, *args: Any, **options: Any) -> None:
        pmi = _optional_finite_threshold(options.get("pmi"), name="pmi")
        cpi_high = _optional_finite_threshold(options.get("cpi_high"), name="cpi-high")
        cpi_low = _optional_finite_threshold(options.get("cpi_low"), name="cpi-low")
        ppi_high = _optional_finite_threshold(options.get("ppi_high"), name="ppi-high")
        activate = options.get("activate")
        if cpi_low is not None and cpi_high is not None and cpi_low > cpi_high:
            raise CommandError("cpi-low must not exceed cpi-high")

        # 获取或创建默认配置
        config = RegimeThresholdConfig._default_manager.filter(is_active=True).first()
        if not config:
            config = RegimeThresholdConfig._default_manager.create(name="默认配置", is_active=True)
        ...
        with transaction.atomic():
            if pmi is not None:
                threshold, _ = RegimeIndicatorThreshold._default_manager.get_or_create(
                    config=config,
                    indicator_code="PMI",
                    defaults={"indicator_name": "采购经理人指数", "description": "PMI ≥ 50 为扩张"},
                )
            ...
            if cpi_high is not None or cpi_low is not None:
                ...
                next_high = cpi_high if cpi_high is not None else threshold.level_high
                next_low = cpi_low if cpi_low is not None else threshold.level_low
...
def _optional_finite_threshold(value: object, *, name: str) -> float | None:
    """Normalize one optional finite threshold from CLI or direct invocation."""
