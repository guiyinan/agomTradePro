...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        config = RegimeThresholdConfig._default_manager.create(
            name="默认配置 (2026-01)", is_active=True
        )
        ...
        RegimeTrendIndicator._default_manager.create(
            config=config, indicator_code="PMI", momentum_period=3, trend_weight=0.3
        )

        # CPI 趋势指标
        RegimeTrendIndicator._default_manager.create(
            config=config, indicator_code="CPI", momentum_period=3, trend_weight=0.3
        )
