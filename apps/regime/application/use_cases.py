...
from apps.regime.application.regime_v2_use_cases import (
    CalculateRegimeV2Request as CalculateRegimeV2Request,
)
from apps.regime.application.regime_v2_use_cases import (
    CalculateRegimeV2Response as CalculateRegimeV2Response,
)
from apps.regime.application.regime_v2_use_cases import (
    CalculateRegimeV2UseCase as CalculateRegimeV2UseCase,
)
    ...
    def execute(self, request: CalculateTermSpreadRequest) -> CalculateTermSpreadResponse:
        ...
        try:
            ...
            long_yield_data = self.repository.get_by_code_and_date(
                code=long_code, observed_at=request.as_of_date
            )

            short_yield_data = self.repository.get_by_code_and_date(
                code=short_code, observed_at=request.as_of_date
            )
            ...
            if not long_yield_data or not short_yield_data:
                ...
                long_yield_data = self.repository.get_latest_observation(
                    code=long_code, before_date=request.as_of_date
                )
                short_yield_data = self.repository.get_latest_observation(
                    code=short_code, before_date=request.as_of_date
                )
...
class HighFrequencySignalUseCase:
    ...
    def execute(self, request: HighFrequencySignalRequest) -> HighFrequencySignalResponse:
        ...
        try:
            ...
            all_agree = all(ind["signal"] == signal_direction for ind in contributing_indicators)
    ...
    def _evaluate_term_spread(self, as_of_date: date) -> dict[str, Any]:
        ...
        try:
            ...
            spread_data = self.repository.get_latest_observation(
                code="CN_TERM_SPREAD_10Y2Y", before_date=as_of_date
            )
    ...
    def _evaluate_nhci(self, as_of_date: date, lookback_days: int) -> dict[str, Any]:
        ...
        try:
            ...
            current_data = self.repository.get_latest_observation(
                code="CN_NHCI", before_date=as_of_date
            )
            ...
            past_data = self.repository.get_latest_observation(
                code="CN_NHCI", before_date=past_date
            )
    ...
    def _evaluate_us_bond(self, as_of_date: date) -> dict[str, Any]:
        ...
        try:
            ...
            us_bond_data = self.repository.get_latest_observation(
                code="US_BOND_10Y", before_date=as_of_date
            )
            ...
            return {"success": True, "value": us_yield, "signal": signal, "score": score}
        except RECOVERABLE_REGIME_USE_CASE_EXCEPTIONS as e:
...
class CalculateRegimeUseCase:
    ...
    def _check_data_completeness(
        ...
        if not growth_series or len(growth_series) < self.MIN_DATA_POINTS:
            ...
            logger.warning(
                f"Growth indicator {growth_code} has insufficient data: {len(growth_series) if growth_series else 0}"
            )

        if not inflation_series or len(inflation_series) < self.MIN_DATA_POINTS:
            missing.add(inflation_code)
            logger.warning(
                f"Inflation indicator {inflation_code} has insufficient data: {len(inflation_series) if inflation_series else 0}"
            )
    ...
    def _fill_missing_data(
        ...
        for indicator_code in missing_indicators:
            ...
            last_observation = self.repository.get_latest_observation(
                code=indicator_code, before_date=end_date
            )

            if last_observation:
                logger.info(
                    f"Filled {indicator_code} with last value: {last_observation.value} from {last_observation.reporting_period}"
                )
    ...
    def execute(self, request: CalculateRegimeRequest) -> CalculateRegimeResponse:
        ...
        if not request.skip_cache and not is_pytest_env:
            try:
                ...
                if cached_data:
                    logger.info(
                        f"Regime缓存命中: {request.as_of_date}, {request.growth_indicator}/{request.inflation_indicator}"
                    )
        ...
        try:
            ...
            if missing_indicators:
                logger.info(
                    f"Missing indicators detected: {missing_indicators}, attempting forward fill"
                )
            ...
            if missing_indicators and self._is_critical_data_missing(missing_indicators):
                logger.warning(
                    f"Critical data missing after fill attempt: {missing_indicators}, using fallback regime"
                )
                try:
            ...
            inflation_z_scores = calculate_rolling_zscore(
                inflation_momentums, window=60, min_periods=24
            )
            ...
            # 降级方案失败
            return CalculateRegimeResponse(
                success=False, snapshot=None, warnings=warnings_list, error=f"计算失败: {str(e)}"
            )
        except RECOVERABLE_REGIME_USE_CASE_EXCEPTIONS as e:
            # 其他异常
