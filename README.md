...
[![Tests](https://img.shields.io/badge/tests-governed-brightgreen.svg)](#测试)
[![Architecture](https://img.shields.io/badge/架构-DDD-purple.svg)](#架构)
[![MCP Tools](https://img.shields.io/badge/MCP_工具-governed-orange.svg)](#ai-原生集成)
...
- Tushare SDK 路由按其单一 Data API URL 协议发送请求，endpoint 与密钥继续由 Data Center / Config Center 和部署环境管理，不写入仓库。发布前的候选绑定门禁会联合校验真实响应与单位重放、完整 universe 容量、隔离 PostgreSQL 写入/回滚和固定 24 项 PostgreSQL 回归；容量结论绑定 active 发布策略与原始运行回执，并按 provider 窗口配额、任务时限、PostgreSQL 全集群连接、锁等待和容器内存峰值重新计算。真实探针先冻结注册证券范围，再从目标日估值形成 eligible/excluded 范围，只要求 eligible 样本具有精确行情，正常停牌不会造成永久假失败。GitHub Actions 官方接口同时核对同 SHA 成功 run、官方 artifact ZIP 摘要及两份 JUnit 原始字节。缺失、过期、mock、partial、blocked、skip 或候选不一致都会阻断部署。
- VPS 发布采用“先构建、再预演、后切换”：`scripts/run_release_rehearsal.py` 是正式的一键预演入口，在代码检出目录之外创建独占证据目录，构建一次候选镜像并按顺序完成真实 provider 探针、响应与单位重放、完整 universe 容量、隔离 PostgreSQL 写入回滚、同 SHA CI 证据和不可变打包。最终 `deploy-vps.ps1` 只复用预演回执绑定的 release tag/image ID，并在读取凭据、SSH 交接和服务切换前复验整套证据、OCI revision、镜像 ID 与 manifest SHA-256；部署入口不会重新构建未经预演的镜像。
...
- Linux 预演目录只向候选镜像主组开放阶段性写权限，命令结束或失败后立即收紧，并用保持打开的目录描述符拒绝符号链接、路径替换和 inode 复用；要求独占创建输出目录的生产器写入挂载根目录下尚不存在的子目录，避免把既存 bind mount 误传为可创建的输出目录。provider 身份由活动数据库连接的实际路由字段和已安装客户端版本生成，真实调用前后都会复核；密钥和动态健康指标不进入身份摘要，错误版本、旧端点或运行中配置漂移都会阻断发布。
...
- Tushare `daily_basic` 和全市场 `daily` 行情快照均改为按交易日批量取数，避免一次发布产生数千次逐股请求；`total_mv/circ_mv` 从万元转换为元，`daily` 行情快照与历史日线统一把成交量从手转换为股、成交额从千元转换为元。只有交易日期时统一把中国市场 15:00 收盘时刻写入 `observed_at`，不再因缺少源观测时间拒绝估值 Publication，也不使用抓取时间冒充市场时间。
...
- 生产 Python 类型债务与大文件治理豁免已清零，跨平台 mypy、架构边界、治理一致性和自动化门禁完成合并后校正，Windows / Linux CI 口径保持一致
- Web → TUI 迁移证据、回滚点与发布前检查已集中归档；Celery 批量任务同步启用规范化业务 `outcome` 契约，任务监控不再只依赖 Celery 自身状态
...
- Pulse 按需重算前现在会先刷新上游宏观输入，减少 stale / degraded 快照反复重建成弱质量结果的问题
- Alpha Qlib / cache 读链再次收口：应用层缓存访问已回到 repository 边界内，架构护栏恢复稳定，当前 `main` 上整条 nightly（unit / API / integration / app-local / guardrail / Playwright smoke）已重新全绿

### 2026-04-20

- Dashboard、SDK 和 MCP 的 Alpha 候选接口现在统一支持 `pool_mode`（`strict_valuation` / `market` / `price_covered`），并通过共享 `contract` 字段明确区分“研究排序”、“异步刷新中”和“可行动推荐”
- Alpha 账户池控制项已进入系统设置，配合价格覆盖同步与资产主数据回填，账户驱动股票池的稳定性和可解释性都有提升
- 决策工作台、首页 Alpha 和 MCP 刷新动作现在使用一致的账户池语义，跨页面 / SDK / MCP 对同一组合的观察口径更一致
...
- 宏观数据源新增“测试连接”能力，管理员可以直接在页面上触发 Tushare / AKShare / QMT 等探针并查看返回日志
- `market_data` 的 Provider 展示已并回统一数据源中心，系统配置中心与数据源清单口径同步
- `macro` 相关四层架构越层导入已修复，GitHub Actions 的 `Architecture Layer Guard` / `Logic Guardrails` 已恢复全绿

### 2026-04-04

- Equity Detail 新增技术图表主视图，补齐日线 / 分时数据读取、前端图表渲染和相关 API 契约
- 个股页上下文进一步丰富：除了估值与基本面，还会展示更完整的市场状态、数据来源与技术面辅助信息
- 当本地 Qlib 运行时不可用时，Alpha 推荐链路现在会自动复用最近可用缓存，而不是直接掉成空结果
- 系统设置中心、管理台、MCP 工具页、管理员文档页和服务日志页已经统一到更一致的管理界面风格

### 2026-04-03

- RSS 抓取链路已修复超时悬挂问题，源配置现在支持 timeout / retry / RSSHub / proxy 等前端可见参数
- 默认 RSS 源初始化脚本已切到一组可工作的 feed 地址，公开演示环境不再依赖失效的 `rsshub.app`
- 开发环境 `runserver` 日志现在会持久化到文件，便于本地排查启动问题和回看最近开发日志
...
> 本地虚拟环境目录默认叫 `agomtradepro/`，在 `.gitignore` 里，**不会提交到 git**。
>
> 如果你是仓库贡献者而不是普通体验用户，可改用 `install.bat --dev` 安装 pytest / Playwright / mypy 等开发工具。
...
本地快速体验不需要 `DATABASE_URL`。

- 复制 `.env.example` 后直接 `migrate`，项目会默认使用根目录下的 SQLite
- 如果后续确实要切 PostgreSQL，再按部署文档单独配置即可
...
运行 `python manage.py bootstrap_local_env` 后，你甚至不需要手工改任何密钥：
...
- **它不是单点脚本，而是可扩展架构**：多业务模块、DDD 四层、明确边界，适合继续长功能
...
**MCP 工具体系**不是只覆盖几个查询接口，而是横跨宏观、政策、信号、回测、账户、组合、交易、AI 能力目录、终端命令、运行时编排、系统配置等多个系统面向。

最近这轮 MCP 契约收口，建议额外记住三点：

- `get_dashboard_alpha_candidates(..., pool_mode=...)` 与 `trigger_dashboard_alpha_refresh(..., pool_mode=...)` 现在都支持显式账户池模式
- 读取 Alpha 候选时必须先检查返回里的 `contract.recommendation_ready` 与 `contract.must_not_treat_as_recommendation`
- `get_pulse_current()` 返回 canonical `/api/pulse/current/` JSON；当当前 Regime 只能解析为 `Unknown` 时，后端会保留最近有效快照，而不是把战术上下文直接覆盖成未知状态
...
| **测试** | Pytest（当前规模口径见 [docs/governance/SYSTEM_BASELINE.md](docs/governance/SYSTEM_BASELINE.md)）、Playwright（E2E） |
