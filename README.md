...
- Linux 预演目录只向候选镜像主组开放阶段性写权限，命令结束或失败后立即收紧，并拒绝符号链接或目录替换；要求独占创建输出目录的生产器写入挂载根目录下尚不存在的子目录，避免把既存 bind mount 误传为可创建的输出目录。provider 身份由活动数据库连接的实际路由字段和已安装客户端版本生成，真实调用前后都会复核；密钥和动态健康指标不进入身份摘要，错误版本、旧端点或运行中配置漂移都会阻断发布。
