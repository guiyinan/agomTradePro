...
**一个 AI-native 的个人投资研究底座：把宏观判断、策略纪律、Agent 能力和执行工作流放进同一个系统里。**
...
> 公开仓库会持续补全生产化细节、监控能力、文档和界面打磨；现阶段更适合把它理解为一个**可运行的投资基础设施系统**，而不是已经完全冻结的成品 SaaS。
...
*第一次启动即可完成管理员、AI Provider、数据源配置，适合 public 仓库访客快速理解系统入口。*
